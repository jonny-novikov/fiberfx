...
Memory graph + stale-reference toolchain for the fireheadz `memory/` corpus.
...
hugot-memory scan --root /Users/jonny/dev/fireheadz/memory --format ndjson | wc -l

# Render the cross-reference graph for visualization.
hugot-memory graph --root /Users/jonny/dev/fireheadz/memory --format dot --out /tmp/memory.dot
dot -Tsvg /tmp/memory.dot > /tmp/memory.svg

# Detect stale references at warn or higher.
hugot-memory stale --root /Users/jonny/dev/fireheadz/memory --severity warn

# Composite audit; non-zero exit on errors. Use in CI.
hugot-memory audit --root /Users/jonny/dev/fireheadz/memory --max-warn 50
```
...
1. `--config <path>`
2. `<root>/hugot-memory.yaml`
3. `<root>/.hugot-memory.yaml`
4. Baked-in defaults

Schema lives in `internal/config/config.go`. The corpus-tied config for the
fireheadz memory tree lives at `memory/hugot-memory.yaml`.

## Architecture

```
cmd/hugot-memory/      # Cobra root + subcommands
