...
- **emq3.1** — The verbs: `XADD` / `XRANGE` / `XREADGROUP` / `XACK` / `XAUTOCLAIM` registered as a no-break minor.
- **emq3.2** — The writer law: hash-tagged streams, branded ids, `append == mint` so byte order is time order.
- **emq3.3** — Readers: consumer groups + a polyglot handler; at-least-once, idempotent. Build-ready.
- **emq3.4** — Retention as policy: `MAXLEN ~` / `MINID` by mint instant; open fork F3.4-A on cadence (Dive 11.2).
- **emq3.5** — The archive: a fold consumer commits trimmed slices to native `EchoStore.Graft`, fold-before-trim; the merge-read below.
