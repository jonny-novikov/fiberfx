...
// New builds the full memory command tree (scan/graph/stale/audit/version) as a
// reusable *cobra.Command so a host binary can mount it as a subcommand. Pass a
// non-empty use/short to rename it (e.g. "memory") when nesting under a parent
// command; pass nil writers to default to os.Stdout/os.Stderr.
func New(use, short string, stdout, stderr io.Writer) *cobra.Command {
	cmd := newRootCmd(rootConfig{Stdout: stdout, Stderr: stderr})
	if use != "" {
		cmd.Use = use
	}
	if short != "" {
...
func resolveRoot(explicit string) (string, error) {
	...
	// .msh-memory.json (walk-up) pins the corpus root once so every `msh memory`
	// invocation auto-resolves it — no --root needed. It takes precedence over
	// the MEMORY.md marker below.
	if mc, err := LoadMemoryConfig(""); err == nil && mc != nil && mc.Root != "" {
		if abs, err := filepath.Abs(mc.Root); err == nil {
			if _, err := os.Stat(abs); err == nil {
				return abs, nil
			}
		}
