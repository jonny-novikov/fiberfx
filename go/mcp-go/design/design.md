...
Therefore, this is the core package layout, assuming github.com/fiberfx/mcp-go/v2 as the module path.

- `github.com/fiberfx/mcp-go/v2/mcp`: the bulk of the user facing API
- `github.com/fiberfx/mcp-go/v2/jsonschema`: a jsonschema implementation, with validation
- `github.com/fiberfx/mcp-go/v2/internal/jsonrpc2`: a fork of x/tools/internal/jsonrpc2_v2
...
Additionally, specs define a simpler (yet stateful) HTTP transport:
...
Rate limiting can be configured using middleware. Please see [examples/rate-limiting](<https://github.com/fiberfx/mcp-go/tree/main/examples/rate-limiting>) for an example on how to implement this.
...
type ClientOptions struct {
  ...
  ResourceUpdatedHandler func(context.Context, *ClientSession, *ResourceUpdatedParams)
}
```
...
```go
func (*Server) ResourceUpdated(context.Context, *ResourceUpdatedParams) error
```
...
The SDK will be hosted under github.com/fiberfx/mcp-go/v2, MIT license, copyright "Go SDK Authors". Each Go file in the repository will have a standard copyright header. For example:
