...
import (
	...
	"github.com/fiberfx/mcp-go/v2/internal/json"
)
...
func bindConnection(bindCtx context.Context, rwc io.ReadWriteCloser, binder Binder, onDone func()) *Connection {
	// settled(aaw): event-span creation and cancellation propagation from ctx
	// are upstream design questions; current behavior kept.
	ctx := notDone{bindCtx}
		...
		case *Response:
			c.updateInFlight(func(s *inFlightState) {
				if ac, ok := s.outgoingCalls[msg.ID]; ok {
					...
					// settled(aaw): unexpected responses are dropped without report;
					// a reporting channel is an upstream design question, not
					// pursued in this fork.
				}
			})
		...
		if err := req.ctx.Err(); err != nil {
			c.updateInFlight(func(s *inFlightState) {
				if s.writeErr != nil {
					...
					// settled(aaw, upstream #51365): plumbing the cause through
					// req.ctx awaits an upstream Context API; the wrapped error kept.
...
func (c *Connection) processResult(from any, req *incomingRequest, result any, err error) error {
	...
	if err != nil {
		// settled(aaw): the error goes to the event log only; further handling
		// (and the right log label) is an upstream open question.
	}
...
func (c *Connection) write(ctx context.Context, msg Message) error {
	...
	//
	// settled(aaw, upstream rfindley): cancellation notifications are rejected
	// during shutdown like all writes (even though writes could still succeed);
	// relaxing this is an upstream design question.
	c.updateInFlight(func(s *inFlightState) {
		err = s.shuttingDown(ErrServerClosing)
	})
	if err == nil {
