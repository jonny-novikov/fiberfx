...
func newMethodInfo[P paramsPtr[T], R Result, T any](flags methodFlags) methodInfo {
	return methodInfo{
		...
		// settled(aaw, upstream jba): reflection stays; a generics approach
		// (treating R as the unpointered type) is an upstream design question.
...
const (
	...
	//
	// settled(aaw, upstream rfindley): upstream flags this code value as wrong
	// (inconsistent with other SDKs); changing the wire error code would
	// diverge protocol behavior, so the value is kept as-is in this fork.
...
func notifySessions[S Session, P Params](sessions []S, method string, params P, logger *slog.Logger) {
	...
	// standalone stream.
	// settled(aaw): the 10s notify timeout stays fixed; configurability or an
	// asynchronous handleNotify is an upstream design question.
	ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
	defer cancel()

	// settled(aaw): a potential spec violation (feature-list change before the
	// client or server session is initialized) is acknowledged upstream; a fix
	// would change protocol behavior, not pursued in this fork.
