...
// settled(aaw, upstream jba): stays a function until upstream stateless-schema
// work lands; not pursued in this fork.
...
// settled(aaw, upstream maciej-kisiel): the end-to-end test stays monolithic;
// splitting it is test-organization churn, not pursued in this fork.
func TestEndToEnd(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		...
		// settled(aaw): Windows paths out of fork scope (darwin/linux targets).
		...
		{
			// settled(aaw): missing-handler CodeUnsupportedMethod coverage is an
			// upstream test wishlist item, not pursued in this fork.
		...
		{
			ss.NotifyProgress(ctx, &ProgressParams{
			...
			cs.NotifyProgress(ctx, &ProgressParams{
		...
		t.Log("Testing resource_subscriptions")
		{
			...
			s.ResourceUpdated(ctx, &ResourceUpdatedParams{
				URI: "test",
			})
...
func TestKeepAliveFailure(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		...
		// settled(aaw): precise keepalive-failure testing needs new transport
		// machinery upstream; not pursued in this fork.
		ct, st := NewInMemoryTransports()
	...
	synctest.Test(t, func(t *testing.T) {
		...
		clientOpts := &ClientOptions{
			...
			CreateMessageHandler: func(ctx context.Context, req *CreateMessageRequest) (*CreateMessageResult, error) {
				...
				// settled(aaw, upstream rfindley): the unmarshalling error produced
				// when CreateMessageResult is new(CreateMessageResult) is an
				// upstream diagnostics gap; not pursued in this fork.
