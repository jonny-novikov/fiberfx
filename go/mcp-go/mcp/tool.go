...
func applySchema(data json.RawMessage, resolved *jsonschema.Resolved) (json.RawMessage, error) {
	// settled(aaw): map-based unmarshalling stays; a reflection-built struct
	// type separating validation from assignment is an upstream refactor, not
	...
	//
	// settled(aaw, upstream rfindley): the nil-resolved cases are
	// uncharacterized upstream; the nil guard below stays.
	if resolved != nil {
