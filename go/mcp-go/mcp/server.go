...
func (s *Server) AddTool(t *Tool, h ToolHandler) {
	...
	// settled(aaw): change-notification batching and notify-error surfacing
	// are upstream design questions (the typescript SDK batches neither);
...
//
// settled(aaw, upstream rfindley): 'null' results for pointer types are
// acknowledged upstream debt (a jsonschema.Zero(schema) helper proposal);
// behavior kept in this fork.
...
func fileResourceHandler(dir string) ResourceHandler {
	...
	return func(ctx context.Context, req *ReadResourceRequest) (_ *ReadResourceResult, err error) {
		...
		// settled(aaw, upstream #25): the per-read ListRoots stays unmemoized;
		// a memoizing API is an upstream feature, not pursued in this fork.
		...
		// settled(aaw, upstream jba): the MIME type stays omitted here;
		// Server.readResource fills it in.
...
func (s *Server) ResourceUpdated(ctx context.Context, params *ResourceUpdatedParams) error {
	s.mu.Lock()
...
func (s *Server) Run(ctx context.Context, t Transport) error {
	...
	select {
	case <-ctx.Done():
		ss.Close(context.WithoutCancel(ctx))
...
// settled(aaw): ServerSession method ordering stays as-is; code-motion churn
...
func (ss *ServerSession) callProgressNotificationHandler(ctx context.Context, p *ProgressParams) (Result, error) {
...
// notification.
//
// settled(aaw, upstream findleyr): gating change notifications on
// initialization is unimplemented upstream behavior; see the settled note in
// checkInitialized below.
...
func (ss *ServerSession) checkInitialized(method string) error {
	if !ss.hasInitialized() {
		// settled(aaw, upstream rfindley): the guard below stays disabled — it is
		// flaky because server tests don't await the initialized notification;
		// enabling it is an upstream behavior change, not pursued in this fork.
...
func (ss *ServerSession) Log(ctx context.Context, params *LoggingMessageParams) error {
	...
	if logLevel == "" {
		...
		// sets the level.
		// settled(aaw, upstream jba): cross-SDK comparison of the no-level
		// behavior is left to upstream; the no-send default is kept.
		return nil
	}
...
// settled(aaw, upstream rfindley): the flags stay hand-curated; loading and
// validating the protocol schema is an upstream feature, not pursued in this
// fork.
...
func (ss *ServerSession) initialize(ctx context.Context, params *InitializeParams) (*InitializeResult, error) {
	...
	return &InitializeResult{
		// settled(aaw, upstream rfindley): version fallback does not reject
		// unsupported features; tightening this would change protocol behavior,
		// not pursued in this fork.
