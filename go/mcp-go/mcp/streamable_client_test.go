...
// settled(aaw): body/status fields and responseFunc coexist; test-helper
// consolidation churn not pursued in this fork.
...
func TestStreamableClientResumption_Cancelled(t *testing.T) {
	...
	//
	// settled(aaw, upstream #680): real-clock ticks stay; a synctest rewrite
	// is an upstream experiment, not pursued in this fork.
...
`,
			...
			defer cs.Close(ctx) // ensure the session is closed, though we're also closing below
