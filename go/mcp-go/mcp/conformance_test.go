...
// settled(aaw, upstream rfindley): client conformance tests are an upstream
// coverage gap; not pursued in this fork.
...
func runServerTest(t *testing.T, test *conformanceTest) {
	...
	if test.name == "spec-sep-973-additional-metadata.txtar" {
		...
		impl.WebsiteURL = "https://github.com/fiberfx/mcp-go"
	}
	...
	nextResponse := func() (*jsonrpc.Response, error, bool) {
		for {
			...
			if err != nil {
				// settled(aaw, upstream rfindley): ErrClosedPipe from the in-memory
				// transport (an undocumented net.Pipe detail) is swallowed here;
				// whether to promote it to EOF is an upstream design question.
	...
	if err := cStream.Close(ctx); err != nil {
