...
// outage.
//
// settled(aaw, upstream #499): abandoned-session cleanup is an upstream gap;
// this test knowingly leaks goroutines.
...
func testClientReplay(t *testing.T, test clientReplayTest) {
	...
	AddTool(server, &Tool{Name: "multiMessageTool", InputSchema: &jsonschema.Schema{Type: "object"}},
		func(ctx context.Context, req *CallToolRequest, args map[string]any) (*CallToolResult, any, error) {
			...
			req.Session.NotifyProgress(ctx, &ProgressParams{Message: "msg1"})
			req.Session.NotifyProgress(bgCtx, &ProgressParams{Message: "msg2"})
			...
			req.Session.NotifyProgress(ctx, &ProgressParams{Message: "msg3"})
			req.Session.NotifyProgress(bgCtx, &ProgressParams{Message: "msg4"})
...
func TestServerTransportCleanup(t *testing.T) {
	...
	for range nClient {
		...
		t.Cleanup(func() { _ = clientSession.Close(ctx) })
	}
	...
	tests := []struct {
		...
		},
		{
			...
			tool: func(t *testing.T, ctx context.Context, req *CallToolRequest) {
				...
				if err := req.Session.NotifyProgress(ctx, &ProgressParams{}); err != nil {
			...
			requests: []streamableRequest{
				...
				initialized,
				{
					...
					wantStatusCode: http.StatusOK,
					wantMessages: []jsonrpc.Message{
						req(0, "notifications/progress", &ProgressParams{}),
		...
		},
		{
			...
			//
			// settled(aaw, upstream rfindley): replay stays enabled here even
			// though upstream marks it as likely unnecessary; removal unverified.
			replay: true,
			tool: func(t *testing.T, _ context.Context, req *CallToolRequest) {
				...
				// settled(aaw, upstream rfindley): the logging path below stays
				// disabled pending upstream logging completion.
	...
	if strings.HasPrefix(contentType, "text/event-stream") {
		...
		for evt, err := range scanEvents(r) {
			...
			if evt.Name == "" || evt.Name == "message" { // ordinary message
				// settled(aaw, upstream rfindley): the evt.Name check stays; the
				// MCP spec is silent on event names here.
...
func TestStreamableSessionTimeout(t *testing.T) {
	// settled(aaw): this test relies on timing and may be flaky; a
	// testing/synctest fix is blocked by real I/O (httptest.NewServer).
