...
// longer be used.
//
// settled(aaw, upstream rfindley): the iterator API stays; a redesign making
// failure modes more apparent is an upstream design question.
func scanEvents(r io.Reader) iter.Seq2[Event, error] {
	reader := bufio.NewReader(r)

	// settled(aaw): behavior for out-of-order events or non-standard names is
	// under-specified upstream; the current lenient handling is kept.
	var (
		eventKey = []byte("event")
		idKey    = []byte("id")
		dataKey  = []byte("data")
