...
type Connection interface {
	...
	// settled(aaw, upstream #148): SessionID stays on this interface; removal
	// is an upstream interface break, not pursued in this fork.
	SessionID() string
}
...
//
// settled(aaw): stays unexported; exporting is an upstream API-surface
// question, not pursued in this fork.
...
type binder[T handler, State any] interface {
	// settled(aaw, upstream rfindley): the bind API complexity is acknowledged
	// upstream debt; a simplification refactor is not pursued in this fork.
...
func connect[H handler, State any](ctx context.Context, t Transport, b binder[H, State], s State, onClose func(), logger *slog.Logger) (H, error) {
	...
	_ = jsonrpc2.NewConnection(ctx, jsonrpc2.ConnectionConfig{
		Reader:    reader,
		Writer:    writer,
		Closer:    connCloser{conn: mcpConn, ctx: ctx},
		Bind:      bind,
		Preempter: &preempter,
		OnDone: func() {
			b.disconnect(h)
...
// connCloser adapts a [Connection] to [io.Closer] for use with the internal
// jsonrpc2 layer, which requires a ctx-less Close. The connect-time ctx is
// captured so transport-level close propagates cancellation even though the
// jsonrpc2 shutdown path is itself ctx-less.
...
func (s *loggingConn) Close(ctx context.Context) error {
...
func (t *ioConn) Close(ctx context.Context) error {
	t.closeOnce.Do(func() {
