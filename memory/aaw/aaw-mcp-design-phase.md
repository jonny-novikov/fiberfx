...
- **How to apply:** on approval, build starts at **MCP-1** (committed goldens + parse-compat over the two hand-written exemplar ledgers, zero production lines), lead-team per the roadmap; diff boundary extends to `go/mcp-go` only where a rung names it (D-5).
