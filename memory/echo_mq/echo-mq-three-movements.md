...
**Streams on the certified `echo_wire`, additive-minor** (the `echomq:3.0.0` MAJOR a DEFERRED cutover until the tier is whole; wire fence FROZEN `echomq:2.4.2`, labels `echomq:2.6.x`). Six rungs / three milestones (`docs/echo_mq/emq.streams.md`): **S1 writer** (emq3.1 verb floor `7b44dc97` · emq3.2 `EchoMQ.Stream` writer law `b6ff483b`) · **S2 readers** (emq3.3 `EchoMQ.StreamConsumer` `71ce78cc` · emq3.4 retention `Stream.trim/4` + opt-in `StreamRetention` `e5cd3ea0`) · **S3 memory** (emq3.5 the archive ✅ · **emq3.6 time-travel + Table hydration NEXT**).
