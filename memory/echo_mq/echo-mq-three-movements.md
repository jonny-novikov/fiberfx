...
description: "EchoMQ program — echo/apps/echo_mq is THE v3 library. DEPTH lives on-disk at docs/echo_mq/program/emq.program.md (the operating manual + agent calibrations). Shipped emq.0. emq.1 emq.2 emq.3 — **MOVEMENT I CLOSED**. Epics layer + AAW Epic/corpus framework instrument live. Recalibration 2026-06-17 (D-1): Mars=code-quality gate+stories, Director verifies code+invariants, Apollo=Mentor ONLY (out of pipeline). Consumer pivot: codemojex (live) + echo_bot (planned Telegram); Exchange dropped; apps/echomq REMOVED. **Movement II OPEN — emq.4.1 (fair-lanes control plane: reassign + DESTRUCTIVE lane-drain) SHIPPED 2026-06-18, HIGH-risk (R3 mid-build → NORMAL→HIGH; gate = blast-radius mutation battery, not the ≥100 loop), conf 54; echomq:3.0.0 era. emq.4.2/4.3/4.4 SHIPPED → the groups family (emq.4) is PARITY-COMPLETE 2026-06-19 (4.4 `361fd663`, Fork B Arm 2 weighted multi-pop, conf 61). **emq.5 batches: 5.1 spine + 5.2 shaping (BatchConsumer/BatchShaper.Core) + 5.3 group-affinity (@gbclaim/bclaim/3) SHIPPED 2026-06-21 — conf 70, labels 2.5.0 held → 2.5.1; next emq.5.4 the partitioned finish.** Apollo Stage-7 Operator-grantable (D-4 process-doc reconcile + D-5 destructive-op eval).** **STREAM TIER (EchoMQ 3.0, emq3.1-3.6) RE-SEQUENCED 2026-06-22 as the ACTIVE delivery ahead of emq.5.4: emq3.1-3.5 SHIPPED; emq3.5 (the archive, the FIRST two-app echo_mq+echo_store rung — EchoStore.StreamArchive fold-then-trim into EchoStore.Graft, conf 78, e2d73e23, BUILD-GRADE) SHIPPED 2026-06-23; emq3.6 (time-travel read_window/5+read_since/4 + Table hydration EchoStore.StreamHydrator one-shot fold, conf 79, 2.6.5, 588d2773, NORMAL Trio BUILD-GRADE) SHIPPED 2026-06-23 — the Stream Tier (emq3.1-3.6) is WHOLE; NEXT the echomq:3.0.0 cutover OR the parked 2.x runway.** This memory = slim pointer + frontier."
metadata:
  node_type: memory
  type: project
...
run + D-5 destructive-op adversarial eval; both PROPOSE-only/docs-only). **emq.4.2/4.3/4.4 SHIPPED → the groups family (emq.4) is PARITY-COMPLETE 2026-06-19.** 4.2 group-aware recovery (`@greap_group`, a group-scoped stalled-sweep into the member's own lane). 4.3 the park-don't-poll metronome (`EchoMQ.Metronome` — one `BLPOP wake` blocker per queue + an idle-consumer registry, BEAM-message fan-out, opt-in; owns NO Valkey lease, decides in a pure `Metronome.Core`; `@gclaim` byte-frozen; `174e1d7f`). **emq.4.4 (weighted/deficit rotation + the starvation drill) SHIPPED 2026-06-19 `361fd663`** — Fork B ruled **Arm 2 (additive weighted multi-pop)**: a new `@gwclaim` serves a serviceable lane K=min(weight, ZCARD lane, glimit headroom) heads per ring rotation on one shared server-clock lease; weight rides a new `emq:{q}:gweight` HASH set by `weight/4`, served by `wclaim/3`; `@gclaim`/`claim/3` byte-FROZEN (equal round-robin + weighted coexist); conf 59→61; NORMAL+ (Mars-2 collapsed for zero findings, Apollo not mandated). **Craft (L-1):** for a fairness property a terminal depth-0 drill is a WEAK no-op-defeater (a no-rotation FIFO drain also empties every lane via the re-ring guard) — use a bounded-early-window INTERLEAVING witness. **RECONCILED 2026-06-19:** the emq.4 family fold is folded into the canon docs (emq.roadmap.md line 36/impl-index/ladder · emq.progress.md · emq.changelog.md spine+table · emq.4.md chapter PROPOSED→SHIPPED) — the docs had LAGGED (showed 55/4.3-RULED/4.4-next); now show conf 61, emq.4 CLOSED. Conformance lineage pinned: 52→54(4.1)→55(4.2)→57(ewr.2.5 pool_enqueue/order)→59(ewr.2.6 native_lock×2)→59(**4.3 metronome +0, a BEAM process/lease, NOT a wire scenario**)→61(4.4). Two planes: wire @wire_version FROZEN echomq:2.4.2, mix label climbs (2.4.4). **emq.5 (batches) family OPEN — emq.5.1 (batch-claim spine) SHIPPED 2026-06-20** via /echo-mq-ship Flat-L2 (`1c36b70a` foundation = the carve reconcile + the emq.5.1 triad / `bca36d0c` rung): the NEW inline `@bclaim` = a count-variant `ZPOPMIN emq:{q}:pending` loop ×N INSIDE the script (the non-grouped generalization of `@gwclaim`; ZCARD-clamp k=min(size,depth), one `TIME`, one batch lease deadline, per-member `HINCRBY attempts`; design §6.2, NOT a client-side LMPOP/ZMPOP) + `Jobs.claim_batch/4` (the manual-pull host API, pause-first, returns `{:ok,[{id,payload,att}]}` | `:empty`). Forks RULED: **D-1 the LOOP** (vs native count-arg) / **D-2 return-the-short-batch** (non-blocking; M=0/paused→:empty; oversized N>depth→depth — the min_size/timeout cadence stays 5.2's) / **D-3 +3 conformance → 64** (Operator chose granular over the +2→63 lean: `batch_claim`·`batch_claim_short`·`batch_partial_failure`) / **D-4 label 2.5.0** (the batches family opens; wire `@wire_version` FROZEN echomq:2.4.2 — label-plane only climbs). `@claim` + every shipped script BYTE-FROZEN (jobs.ex 94 ins/0 del). Declared-keys A-1 = `KEYS[1]=pending`/`KEYS[2]=active` PIN the {q} slot, the row `jk=ARGV[1]..id` rides it. Director verify (Mars-2 COLLAPSED, zero findings) = byte-freeze + **F-1-declared-keys-by-hand** + order-theorem + a net-zero `ZPOPMIN→ZPOPMAX` mutation caught 6× + **TWO independent ≥100 determinism loops** (100/0 each, different load shapes). **Craft (L-1):** INV3's prose called the `@claim`/`@bclaim` ARGV row-key base "a declared root by the A-1 rule" — TIGHTENED to "an ARGV base is NOT a declared root; the real braced `KEYS[]` pin the {q} slot, the row rides it" (the as-built code comment jobs.ex:192-196 already stated it right). F-1 is **gate-invisible on single-node Valkey** → the Director's manual declared-keys review is the ONLY defense, not any green gate. **emq.5.2 (min_size/timeout shaping) SHIPPED 2026-06-21** (`60de5dc8` rung / `47562bbb` fold): forks RULED **D-1 a NEW `EchoMQ.BatchConsumer` module** (a SIBLING of Consumer, NOT a mode — the per-rung lean REVERSED on the 5.3/5.4 family responsibility-boundary) + the pure `EchoMQ.BatchShaper.Core` (`decide/4`, injected clock); WATCH-DEPTH cadence (`pending_size/2` ZCARD, claim ONLY at flush — nothing leased during accumulation, so timeout and lease_ms stay independent) over the byte-frozen `@bclaim`; per-member verdict map `%{id=>:ok|{:error,r}}`, absent→fail-safe retry; conf 64→67; NORMAL (multi-seed, NO ≥100 loop — mints nothing, no new lease); Mars-2 COLLAPSED. **emq.5.3 (group-affinity batch) SHIPPED 2026-06-21** (`a299aa73` rung / `242375df` fold): the NEW additive `@gbclaim` (lanes.ex beside `@gwclaim`) = the `@gwclaim` ISOMORPH minus the gweight read — rotates the ring (LMOVE), serves a HOMOGENEOUS batch from the landed lane K=min(depth, glimit headroom), one TIME, one shared lease, gactive+=served, the re-ring guard + `Lanes.bclaim/3` (conn,queue,lease_ms). Forks RULED (Operator): **5.3-A additive @gbclaim** (NORMAL+, shipped @g*/@bclaim BYTE-FROZEN) / **5.3-B reuse gactive** / **5.3-C ring-rotated** (a NEW fork Venus surfaced at the reconcile — ring-rotated vs caller-named affinity; caller-named = a FUTURE additive `bclaim/4` extension, the codemojex/echo_bot dedicated-worker model). Label **D-2 2.5.1** (within-family PATCH, NOT Mars's next-free 2.6.0 minor — which would falsely open emq.6). Verify = declared-keys-by-hand + byte-freeze (lanes.ex 0-del) + a mutation k=depth→k=1 caught 6/10 + the INDEPENDENT ≥100 loop 100/100. **emq.5 family: 5.1 spine + 5.2 shaping + 5.3 affinity SHIPPED; NEXT emq.5.4** the partitioned finish (over byte-frozen `@complete`/`@retry`/`@schedule`, NORMAL — no new lease). **CALIBRATION COMPOUNDING** (Apollo passes `9b84245b` 5.1/5.2 · `ecb48cd9` 5.3): the echo-mq skills now carry declared-keys A-1/L-1 + conformance-mirror (implementor §3/§4), link-depth + verify-scope (architect §1), the POSITION-ENCODED label convention (implementor §3), the msh bracket-then-paren Lua-prose hygiene (program Framing), the additive-ISOMORPH build-craft (implementor §6) — and L-1/L-2 RE-FIRED GREEN on 5.3 (Mars cited declared-keys verbatim in @gbclaim's comment; Venus self-caught a link-depth defect + scoped her msh claim to the paths run).

## EchoMQ 3.0 · the Stream Tier (the ACTIVE delivery — re-sequenced 2026-06-22 ahead of emq.5.4)

**Streams on the certified `echo_wire`, additive-minor** (the `echomq:3.0.0` MAJOR a DEFERRED cutover until the tier is whole; wire fence FROZEN `echomq:2.4.2`, labels `echomq:2.6.x`). Six rungs / three milestones (`docs/echo_mq/emq.streams.md`): **S1 writer** (emq3.1 verb floor `7b44dc97` · emq3.2 `EchoMQ.Stream` writer law `b6ff483b`) · **S2 readers** (emq3.3 `EchoMQ.StreamConsumer` `71ce78cc` · emq3.4 retention `Stream.trim/4` + opt-in `StreamRetention` `e5cd3ea0`) · **S3 memory** (emq3.5 the archive ✅ · **emq3.6 time-travel + Table hydration ✅**) — the tier is WHOLE.

**emq3.5 (the archive) SHIPPED 2026-06-23 — the FIRST TWO-APP rung (echo_mq + echo_store, D-1; the dep arrow `echo_wire→echo_data→echo_mq→echo_store` FORCES the fold store-side — echo_mq has no echo_store/cubdb dep).** Store-side **`EchoStore.StreamArchive`** (+ `.Core` pure decide/resolve + `.Driver` the supervised :transient fold-then-trim consumer) folds trimmed `EchoMQ.Stream` segments into the native **`EchoStore.Graft`** engine (CubDB, reserved `@archive_base=:erlang.bsl(1,49)`) via the PUBLIC `VolumeServer.commit/3` (INV8 — engine UNTOUCHED) · merge-read over the branded `EVT` watermark **`W`** (the highest-folded id, NEVER the integer `head_lsn`) · offline box-loss restore (`:tigris`-tagged live leg). Bus additive-only: `EchoMQ.Stream.put/get/clear_archived` (the `emq:{q}:stream:<name>:archived` polyglot seam), conf 77→78, label 2.6.4. **HIGH-risk Squad** (Director + Venus + Mars×2 + Apollo), BUILD-GRADE. 6 LAW-4 commits on `04c01e77`: **`e2d73e23` the archive** · `df34e305` calibration · `d84770a6` D-5 · `7249422a` D-4 · `5794938f` Stage-6 · `20a717d5` Z-1.

**The run (rich — 6 learnings L-1..L-6 now in `.claude/agents/mars.md` + the echo-mq-architect/implementor skills + `emq.program.md`, `df34e305`):** a COMPACTION split it; a LAW violation (T-4 — a bare-Agent resume → a clean re-init: `aaw_init` + `TeamCreate` + `aaw_spawn`, **spawned==registered**); a TWO-LAYER pre-existing echo_store breakage (E-1 the `Durability.Adapter` 9-vs-12 module-collision red-at-HEAD, masking E-2 a half-finished 12→9 migration — both plugins dangling+missing `@impl`) → **D-4 delete orphan `core/adapter.ex` · D-5 DROP the stale `Durability.{Graft,Postgres}` plugins** (no backward compat — the EchoMQ 3.x durable-bus forward; facade defaults to SQLite, the native EchoStore.Graft + cubdb survive), each a SEPARATE scoped commit (LAW-4 split-by-concern); **R-1 the alloc-before-commit gap** (a two-phase durable write — `:arc_seq` advance THEN `commit/3` — leaked the allocator past uncommitted pages on a commit-conflict → `read_archive` crash; the Director's verify found it READING THE PRODUCTION ERROR BRANCHES, Mars-2 fixed peek_seq + commit_seq-on-success + read_archive skip-absent + an OCC-contention regression test). The ≥100 loop caught a real cross-VM/shared-Valkey leak (39/100 → 100/100 ×3 independent — a store suite on the shared Valkey :6390 MUST `on_exit`-purge its braced slot; `System.unique_integer` is per-VM). Pathspec commits held through the Operator's concurrent out-of-band index the whole way.

**emq3.6 (time-travel + hydration) SHIPPED 2026-06-23 — the S3-memory closer; the Stream Tier (emq3.1–3.6) is now WHOLE.** `588d2773`, NORMAL Trio (Venus + Mars; Apollo OPTIONAL, not-fired — the one-shot ruling held it NORMAL), Director-verified BUILD-GRADE (zero defects → Mars-2 collapsed). TWO-APP, additive. **BUS** (echo_mq 2.6.5): `EchoMQ.Stream.read_window/5` (CLOSED `[t0,t1]`, inclusive `to` via the NEW `maxid_ceil/1` = `"<ms>-0x3FFFFF"`, the max-22-bit-seq inverse of the shipped `minid_floor/1`) + `read_since/4` (open `[t0,∞)`) — additive over the byte-frozen `read/6`, ZERO new Lua; conformance `:stream_time_travel` 78→79. **STORE** (echo_store 2.0.1): NEW `EchoStore.StreamHydrator.hydrate_from_stream/5` — a ONE-SHOT fold of a live stream tail into an `EchoStore.Table` via `put/4` (versioned by the branded `EVT` id; newer-wins by mint order), NO compactor, source read-only; the `:tracking` fence maintains freshness (hydrate-then-fence == loader truth). **Reconcile correction (T-2):** fold via `put/4` (the PUBLIC versioned write), NOT `apply_coherence/4` (which only DROPS an L1 row; `apply_newer_wins/3` is a private defp) — an apply_coherence-based fold would silently leave the Table empty, a bug a naive test would pass. Determinism = honest multi-seed sweep (one-shot; controlled-instant tests via `min_for`-mint sidestep the same-ms hazard → NO ≥100 loop). The Director's independent lower-bound mutation (`minid_floor(t0)`→`"-"`) was CAUGHT (2 failures) + reverted net-zero. 5 Arms ruled D-2 (one rung · read_window+read_since · one-shot · live tail · land the scenario). Stage-6 fold flipped roadmap/progress/streams to tier-WHOLE; the pathspec commit held clean through the Operator's 145 foreign out-of-band files.
