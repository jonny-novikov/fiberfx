...
**STILL PENDING — own reconcile rungs (NOT done):** the course docs (`docs/echo/bcs` ≈253, `docs/redis-patterns` ≈368, `docs/echo/mesh`, `docs/echo/art`), the two `CLAUDE.md` manuals, the repo-internal `memory/` corpus, and `echo/apps/codemojex` code (still reads `EchoCache.Table`). The course-teaching memory notes ([[bcs-course]], [[redis-patterns-course]], [[redis-reframe-echomq]], [[mesh-course]]) intentionally keep `echo_cache` until their course docs reconcile — but the real code path is `echo/apps/echo_store`.
