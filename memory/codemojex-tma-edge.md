...
- Bundler `assets/vite.config.ts` → content-hashed ESM `../priv/static/game/game-[hash].js` + manifest (es2024, React **bundled** not external; NOT git-tracked — the deploy rebuilds it). Uploaded to edge.codemoji.games, dynamic-imported by the EdgeReact hook. See [[codemojex-livereact-render]].
