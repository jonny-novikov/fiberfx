...
| `/logic`, `/law`, `/physics`, `/ai-rabota` (+ each `/<section>/*`), plus the **Agile Agent Workflow** course **re-mounted one level deeper at `/course/agile-agent-workflow`** (+ `/course/agile-agent-workflow/*`; the bare `/agile-agent-workflow` 404s by design) | five **folder-routed courses**, the first four with mechanics identical to `/elixir` & `/health` via the shared `serveDirTree` (**chapters are directories**, **modules are files inside them**, so the URL tree mirrors the on-disk tree): **Logic** («Логика и принятие решений», `LOGIC_DIR`, default `/app/html/logic`); **Law** («Право повседневной жизни», `LAW_DIR`, default `/app/html/law`); **Physics** («Электричество и устройства: школьная физика дома», `PHYSICS_DIR`, default `/app/html/physics`); **AI-at-work** («ИИ на работе: считать, проверять, автоматизировать», `AI_RABOTA_DIR`, default `/app/html/ai-rabota`); **Agile Agent Workflow** («Agile Agent Workflow in Elixir» — pragmatic programming with Claude agents, `AGILE_AGENT_WORKFLOW_DIR`, default `/app/html/agile-agent-workflow`) — **the only section registered under `/course/`** (`main.go` mounts `/course/agile-agent-workflow` + `/course/agile-agent-workflow/*`, not the bare path), and a **three-level** course (chapter landing → module hub → deep-dive subpage) authored via the `agile-course-writer` skill + the `agile-expert` subagent, guided by `docs/agile-agent-workflow/CLAUDE.md`. Path-traversal-guarded by the shared `resolveUnder`. | each section's `index.html` (the agile course at `/course/agile-agent-workflow`) |
...
Section directories are env-overridable (`EGE_DIR`, `EDU_DIR`, `SCHOOL_DIR`, `FUTURE_DIR`, `ELIXIR_DIR`, `HEALTH_DIR`, `LOGIC_DIR`, `LAW_DIR`, `PHYSICS_DIR`, `AI_RABOTA_DIR`, `AGILE_AGENT_WORKFLOW_DIR`, also `INDEX_HTML`, `DISTR_DIR`, `GAME_HTML`, `VENDOR_DIR`, `MAP_DIR`, `ERROR_DIR`, `SITEMAP_XML`, `ROBOTS_TXT`, `LLMS_TXT`); they default to the container paths `/app/html/ege`, `/app/html/edu`, `/app/html/school`, `/app/html/future` (plus `/app/html/health`, `/app/html/logic`, `/app/html/law`, `/app/html/physics`, `/app/html/ai-rabota` and `/app/html/agile-agent-workflow` for the folder-routed Health, Logic, Law, Physics, AI-at-work and Agile-Agent-Workflow courses, `/app/html/map` for the 3D map, `/app/html/error` for the error pages, and `/app/html/sitemap.xml` + `/app/html/robots.txt` for SEO, plus `/app/html/llms.txt` for the hand-authored llms.txt map). **The whole served site lives under `/app/html/` — mirroring the repo's `html/` dir — with exactly two exceptions kept at the container root: the Elixir course at `/app/elixir` (`ELIXIR_DIR`, repo `elixir/`) and the distr tarballs at `/app/distr` (`DISTR_DIR`, repo `data/`).** `PORT` defaults to `8080`. The server ends in `log.Fatal(app.Listen(...))` — there is no in-code graceful shutdown; Fly sends `SIGTERM` (the binary is PID 1).
...
- `main.go` — the whole server (routing, the four flat `serve*` closures + the shared `serveDirTree` helper for the folder-routed `/elixir`, `/health`, `/logic`, `/law`, `/physics`, `/ai-rabota` and `/agile-agent-workflow` sections, the `resolveUnder` traversal guard, the central `ErrorHandler` + `renderError`/`loadErrorPages`, `/healthz` (JSON probe), `/sitemap.xml` + `/robots.txt` + `/llms.txt`).
