...
| `/health`, `/health/*` | `health/<path…>` — the **Health course** («Здоровье как прикладная математика»), folder-routed like `/elixir` via `serveDirTree`: **chapters are directories** (`/health/son` → `health/son/index.html`) and **modules are files inside them** (`/health/son/pamyat` → `health/son/pamyat.html`), so the URL tree mirrors the on-disk tree. `HEALTH_DIR` env-overridable, default `/app/health`. **Note:** the machine liveness probe is the separate **`/healthz`** route (below), NOT this. | `health/index.html` |
| `/logic`, `/law`, `/physics` (+ each `/<section>/*`) | three more **folder-routed courses**, identical mechanics to `/elixir` & `/health` via the shared `serveDirTree` (**chapters are directories**, **modules are files inside them**, so the URL tree mirrors the on-disk tree): **Logic** («Логика и принятие решений», `LOGIC_DIR`, default `/app/logic`); **Law** («Право повседневной жизни», `LAW_DIR`, default `/app/law`); **Physics** («Электричество и устройства: школьная физика дома», `PHYSICS_DIR`, default `/app/physics`). Path-traversal-guarded by the shared `resolveUnder`. | each section's `index.html` |
| `/` | `index.html` — lightweight, mobile-friendly landing **hub**: hero + a grid of **10 series cards** (school, school/geometria, future, edu/finances, ege, health, elixir, logic, law, physics) plus a topic filter-bar, each card linking into its series. Pure HTML/CSS, **no WebGL**; links to `/map`. | — |
...
| `/llms.txt` | **Hand-authored** markdown site map for LLMs/agents ([llmstxt.org](https://llmstxt.org) convention): H1 → blockquote summary → per-series `##` link-lists with **root-relative** URLs to every course hub + chapter. Served verbatim as `text/plain; charset=utf-8` (renders inline; agents read it raw). `LLMS_TXT` env-overridable, default `/app/llms.txt`. **NOT** produced by `cmd/sitemap` — edit `llms.txt` by hand when course/chapter structure changes. The **Elixir course additionally carries per-chapter maps**: `/elixir/llms.txt` (course index) and `/elixir/<chapter>/llms.txt` for each of the 7 chapter dirs (each chapter's full lesson tree, derived from page `<title>`+`<meta description>`). These are plain files committed under `elixir/`, served `text/plain` by **`serveDirTree`'s co-located-file branch** (not this route), and shipped by the whole-dir `COPY elixir/` — so they are static snapshots: **regenerate them by hand** if lessons are added/renamed (no committed generator). | — |
...
Section directories are env-overridable (`EGE_DIR`, `EDU_DIR`, `SCHOOL_DIR`, `FUTURE_DIR`, `ELIXIR_DIR`, `HEALTH_DIR`, `LOGIC_DIR`, `LAW_DIR`, `PHYSICS_DIR`, also `INDEX_HTML`, `DISTR_DIR`, `GAME_HTML`, `VENDOR_DIR`, `MAP_DIR`, `ERROR_DIR`, `SITEMAP_XML`, `ROBOTS_TXT`, `LLMS_TXT`); they default to the container paths `/app/ege`, `/app/edu`, `/app/school`, `/app/future` (plus `/app/elixir`, `/app/health`, `/app/logic`, `/app/law` and `/app/physics` for the folder-routed Elixir, Health, Logic, Law and Physics courses, `/app/map` for the 3D map, `/app/error` for the error pages, and `/app/sitemap.xml` + `/app/robots.txt` for SEO, plus `/app/llms.txt` for the hand-authored llms.txt map). `PORT` defaults to `8080`. The server ends in `log.Fatal(app.Listen(...))` — there is no in-code graceful shutdown; Fly sends `SIGTERM` (the binary is PID 1).
...
- `COPY index.html /app/index.html`, `COPY game.html /app/game.html`, `COPY sitemap.xml`/`robots.txt` (SEO, pre-generated), `COPY llms.txt /app/llms.txt` (the hand-authored llmstxt.org site map), `COPY ege/ /app/ege/`, `COPY edu/ /app/edu/`, `COPY school/ /app/school/`, `COPY future/ /app/future/`, `COPY map/ /app/map/` (the 3D orbital map), `COPY elixir/ /app/elixir/`, `COPY health/ /app/health/`, `COPY logic/ /app/logic/`, `COPY law/ /app/law/`, `COPY physics/ /app/physics/` (folder-routed Elixir, Health, Logic, Law and Physics courses), `COPY error/ /app/error/` (styled HTML error pages), `COPY assets/ /app/assets/` (vendored three.js) — **whole-directory copies** for the section dirs, so any new `.html` in them ships on the next deploy with no Dockerfile edit (but **re-run `make sitemap`** so new pages enter `sitemap.xml`, which is a single generated file, not a dir);
...
- `main.go` — the whole server (routing, the four flat `serve*` closures + the shared `serveDirTree` helper for the folder-routed `/elixir`, `/health`, `/logic`, `/law` and `/physics` sections, the `resolveUnder` traversal guard, the central `ErrorHandler` + `renderError`/`loadErrorPages`, `/healthz` (JSON probe), `/sitemap.xml` + `/robots.txt` + `/llms.txt`).
...
- `llms.txt` — **hand-authored** ([llmstxt.org](https://llmstxt.org)) markdown map of the whole site for LLMs/agents: the ten course series + their chapters as **root-relative** links, each with a one-line "what it's about". Committed at repo root, `COPY`'d into the image, served at `/llms.txt` (`text/plain; charset=utf-8`). Unlike `sitemap.xml` it is **not generated** — update it by hand when adding or renaming courses/chapters.
- `fly.toml`, `Dockerfile` — deployment + what ships into the image.
- `Makefile` — local dev driver (note the `GOWORK=off` and port 8765 quirks above); its `start`/`run` recipes export every section dir (the four flat sections plus the folder-routed Elixir/Health/Logic/Law/Physics).
