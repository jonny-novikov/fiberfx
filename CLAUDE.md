...
| `/elixir`, `/elixir/*` | `elixir/<path…>` — the **Elixir course**, a **folder-routed section** (the URL tree mirrors the filesystem; `/health` works identically): a wildcard route + `try_files`-style cascade (the shared `serveDirTree`) resolves a clean URL to `elixir/<path>/index.html` for **directories** (`/elixir`, `/elixir/algebra`), to `elixir/<path>.html` for **clean leaf URLs** (`/elixir/algebra/f1-01-functions`), or to an **exact co-located file** (e.g. an `.svg`). `ELIXIR_DIR` env-overridable, default `/app/elixir`. Path-traversal-guarded by the shared `resolveUnder` (a raw `..` segment in any path component → 403). | `elixir/index.html` |
