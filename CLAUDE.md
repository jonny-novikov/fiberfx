...
- `main.go` — the whole server (routing, the four flat `serve*` closures + the shared `serveDirTree` helper for the folder-routed `/elixir` and `/health` sections, the `resolveUnder` traversal guard, the central `ErrorHandler` + `renderError`/`loadErrorPages`, `/healthz` (JSON probe), `/sitemap.xml` + `/robots.txt` + `/llms.txt`).
