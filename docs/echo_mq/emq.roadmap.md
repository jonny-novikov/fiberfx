...
| **emq3.1–emq3.6** | **3.0** | **EchoMQ 3.0 — the Stream Tier** (S1 writer → S2 readers → S3 memory); ships additive-minor, the `echomq:3.0.0` major a deferred cutover ratification | ✅ **SHIPPED — the tier is WHOLE** — **emq3.1–3.6 ALL SHIPPED** (S1 the writer: the verb floor `7b44dc97` + the writer law `b6ff483b`; S2 the readers: `EchoMQ.StreamConsumer` `71ce78cc` + retention `e5cd3ea0`; S3 the memory: emq3.5 the archive conf 78 `e2d73e23` + emq3.6 time-travel + hydration conf 79 label 2.6.5); → the `echomq:3.0.0` cutover ratification is the next frontier ([`emq.streams.md`](./emq.streams.md)) |
