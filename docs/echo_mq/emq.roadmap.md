...
- **When.** The foundation is **established** (`emq.0`). **Movement I is CLOSED** (emq.1 · the emq.2
  ...
  **the emq.4 groups family is CLOSED** (4.1–4.4, conformance **61**) and **emq.5 (batches) is CLOSED**
  ...
  continuation"** (Operator-revisable, not deleted). **emq3.1–emq3.3 SHIPPED — S1 the writer (the verb floor `7b44dc97` + the writer law `EchoMQ.Stream`, conf 75, `b6ff483b`) + S2 the readers part 1: emq3.3 the reader law (`EchoMQ.StreamConsumer`, the BEAM consumer group + the polyglot seam, conf 76, `71ce78cc`); emq3.4 (retention as policy) is next.**
...
| **emq3.1–emq3.6** | **3.0** | **EchoMQ 3.0 — the Stream Tier** (S1 writer → S2 readers → S3 memory); ships additive-minor, the `echomq:3.0.0` major a deferred cutover ratification | 🔨 **IN FLIGHT** — **emq3.1–3.3 SHIPPED** (S1 the writer: the verb floor `7b44dc97` + the writer law conf 75 `b6ff483b`; S2 the readers part 1: the reader law `EchoMQ.StreamConsumer` conf 76 `71ce78cc`); **emq3.4 next** (retention as policy); gated on emq.0 (met) ([`emq.streams.md`](./emq.streams.md)) |
