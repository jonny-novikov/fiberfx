...
- **When.** The foundation is **established** (`emq.0`). **Movement I is CLOSED** (emq.1 · the emq.2
  ...
  per run — **the emq.4 groups family is CLOSED** (4.1–4.4, conformance **61**); **emq.5 (batches) is OPEN** — **5.1 spine + 5.2 shaping + 5.3 group-affinity SHIPPED** (`@bclaim` +
  `claim_batch/4` + `BatchConsumer` + `@gbclaim`/`bclaim/3`, conformance **70**, label `2.5.1`); **5.4 the partitioned finish next**. **EchoMQ 3.0 — the Stream Tier — is the headline delivery that follows**, landing the
  `3.0.0` major.
...
| emq.5 | II | batches: bulk consumption, `min_size`/`timeout` shaping, affinity, the partitioned finish | 🔨 **BUILDING — 5.1 spine + 5.2 shaping + 5.3 affinity ✅ SHIPPED** (`@bclaim` + `claim_batch/4` + `BatchConsumer` + `@gbclaim`/`bclaim/3`, **conformance 70**, label `2.5.1`); 5.4 the partitioned finish carved → [carve](./specs/emq2/emq.5/emq.5.md) · [5.1](./specs/emq2/emq.5/emq.5.rungs/emq.5.1.md) · [5.2](./specs/emq2/emq.5/emq.5.rungs/emq.5.2.md) · [5.3](./specs/emq2/emq.5/emq.5.rungs/emq.5.3.md) |
