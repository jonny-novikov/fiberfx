...
- **Movement I · The Core** — the v1 capability surface pushed to state of the art inside `echo_mq`:
  ...
  ✅ CLOSED — the full 2.1–2.4 floor shipped (Movement I CLOSED, 52/52)), the parent/flow family (**emq.3**). The frozen v1 line (`apps/echomq`, `1.3.0`) is a
...
| The 2.x line specification        | [`./specs/emq.2.specs.md`](./specs/emq.2.specs.md)                                                               | the BCS-side specification of the 2.x line's laws and surfaces — aligned with the program, never redesigning what the canon owns (the delivery view is the consolidated [`./emq.roadmap.md`](./emq.roadmap.md))                                                                                                                    |
| The 3.x stream tier specification | [`./emq.streams.md`](./emq.streams.md)                                                               | the PROPOSED next major: event streams, retention as declared policy, the archive in the `EchoStore.Graft` engine (CubDB → Tigris), time-travel by mint instant — awaiting Operator slot ratification (the delivery view is [`./emq.roadmap.md` §EchoMQ 3.x](./emq.roadmap.md))                                                    |
...
| The shipped record                | [`./emq.changelog.md`](./emq.changelog.md)                                                | per-rung shipped deliverables; the closed Movement I run ledgers are retired into it                                                                                                                                                                                                                                                                       |
