...
- **`emq.3.3.llms.md`** (the Mars brief for the LATER build): References (read-first) · Requirements (each traced
  to a story + forward to an invariant/check) · Execution topology (the runtime shape — the sweep process/cadence,
  the signal-key, the cross-queue add + the recovery; the build-order DAG; the EXACT files) · Agent stories
  (Directive + Acceptance gate). Authored to the ruled ADR arms so the build is build-ready.
...
docs/echo_mq/specs/emq.3.3.llms.md             (NEW)
...
> contract ([`./emq.3.3.md`](emq.3.3.md) authoritative · [`./emq.3.3.stories.md`](emq.3.3.stories.md) ·
> [`./emq.3.3.llms.md`](emq.3.3.llms.md)); the canon is [`../emq.design.md`](../../../emq.design.md); the binding is the
...
Directive (the `echo-mq-implementor` craft; build to [`./emq.3.3.llms.md`](emq.3.3.llms.md) Reqs 1–6 + the
...
docs/echo_mq/specs/emq.3.3.llms.md
...
· [`./emq.3.3.llms.md`](emq.3.3.llms.md) (the build brief) · [`./emq-3-3.progress.md`](../../progress/emq-3-3.progress.md)
