...
emq.1 opens Movement I: the bus's **time-and-retry vocabulary** for its named consumer, the Exchange
platform. It builds — inside `echo/apps/echo_mq` under the v2 laws, additive on the wire — scheduled
...
**Carried, not re-litigated:** the rung contract is Candidate B (Stage-1b); the consumer is the Exchange
platform; the ladder slot is emq.1. If the solo Director review (Stage 2) finds an adopted arm unsound or
...
[`../echo_mq.md`](../../echo_mq.md) M1's emq.1 status (this rung un-blocks the Exchange platform's TRD.3
scheduled/batched settlement trace); write/extend `emq-1.progress.md`; **surface the adopted six arms to
...
docs/echo_mq/specs/emq.1.llms.md
...
The consumer: [`../../exchange/exchange.specs.md`](../../../exchange/exchange.specs.md) §Jobs.
