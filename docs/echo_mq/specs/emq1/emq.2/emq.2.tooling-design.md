...
description: >-
  ...
  rung's spec triad (emq.N.md / .stories.md) to the v2 laws, carving a parity surface from the
...
(US-n in Connextra form + the standing `EMQ.N-US-GATE` Valkey gate story + a Coverage map), built to
...
- **Sync** the `emq.N.md` (and the derived `.stories.md`) to the as-built surface — record what
