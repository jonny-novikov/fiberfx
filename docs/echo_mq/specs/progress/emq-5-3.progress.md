...
- TRIAD: Venus Stage-5 sync (0 WITHHELD, forks RULED, as-built captured); the .prompt.md left as the frozen original scope (5.1/5.2 precedent); msh tree CLEAN (the 2 ledger false-positives — the KEYS[n](word) Lua-prose mis-parse — reworded to KEYS[n]=word, meaning preserved).
