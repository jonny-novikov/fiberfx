...
`registry/addJobScheduler-11.lua` — the original legacy v1 command, verbatim.
...
| v1 (addJobScheduler-11) | v3 (PARTIAL — Repeat.register/6 + Pump) |
|---|---|
| KEYS[1..11] repeat/delayed/wait/.../active | keys = [emq:{q}:repeat, emq:{q}:repeat:<name>] |
| prevMillis = ZSCORE(repeat, schedulerId) | if EXISTS KEYS[2] -> return 0 -- idempotent |
| id = "repeat:"..schedulerId..":"..nextMillis | HSET KEYS[2] every_ms/template |
| evict previous occurrence -- DATA-derived id | ZADD KEYS[1] <first_at> <name> |
| collision -> -11 SlotsBusy / -10 IdCollision | -- collision codes RETIRED: a fresh mint never collides |
| storeJobScheduler ; addJobFromScheduler | -- cadence host-side (Pump); next-at from server TIME |
...
**Covers → v3.** Upsert a scheduler + materialize its next occurrence → registration + cadence are shipped; the in-script slot-collision codes (`-10`/`-11`) have **no port** (fresh per-occurrence mint makes id collision structurally impossible).
