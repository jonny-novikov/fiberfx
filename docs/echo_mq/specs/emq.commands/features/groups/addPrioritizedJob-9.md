...
**BCS** fair lanes give the trading platform many-tenants-on-one-queue isolation (no noisy-neighbour starvation) a single global priority integer cannot. · **EchoMesh** consistency-first / coordinated end — bounded, fair, per-identity service is a server-side invariant computed on the bus, not a client hint a partition could distort. · **[when]** an Exchange venue's work admitted onto its own lane — one Lanes group per venue, no noisy-neighbour starvation.
