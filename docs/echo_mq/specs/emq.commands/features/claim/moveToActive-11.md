...
**BCS** the single-writer claim per instrument — work is drawn atomically and the lease is the only ownership proof, so the decider that prices an order is its sole holder. · **EchoMesh** consistency-first (§4 row 24) — the book surface refuses a second writer rather than risk divergence under partition. · **[when]** an Exchange decider claiming one instrument's work, the lease the only ownership proof.
