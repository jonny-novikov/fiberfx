...
| 2 | [F5.2](f5.2.md) · Model the domain | the Accounts/Catalog/Learning contexts and entities; enroll builds an enrollment | `iex` → `Learning.enroll("USR1","CRS1")` → `{:ok, %Enrollment{progress: 0}}`, retrievable | enforced-keys tests; `@type t`/`@spec` (Dialyzer-checkable); context-ownership check | the domain model and fields; context boundaries; the public API surface |
