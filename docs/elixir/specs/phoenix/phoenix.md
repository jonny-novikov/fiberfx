...
7. `f6-07` — The facade PubSub wrapper and supervision · broadcast domain events from the context · subscribe the
   LiveView and handle them into a stream · verify multi-client live updates · Presence and a live viewer count ·
   an optional raw channel.
8. `f6-08` — Generate auth with phx.gen.auth · protect routes with a plug + a require-auth pipeline · enforce auth in
   a LiveView with on_mount · runtime config reading env at boot · migrate without mix via a release command · cluster
   the nodes and deploy with build-migrate-boot.
9. `f6-09` — Build the dashboard LiveView read model, seeded from the F6.04 contexts, with a feed stream · subscribe
   to "events" on the connected mount · fold each broadcast in handle_info (bump a count, prepend a feed row) · render
   cards + feed in HEEx · track Presence for a live viewer count · route it under the F6.08 auth, read-only and
   clustered. The capstone over F5 and all of F6.
