...
| Module | Abstract | Route | Status |
|---|---|---|---|
| **F6.01 · Architecture & the request lifecycle** | Phoenix architecture and the request lifecycle: endpoint, router, controller, view. | `/elixir/phoenix/lifecycle` | ● ⬡ hub |
| ↳ F6.01 &middot; The request lifecycle | A request from the browser to the response: the endpoint plug stack, the router match, the controller action that calls the Portal facade, and the view that renders the result. | `/elixir/phoenix/lifecycle/request-path` | ● |
| ↳ F6.01 &middot; The endpoint, supervised | PortalWeb.Endpoint is both the outermost plug — static files, parsers, session — and a supervised process, one more child of the OTP tree the F5.09 lab assembled. | `/elixir/phoenix/lifecycle/endpoint` | ● |
| ↳ F6.01 &middot; Controllers, views & the facade seam | Where your code lives: a thin controller calls only the Portal facade and picks a view; the view renders assigns. The web layer translates HTTP to facade calls and back, nothing more. | `/elixir/phoenix/lifecycle/controllers` | ● |
| **F6.02 · Routing, controllers & plugs** | Routing, controllers, and the plug pipeline. | `/elixir/phoenix/routing` | ● ⬡ hub |
| ↳ F6.02 &middot; Routes & verbs | How a verb and a path map to one controller action: get, post, resources, and live routes, route params, and verified ~p paths. | `/elixir/phoenix/routing/routes` | ● |
| ↳ F6.02 &middot; Pipelines & scopes | A named pipeline is a reusable stack of plugs; a scope runs a group of routes through one. The :browser and :api pipelines, and how auth becomes one more pipeline. | `/elixir/phoenix/routing/pipelines` | ● |
| ↳ F6.02 &middot; Writing a plug | The plug contract — init/1 and call/2, conn in and conn out — and how halt/1 stops the pipeline. A module plug that requires a logged-in user, and a function plug. | `/elixir/phoenix/routing/plugs` | ● |
| **F6.03 · Ecto: schemas, changesets & queries** | Ecto schemas, changesets, and queries — data, validation, and the repo. | `/elixir/phoenix/ecto` | ● ⬡ hub |
| ↳ F6.03 &middot; Schemas & migrations | A migration creates and evolves a database table; a schema maps that table to an Elixir struct. The primary key is a Snowflake bigint we mint ourselves, not a serial. | `/elixir/phoenix/ecto/schemas` | ● |
| ↳ F6.03 &middot; Changesets & validation | A changeset is a pure pipeline — cast, validate, constrain — that produces a struct carrying valid? and errors before any write reaches the database. | `/elixir/phoenix/ecto/changesets` | ● |
| ↳ F6.03 &middot; Queries & the repo | The Repo runs composable queries and persists changesets, and it lives behind the engine's port — the F5.09 Postgres adapter — so the domain core still names no database. | `/elixir/phoenix/ecto/repo` | ● |
| **F6.04 · Contexts & domain design** | Contexts and domain design: boundaries that scale. | `/elixir/phoenix/contexts` | ● ⬡ hub |
| ↳ F6.04 &middot; Context boundaries | A context is a dedicated module that groups related functionality behind a public API and hides its schemas and the Repo. How to draw the line by cohesion, and why a schema is private to its context. | `/elixir/phoenix/contexts/boundaries` | ● |
| ↳ F6.04 &middot; Contexts vs the F5 facade | A Phoenix context and the hexagonal Portal facade from F5 are the same idea under two names — a public API over a slice of the domain. How they layer, and where Ecto sits when the port already hides it. | `/elixir/phoenix/contexts/vs-facade` | ● |
| ↳ F6.04 &middot; Composing contexts | How one context depends on another without breaking boundaries: call the public API, pass ids or public structs, never reach a foreign schema or Repo. A with pipeline across contexts that returns the closed error. | `/elixir/phoenix/contexts/composition` | ● |
| **F6.05 · Templates, components & HEEx** | Server-rendered markup with templates, components, and HEEx. | `/elixir/phoenix/heex` | ● ⬡ hub |
| ↳ F6.05 &middot; Templates & assigns | HEEx is the view half of the request the controller returns. A template renders the assigns the controller set — @courses, @course — with :for and :if, curly interpolation, and verified ~p routes, and never queries the database itself. | `/elixir/phoenix/heex/templates` | ● |
| ↳ F6.05 &middot; Function components & slots | A function component is a pure function from assigns to markup, declared with attr and slot. One &lt;.course_card&gt; replaces copied markup; render_slot/1 fills a slot, and attrs are validated at compile time. | `/elixir/phoenix/heex/components` | ● |
| ↳ F6.05 &middot; Forms & inputs | A form is backed by a changeset turned into a form with to_form/1. &lt;.form for={@form}&gt; and &lt;.input field={@form[:title]}&gt; render fields and show the F6.03 changeset errors, with the params flowing back to the context. | `/elixir/phoenix/heex/forms` | ● |
| **F6.06 · Phoenix LiveView fundamentals** | Phoenix LiveView fundamentals — interactive UIs without hand-written JavaScript. | `/elixir/phoenix/liveview` | ● ⬡ hub |
| ↳ F6.06 &middot; mount & assigns | A LiveView is a stateful process connected to the browser over a socket. mount/3 sets the socket's initial assigns from the facade; it runs twice — a disconnected HTTP render, then a connected mount — and connected?/1 guards side effects to the live connection. | `/elixir/phoenix/liveview/mount` | ● |
| ↳ F6.06 &middot; handle_event & state | Bindings like phx-click, phx-change, and phx-submit send events to handle_event/3, which transforms the socket assigns and returns {:noreply, socket}. The state lives on the socket; a search box filters the list and re-renders, no page reload. | `/elixir/phoenix/liveview/events` | ● |
| ↳ F6.06 &middot; render & diffs | render/1 returns HEEx from the assigns, and LiveView tracks which assigns changed, sending only the changed values over the socket. The HEEx static/dynamic split makes the diff tiny; streams keep large collections out of socket memory. | `/elixir/phoenix/liveview/render` | ● |
| **F6.07 · PubSub, channels & real-time** | PubSub, channels, and real-time updates over WebSockets. | `/elixir/phoenix/pubsub` | ● ⬡ hub |
| ↳ F6.07 &middot; Broadcasting engine events | Phoenix.PubSub is process-to-process publish/subscribe over a topic. The domain broadcasts an event after a successful write — the context emits it, not the web layer — and a thin facade wrapper keeps the PubSub server name and topics in one place. | `/elixir/phoenix/pubsub/broadcast` | ● |
| ↳ F6.07 &middot; Subscribing a LiveView | A LiveView subscribes to a topic on its connected mount and receives broadcasts in handle_info/2 — the process-message callback, distinct from handle_event/3 for browser events. It updates assigns and re-renders, so one user's write becomes everyone's live update. | `/elixir/phoenix/pubsub/subscribe` | ● |
| ↳ F6.07 &middot; Channels & presence | Channels are the lower-level real-time primitive LiveView is built on — explicit join and handle_in for custom client protocols. Presence tracks who is subscribed to a topic, synced across nodes by a CRDT, which is how a live viewer count works across a cluster. | `/elixir/phoenix/pubsub/presence` | ● |
| **F6.08 · Auth, deployment & going live** | Sessions and authentication, releases, and going to production. | `/elixir/phoenix/deployment` | ● ⬡ hub |
| ↳ F6.08 &middot; Sessions & authentication | mix phx.gen.auth generates an Accounts context, a User schema, and the session plumbing. The session is a signed cookie carrying a token; a plug loads the current user into conn.assigns, and a LiveView enforces auth with on_mount before the socket connects. | `/elixir/phoenix/deployment/auth` | ● |
| ↳ F6.08 &middot; Releases & config | mix release packages the app, its dependencies, and the BEAM into one self-contained artifact. config/runtime.exs is evaluated at boot and reads env vars for secrets and the database URL, and a release has no mix, so migrations run through a release command. | `/elixir/phoenix/deployment/releases` | ● |
| ↳ F6.08 &middot; Deploying to production | The deploy is build, migrate, boot: compile the release, run pending migrations with a release command, then start the supervision tree so the endpoint serves. Clustering connects nodes so the F6.07 PubSub and Presence span the whole cluster. | `/elixir/phoenix/deployment/deploy` | ● |
| **F6.09 · The live dashboard** | An interactive lab: real-time LiveView state over a socket, with multiple clients via PubSub. | `/elixir/phoenix/live-dashboard` | ● ⬡ hub ▣ lab |
| ↳ F6.09 &middot; Build the dashboard | The dashboard is a LiveView that holds a read model on its socket: metric counts seeded from the F6.04 contexts at mount, plus a capped stream for a live activity feed. render/1 draws metric cards and the feed; the socket holds derived state, never stored. | `/elixir/phoenix/live-dashboard/build` | ● |
| ↳ F6.09 &middot; Broadcast engine events | The domain emits events after a write (F6.07) and the dashboard subscribes to the topic on its connected mount. handle_info/2 folds each event into the read model — bumping a count and prepending to the feed stream — so the numbers and the feed stay live. | `/elixir/phoenix/live-dashboard/stream` | ● |
| ↳ F6.09 &middot; Many clients, live | One broadcast reaches every connected dashboard at once, so all viewers update together, and Presence reports a live count of who is watching. The dashboard only reads and folds — a projection over the event stream — and clustering spans nodes in production. | `/elixir/phoenix/live-dashboard/multi-client` | ● |
