...
- Page meta — `<title>`: `Branded ids — F4.07.3 · jonnify`. `<meta description>`: "A 64-bit Snowflake is encoded to eleven base62 characters over the alphabet 0-9A-Za-z (url-safe, no padding), left-padded to a fixed width, and prefixed with a three-letter namespace like PGE. Fixed width makes lexical order equal numeric order, so branding changes how an id looks but never how it sorts." (The hero kicker and lede carry the same facts; the meta description here is the page's `<meta name="description">` content — confirm against the head when rebuilding.)
