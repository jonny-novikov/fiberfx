...
> `echo/apps/codemojex/assets/` becomes its own npm **workspace** (the Phoenix client libs vendored
> dependency-free under `packages/*`), the edge image builds from a **self-contained `assets/` context** (no
> umbrella, no `deps/`), the publish script lives at `assets/bin/edge-deploy.sh`, and the image's awscli
> installer is fetched from a **pre-staged** `edge.codemoji.games/dist/awscli-<awsarch>.zip`. Each story is
> Connextra + Given/When/Then; each names the invariant(s) it exercises and the surface that closes it. A gate
> must **exercise** its outcome — a no-op must not satisfy a story's letter.
>
> **Forward-tense** for the unbuilt surface (the vendored `packages/*`, the self-contained `Dockerfile`, the
> relocated `bin/edge-deploy.sh`, the `dist/awscli-*` pre-stage). Framing: third person; no first-person-agent
> narration. As-built naming: `edge.codemoji.games` + the **game** bundle (`game-<hash>.js`) +
> `Codemojex.Edge.game_url/0` (not the narrative docs' stale `static.codemoji.games`/`board`).

## Roles

- **A front-end engineer** — builds and iterates the front end on a checkout. Wants
  `cd assets && npm install && npm run dev` to work with **no umbrella and no `mix deps.get`**.
...
- **The edge image** — the ephemeral one-shot Fly task (`codemojex-edge-deliver`) that builds the game bundle,
  uploads it immutable, flips the pointer, and exits. Builds from the `assets/` context alone.
...
**Exercises:** INV-STANDALONE-DEV (A3); the removal of the `file:../../../deps/*` coupling
(`assets/package.json:11-14`). **Surface:** the npm workspace (`assets/package.json` `"workspaces"`) +
`assets/packages/*`.

- **Given** a checkout where `echo/deps/` is absent (no `mix deps.get` has run)
- **When** `cd echo/apps/codemojex/assets && npm install && npm run build && npm run build:client`
...
- **And** the **negative check:** with the pre-rung `file:../../../deps/*` `package.json`, `npm install`
  against an absent `deps/` **fails** — proving the workspace is what removed the coupling.

## S2 — Dep-free `package.json`; workspace resolution to `packages/*`

*As the Operator, I want zero `file:../` links in the front-end manifest, so that the build context never
reaches outside `assets/`.*

**Exercises:** INV-DEP-FREE (A1); workspace resolution (A2). **Surface:** `assets/package.json`
(`"workspaces": ["packages/*"]`; `phoenix*` → `"*"`) + the regenerated `package-lock.json`.

- **Given** the converted workspace
- **When** `grep -nE 'file:\.\.' echo/apps/codemojex/assets/package.json` runs, and `npm ls phoenix
  phoenix_live_view phoenix_html` is inspected after `npm install`
- **Then** the grep returns **0 matches**, and the three libs resolve to **`packages/*`** (symlinked in
  `node_modules`), not to `deps/`.
...
lifecycle (`js/app.js:5-6,11-66`).

- **Given** the LiveView client built from the vendored packages (`npm run build:client`)
- **When** the app serves it and a player opens the lobby
- **Then** `import { Socket } from "phoenix"` and `import { LiveSocket } from "phoenix_live_view"` resolved to
  `packages/*`, the `LiveSocket` **connects** (`window.liveSocket` live), and entering a room **mounts the
  game island** via the `EdgeReact` hook (`this.pushEvent` / `this.handleEvent` work),
...
**Exercises:** INV-SELF-CONTAINED-CONTEXT (A5). **Surface:** the rewritten `assets/Dockerfile` (no `deps/`
COPYs; `COPY . .`; `ENTRYPOINT bin/edge-deploy.sh`) + `assets/fly.toml` (`dockerfile = "Dockerfile"`).
...
- **And** at runtime the entrypoint's `npm ci` resolves `phoenix*` from `packages/*` (no `deps/` present in
  the image).
...
**Exercises:** the relocation (A7) + the reliability gate (A11). **Surface:** `assets/bin/edge-deploy.sh`
(moved from `scripts/`; `cd "$(dirname "$0")/.."`).

- **Given** the relocated script
- **When** `bin/edge-deploy.sh --dry-run` runs (from the `assets/` tree, env sourced)
- **Then** it `cd`s to `assets/`, builds the game bundle, and prints the would-upload/would-flip plan
  **without** writing to the bucket; the old `apps/codemojex/scripts/edge-deploy.sh` no longer exists,
- **And** the script's build → upload-immutable → flip-pointer-last → verify contract (and `--rollback`) is
  byte-stable aside from the `cd` and the header doc-links.
...
- **And** `npm run build` still emits `game-<hash>.js` + a vite manifest.
