...
status: Built (ec.1–ec.6 complete; ec.6 docs-only — production cutover Operator-pending)
owner: Fireheadz
...
| **ec.5** | **polish the live site** — externalize the design-system CSS + the two interactive scripts to embedded, content-hash-served `web/static` (immutable cache; fonts stay on the Google Fonts CDN as published); per-page meta, Open Graph, `sitemap.xml`, `robots.txt`; redeploy (still complete + live) | ec.4 | **M** | **NORMAL** | Flat-L2 |
| **ec.6** | **the production cutover** (DOCS-ONLY) — reconciled spec + an Operator-facing cutover runbook (an **index-only** `/` flip, no-shadow) + a flyctl-pinned one-step rollback + a verified smoke battery; the **Operator** executes the flip | ec.1–ec.5 | **S** | **HIGH** (live-domain; Apollo) | Venus + Director + Apollo |

**Status.** `ec.1`–`ec.3` are **built** at `go/echo-courses/` — ec.1 the scaffold, ec.2 the templating engine + base layout (the embedded `html/template` `Renderer`, the verbatim chrome, the card/filter partials), and ec.3 the course catalog + content model (the file-backed loader over `content/<slug>.html`, the five courses seeded in published order, fail-fast; all acceptance criteria green). **ec.4 the live site** is built — the `/courses` index + the five course pages on their published paths + the track filter, catalog-driven and styled, with deploy-ready `Dockerfile`/`fly.toml`; verified via the local dev server (the Operator runs `fly deploy`). **ec.5 the polish** is built — the design-system CSS + the two interactive scripts externalized to embedded, content-hash-served `web/static/app.<hash>.{css,js}` (immutable cache, byte-equivalent to the former inline blocks), per-page SEO `title`/`description` + Open Graph + `canonical` (from env `CANONICAL_BASE`), and `GET /sitemap.xml` + `/robots.txt`; verified via the local dev server (the Operator runs `fly deploy` to redeploy). **ec.6 the production cutover** ships **DOCS-ONLY** (Apollo BUILD-GRADE) — the reconciled spec + an Operator-facing cutover runbook (`echo-courses.6.cutover-runbook.md`) + a verified smoke battery (`ec.6.smoke.sh`). The recon found `jonnify.fly.dev` is a large Operator-owned multi-section site (its server source out-of-VCS) serving the courses index at `/` (not `/courses`) and the five course paths as the deep-course roots — so the cutover is reconciled to an **index-only** (`/`) flip that leaves every deep-course root + other section byte-untouched, with a flyctl-pinned (`fly v0.4.6`) one-step rollback. The team ran no `fly` and touched no live app; the **production cutover is the Operator's to execute** (the runbook drives + verifies it). **The echo-courses build program (ec.1–ec.6) is complete; only the Operator's manual cutover remains.**
