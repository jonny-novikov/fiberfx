...
> to the Exchange Platform's instrument catalog; no single `content/…md.txt` author source. · Grounding:
> `EchoCache.Coherence` over `EchoCache.Table` (`echo/apps/echo_cache`).

A change must reach the cache. When an instrument is re-listed, drop the stale L1 row and resolve newer-wins, then
choose the lane each surface needs — the fast broadcast lane or the durable job lane. The cache from stage 1 keeps
serving an instrument's old copy after it changes; coherence closes that gap. The deeper functional-Elixir craft
behind the writer is [`/elixir`](/elixir).

## A message about a name

EchoCache's coherence message carries exactly two identities and nothing else: the cached name and the writer's
mint-time version — twenty-nine bytes, `id <> ":" <> version` (`EchoCache.Coherence.payload/2`). Everything else falls
...
Two lanes carry the same twenty-nine bytes, and the catalog's surfaces do not all need the same one. The choice is a
...
that surface. A tradeable instrument's halt state or a risk parameter needs the durable lane — a stale ceiling is
money, so the job lane earns its cost. A display-only descriptor tolerates one TTL of lag for a cheaper hop — the
...
## Coherence on EchoCache

Take one change: an instrument is re-listed, and the writer has placed the new value in L2 with a fresh mint-time
version. Its one obligation after the store write is one coherence call carrying that version: `Coherence.broadcast/4`
on the fast lane, or `Coherence.enqueue/5` on the durable lane. Every node's table applies the message newer-wins:
the broadcast lane lands on a RESP3 push subscription that drops the L1 row; the job lane lands on a consumer running
`Table.coherence_handler/1` that calls `apply_coherence/4`. The next read of that instrument re-fills from the
already-updated L2. The deeper functional-Elixir and OTP craft of the writer and the consumer is [`/elixir`](/elixir).

```elixir
# Coherence on a change — one call after the L2 write, carrying the write's version.
# The fast lane: a PUBLISH on ecc:{instruments}:coh, at-most-once.
{:ok, _receivers} = EchoCache.Coherence.broadcast(conn, "instruments", id, version)

# The durable lane: an enqueue on ecc.coh.instruments over EchoMQ, at-least-once.
{:ok, :enqueued} = EchoCache.Coherence.enqueue(conn, "instruments", group, id, version)
...
**The pattern → EchoCache.** Invalidate on a change: place the newer value, then carry the version to every node and
resolve newer-wins. On the Exchange Platform an instrument change runs `Coherence.broadcast/4` (fast) or
...
the one instrument every order touches can still send a herd at the loader on expiry. The next dive shows how the
...
the hit rate from `EchoCache.Table.stats/1`.
...
- [/echomq](/echomq) — the fair-lanes job queue the durable lane rides.
- [/bcs](/bcs/cache/coherence-by-mint-time) — the EchoCache coherence-by-mint-time manuscript chapter.
