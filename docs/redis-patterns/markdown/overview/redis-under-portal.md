# Valkey under the Exchange Platform

> Route: `/redis-patterns/overview/redis-under-portal` · Module R0.2 (hub) · Source: ORIENTATION — the placement
> module, no single pattern source (the connector and the protocol are taught by `/echomq`) · Grounding:
> `echo/apps/echo_wire` · `echo/apps/echo_mq` · `echo/apps/echo_cache` · `docs/echo/bcs/content/bcs3.1.md` ·
> `bcs4.md` · `docs/exchange` — every figure verbatim from a committed record. Reframed under
> [`specs/reframe-echomq/`](../../specs/reframe-echomq/reframe-echomq.md).

Before the patterns, the placement. The **Exchange Platform** reaches one boundary — the echo data layer — and
below it Valkey serves two roles: **EchoCache** — the near-cache, an L1 of ETS tables over the L2 Valkey — and the
**EchoMQ bus** — the owned-protocol bus at `echo/apps/echo_mq` — both over one Valkey, and no surface holds a raw
Valkey key. Three dives take the placement apart, in the arc *the seam → the roles → the tier*: the **facade seam**
...
The Exchange Platform reaches Valkey through one owned boundary — the echo data layer — and every surface, from the
...
- **EchoCache** — the near-cache in front of the read path. Its one-line case is the Part IV title made plain:
...
- **The cache machine is EchoCache.** It holds **derived** state — a copy of the truth, safe to drop. A dropped
...
Each dive takes one part of the placement, in the arc *the seam → the roles → the tier*. Read them in order —
...
- **R0.2.2 · The two roles** — EchoCache, the derived near-cache in front of the read path, and the EchoMQ bus,
...
- **R0.2.3 · The reserved tier** — the tier once held in reserve, now shipped: EchoMQ at `echo/apps/echo_mq`,
  the convergence target of the EchoMQ program, with the real `claim` script as the worked move.

**The placement → its BCS application.** Keep one store below one facade, in two clear roles; surfaces never
depend on the store, and the store can change without them. In the BCS build that is EchoCache and the EchoMQ
...
- [R0.2.1 · The facade seam](/redis-patterns/overview/redis-under-portal/the-facade-seam) — the master invariant.
- [R0.2.2 · The two roles](/redis-patterns/overview/redis-under-portal/two-roles) — the near-cache and the bus.
