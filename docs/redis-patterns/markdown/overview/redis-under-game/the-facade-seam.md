...
> Route: `/redis-patterns/overview/redis-under-game/the-facade-seam` · Module R0.2 · dive 1 · Source:
> ORIENTATION — the named-consumer module, no single pattern source · Grounding: the master invariant — every
...
lives below it, never on a surface's path. The play API, a consumer, and a background worker each call one thing —
the echo data layer — and read only a typed result from a closed set. Below the seam sits the one owned Valkey
client, EchoWire; EchoStore and the EchoMQ bus ride it, both over Valkey. The connector behind the boundary is
taught by the [`/echomq`](/echomq) course; this dive states the rule and shows it holding.

## §1 · One boundary, one return set

The codemojex reaches Valkey through one boundary, the echo data layer. The master invariant is a rule
...
A condition *below* the boundary — an EchoStore L1 hit, a cold read that runs the loader once, a dropped socket, a
full in-flight queue — never escapes as a raw store condition. The data layer folds each one into the closed
return set, and the surface reads the typed result. The committed EchoStore walk from `bcs4.1` shows the read path
...
Take one call: the play API admits a guess. The Telegram webhook hands each decoded update to
`Codemojex.EchoBot.ingest/1`, which mints a branded `CMD` and enqueues the command; the play surface itself reads
a typed result from a closed set, and `Codemojex.Guesses.submit/3` validates the guess and answers from one —
never a raw exception:

```elixir
# A surface (the play API) calls ONLY the consumer's seam and matches ONLY the closed result set.
case Codemojex.Guesses.submit(round, player, emojis) do
  {:ok, :enqueued} -> await_score()
  # one of a closed set: :no_round | :closed | :expired | :bad_guess | :insufficient
  {:error, reason} -> reject(reason)
end
```

Nowhere in the play API is there a socket, a key, or a parse branch on a raw field. And the discipline reaches
...
`GET {emq}:version answers echomq:3.0.0 through the fenced connector itself`. Typed outcomes at the edge, typed
refusals at the wire: the same shape, twice.

> Notes on Valkey · The EchoStore L2 row is written with `SET … PX`, so the second layer expires itself on the
...
- [R0.2 · Valkey under codemojex](/redis-patterns/overview/redis-under-game) — the module hub.
- [R0.2.2 · The two roles](/redis-patterns/overview/redis-under-game/two-roles) — the next dive.
