...
> Route: `/redis-patterns/overview/redis-under-game/reserved-tier` · Module R0.2 · dive 3 · Source:
...
code**: `echo/apps/echo_mq`, the Valkey-native bus — born braced (`emq:{q}:`), born branded (`JOB` ids
gated at the key builder), born declared (every Lua key declared or grammar-derived). This dive retells the tier as it now stands, works the bus's defining move — the real `claim` script — and
...
into. The program has since filled the seat with code. `echo/apps/echo_mq` is the bus — `EchoMQ.*`, lib-only — re-derived from first principles by the Branded Component System and shipped as measured,
rung-gated code.

Two disciplines on that wire are birth conditions, not retrofits: operand keys are never built from an `ARGV`
prefix inside a script body, and the keyspace has a total parse — every key declared or grammar-derived. A key a
script touches but never declares is invisible to the gate, so the bus refuses it at the key builder, before
anything reaches Valkey.

The tier also has a named tenant now. **codemojex** (`echo/apps/codemojex/`) is the program's named consumer:
command dispatch (`Codemojex.CommandWorker`), scoring (`Codemojex.ScoreWorker`), round settlement
(`Codemojex.Settle`), and notifications (`Codemojex.NotificationWorker`) as `EchoMQ.Jobs` work drained by
`EchoMQ.Consumer` and shaped by `EchoMQ.Lanes`.
...
at `echo/apps/echo_mq` over Valkey — the bus the program extends rung by rung, with codemojex
draining it.
...
- [R0.2 · Valkey under codemojex](/redis-patterns/overview/redis-under-game) — the module hub; the pager loops back here.
- [R0.2.2 · The two roles](/redis-patterns/overview/redis-under-game/two-roles) — the previous dive; the bus role.
