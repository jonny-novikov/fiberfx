...
5. VALIDATE (dependency-light pre-flight; the course has no per-page CLI gate, though a Playwright DOM toolkit exists at docs/validator/ for deeper manual checks). Confirm: exactly one references block on the page; the dollar-sign delimiters across the page are balanced and no « » № … characters appear inside any math span (KaTeX strict); and every /health/... link added or unlocked resolves to a file that exists on disk. Fix until all hold.
