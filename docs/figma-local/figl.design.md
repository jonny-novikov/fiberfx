...
| `bridge-server.js` | Windows (`192.168.1.120`) | HTTP `:3001` (`/health`, `/document`, `POST /request`) + WebSocket `:3000`; pure relay request→plugin→response; ~30s timeout (`bridge-server.js:148`); single connection; **no auth**, CORS `*` (`:63`). | `/request`, `/health` |
