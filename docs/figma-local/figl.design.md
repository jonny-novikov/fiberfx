...
|---|---|---|---|
| `mcp.js` | Mac (this repo) | stdio MCP server; each plugin-backed tool forwards as HTTP `POST /request {action,params}` to `BRIDGE_URL` (`mcp.js:5`). Post-`figl.1` … `figl.3`, registers **10** tools: the **7** plugin-backed (`get-current-page` · `get-selection` · `get-all-pages` · `find-nodes` · `get-node-properties` · `export-node` · `get-batch-nodes`) plus the **3** Mac-side (`get-figma-document` over the cached `/document` · `check-bridge-status` over `/health` with the advertised-vs-backed handshake · `cleanup-renders` over the bounded `RENDER_ROOT`). `version: "2.0.0"` is a bare label. | the 10 tool ids |
...
| `figma-plugin/` | inside Figma (Windows) | `code.ts` action switch backs **7** actions (`get-current-page`, `get-selection`, `get-all-pages`, `find-nodes`, `get-node-properties`, `export-node`, `get-batch-nodes`); all else falls to `default → "Unknown action"`. `BACKED_ACTIONS` is reported on `ws-connected` (ADR-5). `serializeNodeDetailed` carries cornerRadius (+ per-corner, `figma.mixed`-guarded), auto-layout (only when `layoutMode !== 'NONE'`), and `absoluteBoundingBox` (ADR-3). `ui.html` holds the WebSocket (the plugin can only dial *out*). | the 7 actions |
