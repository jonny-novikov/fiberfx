...
|---|---|---|---|
| `mcp.js` | Mac (this repo) | stdio MCP server; each plugin-backed tool forwards as HTTP `POST /request {action,params}` to `BRIDGE_URL` (`mcp.js:5`). Post-`figl.1` … `figl.4`, registers **10** tools: the **7** plugin-backed (`get-current-page` · `get-selection` · `get-all-pages` · `find-nodes` · `get-node-properties(nodeId?, depth?, maxNodes?)` · `export-node` · `get-batch-nodes`) plus the **3** Mac-side (`get-figma-document` over the cached `/document` · `check-bridge-status` over `/health` with the advertised-vs-backed handshake · `cleanup-renders` over the bounded `RENDER_ROOT`). `version: "2.0.0"` is a bare label. | the 10 tool ids |
