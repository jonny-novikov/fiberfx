...
Trimming is safe only relative to what has been preserved elsewhere. The next rung (emq3.5) archives the trimmed tail into durable segments via the native `EchoStore.Graft` engine — the same engine Chapter III built — and the rule there is **fold-before-trim**: a dedicated fold consumer commits a slice to the archive, and only then may the stream cut up to the folded frontier. Couple the two and a trim can never remove an un-archived entry, because the trim watermark is bounded by the fold watermark by construction; that is the safe path the figure shows. The genuinely open question — recorded as fork **F3.4-A** — is the cadence of the general-purpose trim that is not the archive fold. One option drives trimming on the consumer beat and ties the trim watermark to the fold watermark, so safety is automatic but liveness and safety are now one knob. The other gives trimming a separate, named cadence and keeps safety as an explicit precondition, on the principle that you should not couple a liveness concern (how often we trim) to a safety concern (what we are allowed to trim). The Director's reconciliation lands between them: ship the `trim` verb with its policy converged, make a general cadence opt-in and named rather than implicit, and let the emq3.5 fold consumer be the one place that does fold-before-trim. The naive failure the figure dramatizes — a timer that advances the cut past the fold — is precisely what naming the cadence is meant to prevent.
