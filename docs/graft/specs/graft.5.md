...
size: L
risk: HIGH
status: SHIPPED — working tree, uncommitted (Director Stage-6 fold 2026-06-22 · Apollo BUILD-GRADE 2026-06-22 · D-3 raw-RESP3 socket · D-4 Fjall buffer · D-5 drop-v1/COMMIT-as-v2/PROTO=2 · the live Rust↔Valkey binding now real, crit-7 3/0) — synced to as-built
stands-on: "eg.4"
---
...
- `Backpressure::admit(vol)` (`backpressure.rs:66`) is consulted in the live request path (`live.rs:192`, `LiveBackend::handle_request_frame`) before dispatching a `{vol}`-bearing command; over the per-Volume cap the request is refused with `Msg::Err{kind: Unavailable}` (`dispatch.rs:279` taxonomy) — never unbounded buffering, never a blocked dispatch thread. The control lane stays exempt by construction (`backpressure.rs:12-23`).
...
- **The abstract transport (swap `InMemorySink` for a live one)** — `crates/echo_graft_backend/src/transport.rs`: the `FeedSink` trait (`:24`, publish-only), `InMemorySink` (`:43`). The live transport is the as-built `crates/echo_graft_backend/src/live.rs` (`LiveBackend::handle_request_frame` `:192` — the `admit` consult; `LiveBackend::serve` the event loop).
...
- **The 1:1 dispatch + error taxonomy** — `crates/echo_graft_backend/src/dispatch.rs`: `dispatch` (`:32`), `err_kind_of` (`:279`, the closed `{conflict, not_found, version_mismatch, unavailable}`; the `VolumeNotFound → not_found` arm `:282` — UF-2), `to_page` (`:253`, the page-size realization).
- **The byte-frozen wire codec (the ruled D-A2 raw socket reuses this)** — `crates/echo_graft_proto/src/lib.rs`: `Msg::{encode,decode}` (`:586,591`), `encode_parts`/`decode_parts` (`:77,95`, byte-identical to `EchoMQ.RESP.encode/1`), `PROTO_MIN`/`PROTO_MAX` (`:34,38`, both `2` under D-A1 — v1 dropped), the `Mode` enum + tokens (`:192-219`), the `Commit` variant carrying `mode: Mode` (`:315-326`), its encode arm (`:427-443` `to_parts`) + decode arm (`:506-525` `from_parts` — the v2 shape; the v1 generation is dropped).
...
| `egraft:cmd:_control` | the backend now **SUBSCRIBE**s the handshake/open/resolve lane | `graft_backend.ex:395` (`control_lane`), `live.rs:54` (`CONTROL_LANE`), `backpressure.rs:18` |
...
| `egraft:feed:{vol}` | the backend's live `FeedSink` now **PUBLISH**es here (was: in-memory sink) | `echo_graft/src/feed.rs:42` (`lane_for`, the engine crate), `transport.rs:24` (`FeedSink` trait), `feed_sink.rs:34-40` (`BusFeed::frame` → `Msg::Feed`) |
| `corr: u64` | unchanged — the request↔response pairing inside a reply lane | `echo_graft_proto/src/lib.rs` (every request/response) |
| **`COMMIT.mode` (ruled D-A1)** | a fixed-position `mode` field (`async`\|`sync`) added to `COMMIT` in place; `PROTO_MIN = PROTO_MAX = 2` (v1 dropped); the client always encodes the mode (defaults to `sync`) | `echo_graft_proto/src/lib.rs:315-326` (the `Commit` struct carries `mode: Mode`, between `base` and `pages`) + the `Mode` enum + tokens `:192-219` + the encode arm `:427-443` (v2 shape `[COMMIT, corr, vid, base, mode, npages, (idx,page)*]`) + the decode arm `:506-525` (`Mode::from_token(&rest[3])`, pages tail from index 5) + the fixture `:662-663` (both modes round-trip) |
...
> **The error enum stays closed:** `{conflict, not_found, version_mismatch, unavailable}` (`echo_graft_proto/src/lib.rs:146-154`, `ErrKind`). A cap rejection reuses `unavailable`; no new kind.
...
- **Every NON-v2-touched fixture stays byte-identical to HEAD** — `FeedEvent` (the 51-byte opaque blob, `echo_graft/src/feed.rs:166` `feed_event_encoding_is_byte_frozen`), `PUSH`, `PULL`, `READ`, `SNAP`, `ACK`, `PAGES`, `ERR`, etc. A silent re-encode of any of these is a LOUD failure (gate G3, `graft.roadmap.md:92`). The live `FeedSink` publishes the SAME `Msg::Feed{blob}` framing the eg.4 `BusFeed` produces (`feed_sink.rs:34-40`) — no re-encode.
