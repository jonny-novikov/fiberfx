...
| **eg.6** | **DEFERRED** — ship **AND run** fully integrated EchoMQ + Graft (BEAM↔Rust): the whole stack live on a real deploy · the **fly.io EchoMQ deploy floor** (Dockerfile + `fly.toml` — the deferral root, not yet ready) · cross-compile (Mac + Windows) + CI ship-hardening · the eg.5-carried stdin-EOF watchdog · the **per-workload** durability shootout (`echo_graft` beside the native Elixir `EchoStore.Graft.*` engine, Champ, Oban) | eg.1–eg.5 · **the fly.io EchoMQ deploy floor** | **L** | **HIGH** (live integrated run + first deploy) | Flat-L2 + Apollo (when taken up) |

> {style="warning"}
> **eg.6 is DEFERRED (2026-06-22).** Reframed from "cross-compile + CI + shootout" to "**ship AND run fully integrated EchoMQ + Graft**" — the whole BEAM↔Rust stack running live and integrated, then measured per-workload. The new load-bearing leg (the live integrated run) stands on a **fly.io EchoMQ deployment that is not yet ready** (no Dockerfile / `fly.toml` for EchoMQ; eg.5 proved the binding against a local Valkey `:6390`, not a deployed bus), so the integration acceptance cannot be discharged. The spec is **drafted** (`specs/graft.6.md`, `status: Draft`) to capture the reframed scope; the rung is built once the deploy floor exists. **eg.1–eg.5 are shipped** — the durability engine is complete and proven in isolation; only the integrated-deploy-and-measure rung is deferred. This rung does **not** scaffold the fly.io artifacts (they are the deferred deliverable).
