# Portal · Telegram bot stack — sources, decision matrix, optimal choice

The Telegram Bot API is at **9.5** (released March 1, 2026), with later-2026 additions including guest mode and opt-in bot-to-bot communication, so "current API support" below means tracking the 9.x line. Telegram Bot API 9.5 is fully supported — member tags, date_time entities, and can_manage_tags admin rights, and on May 7, 2026, Telegram… enable[d] native, direct communication between autonomous AI bots… expand[ing] the Telegram Bot API to allow what the company calls "bot-to-bot communication mode."
...
5. **Gary Bernhardt — "Boundaries" / Functional Core, Imperative Shell** · https://www.destroyallsoftware.com/talks/boundaries — Pure decide/evolve core (many fast tests, no doubles) wrapped by a thin effectful shell; the architecture spine for Portal's contexts and the bot adapter.
...
### 1.6 Node interop (EchoMQ / BullMQ)
30. **BullMQ** · https://bullmq.io/ — Redis-backed Node queue (delayed/repeatable jobs, rate limiting); the substrate under your EchoMQ and the clean bridge to Node workers. (Vendor benchmark claims higher raw throughput than Oban but trades Postgres transactional guarantees for Redis — read with that bias.)
...
## 2. Top 5 bot libraries for Portal (sorted by stars)
...
| 4 | **ex_gram** · Elixir/BEAM | 223 | **current (auto-gen)** | **Mar 2026 (0.64)** | **native** | Beer-Ware | **Best in-BEAM choice**: OTP-supervised, calls the `Portal` facade in-process. |
...
Both keep the master invariant ("the bot calls only the `Portal` facade"); the difference is **whether that facade call is an in-process function or a network/EchoMQ hop**.

| Dimension (weight) | **Fastify worker** (grammY + EchoMQ/BullMQ) | **Elixir-native** (ex_gram in the BEAM) |
|---|---|---|
| Time-to-first-ship (MVP) ★★★ | △ must build the EchoMQ request/reply contract, idempotency, timeouts before story 1 ships | ✓ bot is an adapter over `Portal`; first quiz ships same day |
...
| Code/type sharing with domain ★ | ✗ models + validation duplicated across runtimes | ✓ shares the Portal domain and tests |
...
| Team-skills fit ★★ | ✓ your stated Fastify/Node/EchoMQ expertise | △ Elixir, but you already run the BEAM for Portal |
...
## 4. The optimal choice for Portal

**Ship the Telegram bot Elixir-native with `ex_gram`, in the BEAM, as an adapter over the `Portal` facade — and keep grammY + Fastify + EchoMQ as a documented, pre-designed scale-out seam.**
...
- **It collapses the topology.** The bot becomes "just another caller of `Portal`," exactly like a controller or LiveView. No second runtime, no Redis hop on the critical path, no cross-runtime consistency problem to solve before your first quiz ships. That is the shortest tracer bullet to a live feature [12, 17].
...
**The seam that protects your Node/EchoMQ vision.** Because the bot only ever calls `Portal`, moving it to a Fastify/grammY worker later is a **boundary swap, not a rewrite**: the Node bot becomes a *remote* adapter that reaches `Portal` over EchoMQ instead of in-process. Adopt that exactly when a real trigger appears — a grammY-only plugin you need, a Telegram **Mini App** (TS) backend that grows large enough to own the bot, or a need to scale the bot tier independently of the BEAM. If/when you do, use **grammY** (current, TS-first, plugins, runner) [36] — and consider **GramIO** for its CLAUDE.md generation [40] — never telegraf or node-telegram-bot-api for new work.

**One-line rule:** *the `Portal` facade is the contract; where the bot process runs is an implementation detail you can change later for free.*
...
- **What** — A `Portal.Telegram` bot built with `ex_gram` as a supervised adapter that calls **only** the `Portal` facade; streaks/quiz state flow through the existing domain + PubSub; notifications run on Oban; Telegram Login/`initData` is HMAC-validated server-side into a Portal session. grammY + Fastify + EchoMQ is documented as the scale-out replacement behind the same facade.
...
- **Where** — Inside the Portal release's OTP supervision tree as a bot adapter directly above the `Portal` facade (no network boundary); EchoMQ/BullMQ and Fastify workers stay reserved for genuinely Node-side workloads until that trigger fires, at which point the bot moves out behind the same facade contract.
