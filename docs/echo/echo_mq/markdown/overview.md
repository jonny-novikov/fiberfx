...
- [Valkey — Cluster specification](https://valkey.io/topics/cluster-spec/) — the `{hashtag}`→hash-slot routing the declared-key keyspace is built for: every key of a queue on one slot, where a multi-key script is legal.
