...
### T-23 — Apollo high-risk evaluation entry: scope, independent-verify posture, the §11.2 plan

Continuing scope codemojex-game-rename (registry ccl-5; the L2 Squad's mandatory evaluator). Read the FULL ledger (T-1..T-22 · D-1..D-19 · V-1..V-16 · L-1..L-8 · Y-1..Y-8 · P-1/P-2) + the migration on disk + the apollo charter + the codemojex-ship facts. The build claims BUILD-GRADE (Mars-2 Y-8; Director-verify Y-7). The evaluator does NOT trust those — re-run the gate INDEPENDENTLY, adversarially verify, render BUILD-GRADE/BLOCKED.

Environment re-probed THIS turn (not trusted from Mars): elixir 1.18.4 / erlang 28.5.0.1 (echo/.tool-versions); valkey-cli -p 6390 → PONG; pg_isready → /tmp:5432 accepting; branch echo_mq.

FIRST INDEPENDENT FINDINGS (re-run, not Mars's claim):
- G1 residual brand grep: /usr/bin/grep -rnoE '\bRND\b' lib test = 0; '\bRMM\b' = 0; '\bUSR\b' = 0. The three re-bases are clean.
- GAM/ROM/PLR mint sites present: rooms.ex:24 generate!("ROM"), rooms.ex:71 generate!("GAM"), wallet.ex:21 generate!("PLR"), tables.ex:59 kind:"GAM".
- ONE collapsed migration on disk: 20260618000000_create_codemojex.exs (Mars renamed _create_codemoji → _create_codemojex). 6 tables (players/transactions/emoji_sets/rooms/games/guesses); NO notifications table. games_type CHECK IN ('classic','golden') + games_status CHECK over the 7 words + players_non_negative CHECK all present. New columns present: games.top_k NOT NULL DEFAULT 5, games.payout_split DEFAULT [40,25,15,12,8], games.cell_codes, games.commitment, games.nonce, games.revealed_ms; rooms.payout_split + rooms.cell_count. games.secret is NOT NULL no default (classic draws a secret too — to verify).
- 6 schema files (game/guess/player/room/transaction/emoji_set); round.ex deleted, game.ex present.

THE §11.2 PLAN (charter A-D): A=the prompted-checks table (the 3 re-bases · the 6-table + ONE migration + CHECKs · INV-9 privacy · V-14 commit-reveal · V-15 sealed top-K + dust · V-16a reduced cell snapshot + EMS seed · tier removal + state machine), each file:line PASS/FAIL; B=≥1 un-prompted finding; C=≥1 attack-that-held; D=a mutation kill-rate (N money/privacy mutations, each reverted NET-ZERO). INDEPENDENT gate re-run: compile --warnings-as-errors + mix test --include valkey (expect 41/0) + residual greps + migration up/down + a multi-seed determinism sample (verify the golden timer-race flake Mars fixed in L-8/Y-8 HOLDS across seeds). SPEC-SYNC: fold the dust-drain (add_dust/2, whole-pool) + the golden timer-race test hardening + the economy="winner_take_all"-descriptor clarity note into the triad + game-model.design.md; design.md verify-only; the bcs.2/bcs.todo reconcile is OUT of scope. No production code except a reverted-net-zero mutation; no git.

### T-24 — Apollo verdict derivation: independent gate GREEN + 6/6 mutation kill + spec-sync DONE → BUILD-GRADE

INDEPENDENT GATE (re-run from echo/apps/codemojex, NOT trusting Mars/Director):
- compile --warnings-as-errors → exit 0 (only the pre-existing sibling echo_data/champ_view.ex Graft warnings, untouched — NOT codemojex).
- mix test --include valkey → 41 tests, 0 failures.
- Residual greps: \b(RND|RMM|USR)\b in lib+test = 0; round→game entity/api/wire + bonus (round_view/cm_rounds/"round:"//rounds/:no_round/Schemas.Round/tierfirst/claim_tier/guesses.tier) = 0; firsts = 0; :(secret|nonce) in view.ex + lib/codemojex_web = 0; Kernel.round BIF survives scoring.ex:52 + economy.ex:22/94.
- GAM/ROM/PLR mint sites: rooms.ex:24 ROM, rooms.ex:71 GAM, wallet.ex:21 PLR, tables.ex:59 kind GAM.
- Migration up/down/re-up: independently GREEN — migrate up (6 tables + indexes + 3 CHECKs), rollback --all (clean to zero), re-migrate (clean). The ONE collapsed 20260618000000_create_codemojex.exs.
- Determinism: golden_blind ISOLATION across 10 seeds (1,2,3,5,8,13,42,99,777,2026) all 4/0 — the L-8 timer-race fix HOLDS (seed 1, the original failing seed, green). Scoped 30× full valkey loop = 30/30 green (the build ran 150×+150×; a third 150× is waste per the charter).

MUTATION KILL-RATE (6 distinct PRODUCTION-code mutations, each Edit-in→catch→revert→shasum byte-exact; 4 files all match pristine post-battery): M1 secret-leak→KILLED(2 tests); M2/M2b/M2c exactly-once→defense-in-depth (single-guard survive, both-guards KILLED, see L-9); M3 commit-hash-corrupt→KILLED(golden_blind S3 binding); M4 dust-skip→KILLED(2 economy tests); M5 suppression-defeat→KILLED(golden_blind S2 refute_received); M6 score-gate-defeat→KILLED(2 golden_blind asserts). 6 introduced, 6 killed. No survivor leaves a money/privacy hole (the M2/M2b survivors are the OTHER guard backstopping — robustness, not a gap; L-9).

UN-PROMPTED (L-10): the privacy line (INV-9) is a VIEW-LAYER invariant — Board.record/3 writes the golden score to Valkey in-flight + the secret sits on the Postgres games row from open; what never crosses the WIRE is gated by View.revealed?/1 + the game_view base map. Correct design (a sealed contest needs its board to settle); the grep that proves it is :(secret|nonce) in view+web (=0) PLUS the M1/M6 view mutations. Also swept: wallet.ex (every money path FOR-UPDATE-locked + non-negative-checked + ledgered, the credit/5 negative guard is typed not catch-all); the ONLY catch-all in lib is telegram.ex:54 (external HTTP-parse boundary, typed to :bad_response — not a money/state mapper); games.secret NOT NULL satisfied for classic too (rooms.ex:81 unconditional draw); economy="winner_take_all" on golden is a stored descriptor (do_close dispatches on settlement, not economy — the Director's Y-7 INFO note, confirmed).

SPEC-SYNC (docs only; production code shasums unchanged): game-model.design.md §3.8.2 (top_k_split/2→/3 arity + the add_dust/2 whole-pool drain) + the economy-column descriptor note (line 370); cm.3.md §G3 (top_k_split/3 + the dust drain, pinned by "drains the whole pool"); cm.3.stories.md Liveness (the as-built timer-race test discipline: duration_ms 600_000 + explicit close_now, the L-8 fix). design.md verify-only (0 legacy brands + 0 entity-round prose, already forward-reconciled, committed d508d419). bcs.2/bcs.todo OUT of scope (separate /bcs task).

VERDICT: BUILD-GRADE. Gate independently green; privacy + commit-reveal fairness + sealed top-K + dust-drain + exactly-once all verified by read AND exercised by test AND pinned by a 6/6 mutation kill. No BLOCKING delta. No git; no production code (the 6 mutations reverted net-zero).
...
L-Mars-2 — The blue phase surfaced a latent flake the green gate hid. golden_blind_story_test.exs used duration_ms: 50 (a "short timer so the sealed close runs within the test") then submitted a guess + Process.sleep(60) + close_now. The Mars-1 full-suite run (40/0) AND the 150× loop both PASSED — because the 8 files run together under one seed and the timing landed safely every time. The flake only fired when running the ONE file in ISOLATION under an unlucky seed (seed 1): the "exactly-once" scenario's submit returned {:error, :expired} instead of {:ok,_} because the 50ms timer had already passed before the submit ran. This is the "an async/timing test races" class (mars.md): a same-millisecond/timer race invisible until the unlucky ordering. ROOT CAUSE: coupling the close to TIMER EXPIRY when the test ALSO needs the game OPEN to submit. FIX (strictly more robust, net-zero on what it proves): duration_ms: 600_000 (game stays open through every submit — no :expired race) + trigger the sealed close EXPLICITLY via close_now/1 (it dispatches on settlement=="sealed" regardless of ends_ms) + drop the Process.sleep(60) timer-waits. The 50ms timer was a red herring — close_now was always doing the close, not the timer; the timer only created the race. Verified across 10 seeds (1,2,3,5,8,13,21,42,100,999) all 4/0, seed-1 (the failure) now green. LESSON: a green full-suite + a green ≥100 loop do NOT prove a per-file isolation run is flake-free — a single-file unlucky-seed run is a distinct hazard surface. The mutation probe (asked for an unrelated reason) is what put a fresh seed on this file and surfaced it — adversarial probing finds latent defects the happy-path gate cannot. Also: the boot-the-real-release check (mars.md) earned its keep — the suite runs server:false and never proves the dev node boots+binds+seeds+serves an HTTP round-trip; the live-boot smoke proved GET /api/games/:id serves the renamed entity with no secret on the wire, which no suite assertion covers.

### L-9 — The exactly-once is DEFENSE-IN-DEPTH (status guard + SET NX); a single-guard mutation survives because the other backstops — the attack-that-held

The mutation battery probed close_game/1's exactly-once three ways (each reverted net-zero):
- M2 (SET-NX failure path → do_close): SURVIVED green. The status `%{status: :settled} -> {:ok, :already_closed}` guard (rooms.ex:175) short-circuits the SECOND close BEFORE the SET NX is reached, so the broken NX path is never exercised on re-close.
- M2b (status guard `:settled` → `:__never__`, SET NX intact): SURVIVED green. With the status guard defeated, the second close reaches the SET NX; the lock (`cm:<game>:closed` already set by the first close) returns not-OK → `{:ok, :already_closed}`. The SET NX IS a real backstop.
- M2c (BOTH guards defeated): KILLED — settlement + golden_blind both fail `assert second == {:ok, :already_closed}` (the re-pay lands). Proves the coverage has teeth (not vacuous).

CONCLUSION: exactly-once is doubly-guarded — the status `:settled` check (the fast path for a sequential re-close) AND the Valkey SET NX lock (the load-bearing guard for the genuine CONCURRENT race: a perfect-crack close and a timer close arriving before either writes `:settled`). Each independently refuses a re-pay. This is the ATTACK THAT HELD for the §11.2 charter: a second close cannot re-pay because two independent guards each refuse it. NOTE: the suite does NOT exercise the SET NX under genuine concurrency (the two closes are sequential), so the status guard masks the NX in the happy path — but the NX's standalone efficacy is proven by M2b, and the design's defense-in-depth means a single-point failure does not leak money. A future hardening could add a concurrent-close test that races two close_game/1 calls in parallel tasks to exercise the NX directly (the status guard would not have written `:settled` yet) — non-blocking; the invariant holds.

How to apply (mentoring, for the mars charter — the BDD blue-phase): when an invariant is enforced by defense-in-depth (two independent guards), a single-guard mutation that survives is NOT a coverage gap — it is the other guard backstopping. The test that proves teeth is the BOTH-guards-defeated mutation (M2c). State the layering explicitly so the survivor is read as robustness, not a hole. Links: [[L-7]] (Mars's L-7(6) state-terminal `:closed`→`:settled`), [[T-23]] (the evaluation entry).

### L-10 — Mutation kill-rate 6/6 on the money/privacy/fairness surface; the privacy line is a VIEW-LAYER invariant (scores DO sit in Valkey in-flight), and that is correct

THE BATTERY (6 distinct PRODUCTION-code mutations, each Edit-in → suite-catches → revert → shasum byte-exact; net-zero verified: view.ex/rooms.ex/economy.ex/game.ex all match pristine sha post-battery):
- M1 leak `:secret` in view.ex game_view base map → KILLED by 2 tests (privacy_story "game view never carries the secret" + golden_blind S1, both `refute Map.has_key?(view, :secret)`).
- M2/M2b/M2c exactly-once (see [[L-9]]): single-guard SURVIVE (defense-in-depth), both-guards KILLED.
- M3 corrupt the commit hash (seal over reversed secret, recompute canonical) → KILLED by golden_blind S3 (`assert Rooms.commit(secret,nonce)==commitment` fails). V-14 binding genuinely verified — NOTE the test checks SELF-CONSISTENCY (seal vs recompute via the same `commit/2`), so a mutation to `commit/2` itself would be self-consistent and survive; the real catch is a seal/recompute DIVERGENCE, which M3 is.
- M4 skip the dust-drain (`add_dust` → no-op) → KILLED by 2 economy tests ("drains the whole pool" 995≠999 + "fewer players" 615≠616). V-15 money-conservation pinned.
- M5 defeat the golden feedback suppression (publish for feedback in [score,none]) → KILLED by golden_blind S2 (`refute_received {:scored,_}`). B-1 pinned.
- M6 break the privacy score-gate (`revealed?/1` → always true) → KILLED by 2 golden_blind asserts (totals.best leaked + history points leaked). The score-withholding half of INV-9 pinned.

KILL-RATE: 6 distinct invariant mutations introduced, 6 killed (the 3 exactly-once SUB-mutations resolve to: the invariant holds via defense-in-depth, the BOTH-defeated case killed). No survivor leaves a money/privacy hole.

UN-PROMPTED FINDING (the privacy line's actual shape): INV-9 is enforced at the VIEW LAYER, not at storage. `Board.record/3` (game.ex:122) writes the golden score to the Valkey board ZSET UNCONDITIONALLY in-flight (the sealed settlement needs it at close); the secret + nonce sit in Postgres on the `games` row from open. So scores + the secret DO exist server-side during a blind game — what never crosses the WIRE is gated by `View.revealed?/1` (totals/points/leaderboard withheld) + the `game_view` base map (never selects :secret/:nonce). This is the CORRECT design (a sealed contest needs its board to settle), and the wire is the boundary that matters — but it means the privacy guarantee is a VIEW-layer property, exercised by M1/M6 at the view, NOT a storage-absence property. A reviewer who greps only for `:secret` in Postgres writes would mis-locate the invariant. The grep that proves it is `:(secret|nonce)` in view.ex + lib/codemojex_web (= 0, confirmed) PLUS the M1/M6 view-layer mutations.

How to apply: for a money/privacy rung, the kill-rate is the load-bearing evidence — a green suite is necessary not sufficient; the mutation battery converts "the test passed" into "the test would FAIL if the invariant broke." Cite the per-mutation catcher (test name + assert line). Links: [[L-8]] (Mars's own privacy mutation probe — M1 is its independent re-run), [[L-9]].
...
Director Stage-3 verify — INDEPENDENT gate GREEN; BUILD-GRADE; 2 minor nits for Mars-2/Apollo

INDEPENDENT re-run (not Mars's claim), from echo/apps/codemojex on Valkey 6390 + Postgres:
- compile --warnings-as-errors: CLEAN (only sibling echo_data/champ_view.ex Graft warnings — untouched).
- Residual: \b(RND|RMM|USR)\b → 0; round→game/bonus tokens (round_view/cm_rounds/no_round/Schemas.Round/tierfirst/claim_tier/"round:"//rounds) → 0; Kernel.round BIF in scoring.ex survives; brands mint GAM (rooms.ex:71) / ROM (rooms.ex:24) / PLR (wallet.ex:21); design.md mirror → 0 legacy brands.
- Privacy grep: 0 :secret/:nonce in view.ex + the web layer.
- Migration: up → rollback --all → re-migrate CLEAN — the ONE collapsed 20260618000000_create_codemojex.exs, 6 tables (players/transactions/emoji_sets/rooms/games/guesses) + players_non_negative + games_type + games_status CHECKs.
- mix test --include valkey: 40 tests, 0 failures (SQL log confirms the games row carrying type/feedback/top_k=5/payout_split=[40,25,15,12,8]/commitment/nonce/cell_codes + GAM/ROM/PLR/EMS ids).

CODE READ (privacy/fairness/settlement):
- view.ex withholds the secret; revealed?/1 (feedback!="none" OR revealed_ms set) gates totals + points + leaderboard — blind pre-reveal exposes only state+timer+commitment.
- rooms.ex close_sealed: Board.top = Cmd.zrevrange (DESCENDING) → top_k_split gets a correctly-ranked board; exactly-once via SET NX on cm:<game>:closed (a perfect-crack/timer race → one payer); commit/2 = SHA-256(code₀‖…‖code₅‖nonce, 0x1E sep) lowercase hex (V-14); snapshot_cells = take_random(codes, min(n,len)) (V-16a); the privacy gate (revealed_ms) opens BEFORE the fat :revealed broadcast.
- golden_blind_story_test EXERCISES: suppression (refute_received :scored, history withholds points, leaderboard []), reveal (assert_receive :revealed + Rooms.commit recomputes the SHA-256 == commitment), exactly-once (2nd close → {:ok,:already_closed}, balance unchanged).

VERDICT: BUILD-GRADE. Gate independently green; privacy + commit-reveal fairness + exactly-once verified by read AND exercised by test.

2 MINOR nits (neither a correctness bug):
1. [LOW — Mars-2] top_k_split integer-division DUST: div(pool*w,sum) truncates → up to K-1 diamonds unpaid; economy.ex doc claims 'the whole pool is distributed.' Matches the existing winner_take_all (div(pool,n)) convention; money conserved (dust stays in pool). FIX: soften the doc claim OR assign the remainder to rank-1.
2. [INFO — Apollo spec-sync] golden economy='winner_take_all' is a DESCRIPTOR (do_close dispatches on settlement='sealed'→top_k_split, NOT economy) and matches the design default — a clarity note, NOT a code change.

Mars-2 (resume, blue-phase): the live-boot smoke on the reinit'd dev DB (Operator granted destructive PG), nit #1, a privacy mutation probe (own context), full re-gate.

### Y-8

Y-Mars-2 — Mars-2 blue-phase harden COMPLETE + full re-gate GREEN. All in the working tree (no git).

LIVE-BOOT SMOKE (Operator-granted destructive dev ops): dev DB codemojex_dev reinitialized (DATABASE_URL UNSET → config target; codemoji_game/codemoji_game_snapshot/codemoji_game_test/codemoji_snapshot UNTOUCHED). Migration up clean from zero. Seed: EMS-1 (10×15@72, 150 cells) + EMS-2 (10×21@72, 210 cells) → Postgres. Booted MIX_ENV=dev mix phx.server (Bandit 1.11.1 @ 127.0.0.1:4000), polled ~1s to bind. Curls: GET /api/health → {"status":"ok"}; GET /api/rooms → {"rooms":[]} then non-empty after a shared-DB room create; GET /api/games/GAM0OFexWes5p2 → {"view":{"free":false,"status":"open","game":"GAM…","emojiset":{"count":36,...},"room":"ROM…","ends_ms":…,"prize_pool":100,"prize_usd":"$1.20","totals":{...}}} — the renamed entity serves over HTTP; grep for 'secret' in the body → EMPTY (privacy line holds at the wire). Server killed (prove-then-kill, :4000 down).

DUST FIX (NIT #1) — CHOSE THE CODE FIX (remainder→rank 1), not doc-softening: economy.ex's doc + design §3.8.2 promise "the whole pool is distributed", and the golden pool is BOOSTED (stranding boosted diamonds is worse than the winner_take_all floor-convention). New private add_dust/2 adds the integer-division remainder to rank 1 (the top scorer); purity preserved. Pinned with 2 tests (updated "fewer players" → 616+384=1000; new "drains the whole pool" → pool 999 sums to 999). The K=5 / [70,30] cases are unaffected (dust=0). +1 test (40→41).

MUTATION PROBE (privacy, reverted net-zero IN this turn): injected secret: r.secret into view.ex game_view; golden_blind scenario 1 FAILED exactly (refute Map.has_key?(view, :secret); secret shown). Reverted byte-exact; grep view.ex → no :secret/:nonce. The privacy assertion is proven LIVE (catches a real leak); no leak ever sat in the tree across a Director turn.

BONUS HARDEN (blue-phase find): golden_blind_story_test.exs had a latent timer-race flake (duration_ms: 50 → submit could hit {:error, :expired} under an unlucky isolation seed; seed 1 reproduced it). The Mars-1 40/0 suite + 150× loop both hid it (full-suite ordering landed safely). FIX: duration_ms: 600_000 + explicit close_now/1 (dispatches on settlement=="sealed" regardless of timer) + dropped the Process.sleep(60) timer-waits. Verified 10 seeds all 4/0; seed-1 now green. Net-zero on what it proves.

RE-GATE (all PASS, echo/apps/codemojex, TMPDIR=/tmp): compile --warnings-as-errors CLEAN; valkey 6390 PONG + pg up; mix test --include valkey → 41 tests, 0 failures; residual greps \b(RND|RMM|USR)\b → 0, entity/wire/bonus → 0, BIF round( survives scoring.ex; migration up/down clean on test DB; ≥100 loop → 150/150.

BOUNDARY: Mars-2 delta = economy.ex + economy_story_test.exs + golden_blind_story_test.exs (+ the net-zero-reverted view.ex mutation). No sibling umbrella app; no mix.lock. Apollo spec-sync note (golden economy="winner_take_all" descriptor) LEFT as-is — do_close dispatches on settlement=="sealed"→top_k_split, NOT economy; correct + harmless, no code change.

STATUS: BUILD-GRADE green, ready for the Director's ship-gate + LAW-4 pathspec commit. Ledger: P-2, L-8, Y-2.

### Y-9 — Apollo HIGH-risk evaluation: BUILD-GRADE (the §11.2 charter, all four elements + spec-sync)

VERDICT: **BUILD-GRADE.** The rung (3 brand re-bases + the from-scratch 6-table redesign + the money-adjacent blind/sealed/commit-reveal Golden + the wire cutover) is independently gate-green and every load-bearing invariant is proven by read, exercised by test, AND pinned by a 6/6 mutation kill. No blocking delta.

A. THE PROMPTED-CHECKS TABLE (each PASS, file:line):
| Invariant | As-built | Verdict |
|---|---|---|
| 3 brand re-bases (0 RND/RMM/USR, GAM/ROM/PLR mint) | lib+test grep=0; rooms.ex:24 ROM, rooms.ex:71 GAM, wallet.ex:21 PLR, tables.ex:59 kind GAM | PASS |
| BIF spared | Kernel.round scoring.ex:52, economy.ex:22/94 survive | PASS |
| 6-table + ONE migration + CHECKs | 20260618000000_create_codemojex.exs: players/transactions/emoji_sets/rooms/games/guesses; games_type + games_status + players_non_negative CHECKs; NO notifications | PASS |
| migration up/down | independent: up→rollback --all→re-up all clean | PASS |
| INV-9 privacy (secret + preimage never cross pre-reveal) | view.ex game_view base never selects :secret/:nonce; revealed?/1 (view.ex:121) gates totals/points(my_history:93)/leaderboard(:106); grep :(secret\|nonce) in view+web=0 | PASS |
| commit-reveal V-14 | rooms.ex:153-156 SHA-256(code₀‖…‖code₅‖nonce, 0x1E)|>encode16(:lower); seal at open (rooms.ex:145), published commitment recomputes (golden_blind S3) | PASS |
| sealed top-K V-15 | top_k=5 (rooms.ex:104); stored payout_split [40,25,15,12,8]; economy.ex top_k_split/3:64 normalizes over present weights; dust→rank-1 add_dust/2:80 (whole pool); exactly-once SET NX rooms.ex:181 | PASS |
| reduced N-cell V-16a | snapshot_cells rooms.ex:135 take_random(codes,min(n,len)); secret from cell_codes (rooms.ex:81); EMS seed cell_size 72 (10×15=150, 10×21=210) | PASS |
| tier removal + linear | scoring.ex linear only (no tier fn); Board.record/3:15 raw best; guesses schema no tier/percentage | PASS |
| state machine | classic open→settled; golden open→revealing(rooms.ex:230)→settling→settled(:248); games_status CHECK 7 words | PASS |

B. UN-PROMPTED FINDING: INV-9 is a VIEW-LAYER invariant, not storage-absence — Board.record/3 (game.ex:122) writes the golden score to Valkey in-flight + the secret sits on the Postgres games row from open; the WIRE is gated by View.revealed?/1 + the game_view base map. Correct (a sealed contest needs its board to settle). Swept clean: wallet.ex (every path FOR-UPDATE + non-negative + ledgered; credit/5 negative guard typed, not catch-all); the ONLY lib catch-all is telegram.ex:54 (external-HTTP boundary→:bad_response, not money/state); games.secret NOT NULL satisfied for classic too (rooms.ex:81 unconditional); economy="winner_take_all" on golden = a stored descriptor (do_close dispatches on settlement, NOT economy — Director Y-7 INFO confirmed).

C. ATTACK THAT HELD: the exactly-once double-close. M2 (break SET NX path) SURVIVED green — the status %{status: :settled} guard (rooms.ex:175) short-circuits a sequential re-close before SET NX. M2b (defeat the status guard, NX intact) ALSO survived — the SET NX lock (cm:<game>:closed already set) returns not-OK→{:ok,:already_closed}. Only M2c (BOTH defeated) killed it. So a second close cannot re-pay: TWO independent guards (the status check + the Valkey SET NX) each refuse it (defense-in-depth, L-9). Held.

D. MUTATION KILL-RATE: 6 distinct PRODUCTION-code mutations on the money/privacy/fairness surface, each Edit-in→suite-catches→revert→shasum byte-exact (view/rooms/economy/game.ex all match pristine post-battery, net-zero). M1 secret-leak→KILLED(privacy_story + golden_blind S1); M2/M2b/M2c exactly-once→defense-in-depth (single survive, both KILLED); M3 commit-hash-corrupt→KILLED(golden_blind S3); M4 dust-skip→KILLED(2 economy tests, 995≠999 + 615≠616); M5 suppression-defeat→KILLED(golden_blind S2 refute_received); M6 score-gate-defeat→KILLED(2 golden_blind asserts). **6 introduced / 6 killed.** No survivor leaves a hole.

INDEPENDENT GATE (re-run): compile --warnings-as-errors exit 0; mix test --include valkey 41/0; migration up/down/re-up clean; golden_blind isolation 10 seeds (incl. seed 1) all 4/0 → the L-8 timer-race fix HOLDS; scoped 30× valkey loop 30/30.

SPEC-SYNC (docs only; production shasums unchanged): game-model.design.md §3.8.2 (top_k_split arity /2→/3 + add_dust/2 whole-pool drain) + §line-370 economy-descriptor note; cm.3.md §G3 (top_k_split/3 + dust drain); cm.3.stories.md Liveness (the duration_ms 600_000 + explicit close_now test discipline). design.md verify-only (0 legacy brands, committed d508d419). bcs.2/bcs.todo OUT of scope.

NON-BLOCKING (for the Director / a future rung): (1) the SET NX is not exercised under genuine concurrency by the suite (the two closes are sequential; the status guard masks the NX in the happy path) — a concurrent-close race test would exercise the NX directly; the invariant holds via M2b's standalone proof. (2) the dev-DB liveness (mix phx.server + curl) is an Operator/Director non-ephemeral task (Mars proved it transiently in Y-8; a spawned node is reaped at turn-end). Neither blocks the ship.

No git; no production code (the 6 mutations reverted net-zero). Working tree left for the Director to ratify + LAW-4 commit. Ledger: T-23/T-24, L-9/L-10, this Y-9.
...
P-Mars-2 — Mars-2 blue-phase harden COMPLETE (net-zero behaviour over green, except the one ruled dust fix). Four asks done:
(1) LIVE-BOOT SMOKE: dev DB codemojex_dev reinitialized (Operator-granted; DATABASE_URL UNSET → config target; codemoji_game* DBs untouched). Migration up clean from zero. Seed ran: EMS-1 (10×15@72, 150 cells) + EMS-2 (10×21@72, 210 cells) persisted to Postgres. Booted MIX_ENV=dev mix phx.server (Bandit at 127.0.0.1:4000), polled until bound (~1s), curled REAL routes: GET /api/health → {"status":"ok"}; GET /api/rooms → {"rooms":[]} then (after a one-shot mix run created ROM…/GAM… on the shared DB) the active room renders; GET /api/games/GAM… → the full game view ({"view":{"game":"GAM…","room":"ROM…","emojiset":{count:36,...},"prize_pool":100,...}}) — the renamed entity serves over HTTP; NO secret in the body (privacy line holds at the wire). Killed the server (prove-then-kill; :4000 down — exit 144 = SIGTERM, clean). The suite cannot give this (server:false) — the mars.md boot-the-real-release check.
(2) top_k_split DUST (NIT #1) — chose the CODE fix (assign the integer-division remainder to rank 1) over softening the doc, because economy.ex's doc + design §3.8.2 promise "the whole pool is distributed" and a golden pool is BOOSTED (stranding boosted diamonds is worse than the winner_take_all strand-convention). New private add_dust/2 gives the floor remainder to rank 1; purity preserved (deterministic). Pinned with 2 tests: the updated "fewer players" scenario (now 616+384=1000, was 615+384=999) + a new "drains the whole pool" scenario (pool 999, 5 ranks, sum==999). +1 test (40→41).
(3) MUTATION PROBE (privacy): injected secret: r.secret into view.ex game_view base map, ran golden_blind — scenario 1 FAILED exactly as designed (code: refute Map.has_key?(view, :secret); secret: ["0503",...]). Reverted byte-exact; grep view.ex → no :secret/:nonce (clean); the probe never left a leak in the tree. The privacy assertion is LIVE (proven it catches a real leak).
(4) RE-GATE all green: compile --warnings-as-errors clean; mix test --include valkey → 41 tests 0 failures; residual greps \b(RND|RMM|USR)\b → 0, entity/wire/bonus → 0, BIF round( survives scoring.ex; migration up/down clean on test DB; ≥100 loop 150/150.
BOUNDARY: Mars-2 delta is economy.ex + economy_story_test.exs + golden_blind_story_test.exs ONLY (+ the reverted-net-zero view.ex mutation). No sibling app, no mix.lock. Apollo spec-sync note (golden economy="winner_take_all" descriptor) left as-is — do_close dispatches on settlement=="sealed"→top_k_split, NOT economy; correct + harmless.
