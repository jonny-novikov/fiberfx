...
<!-- [RECONCILE] Scoring/Board mechanics (the tier + first-mover-bonus economy) are removed by D-5
     (linear `points` is the sole score and rank), and the blind/sealed Golden mode is added by V-6
     Arm B. Those are the relational-redesign surface — authoritative in `codemojex.game-model.design.md`
     (VenusPG) + `specs/cm.1.md`. This mirror is reconciled HERE for the entity/brand rename forward to
     the canon brands (game `GAM`, room `ROM`, player `PLR`) only; its deep model prose follows the
     as-built code after the build, not ahead of it. -->
...
**Rooms and the keyboard — `Codemojex.Rooms` and `Codemojex.EmojiSet`.** A room holds the props a game inherits — emoji set, duration, seed prize pool, guess fee, and whether it is free. An emoji set is a sprite sheet plus the subset of cells a room exposes; a code is `XXYY` (column then row), and a game's secret is six distinct codes drawn from the set. The player-facing snapshot of a set carries the keyboard and nothing the secret could leak from.
...
**Competitive state — `Codemojex.Board`.** The leaderboard is a Valkey sorted set per game, scored by the player's best linear total (D-5 removes the first-mover tier-bonus economy; the raw best total is the rank). A separate in-memory CHAMP projection (`Codemojex.Leaderboard`) is available as a rebuildable view.
...
When the scoring worker finishes a guess in a **live** (classic) game, it announces the result twice for two audiences. It publishes a `scored` event on the bus with `EchoMQ.Events` for any bus consumer, and it broadcasts a `:scored` message over Phoenix PubSub on the game's topic for the live surface. Both carry the player's name, percentage, and effective score — and neither carries the secret or the guess content. A **blind** (golden) game suppresses this per-guess announcement entirely (V-6 Arm B): it stores the guess and reveals nothing until the sealed reveal at close.
...
- **`players`** — `PLR` key; `keys`, `clips`, `diamonds`, `bonus_diamonds`, `locked_diamonds` as non-negative big integers, guarded by a CHECK constraint that the wallet leans on as a backstop.
- **`transactions`** — `TXN` key; `player`, `currency`, `delta`, `reason`, `ref`; append-only, indexed by `(player, inserted_at)` for a statement. (The column name `player` holds a `PLR` id; the brand re-base moves the id value, not the column name — V-12 Arm A.)
- **`emoji_sets`** — `EMS` key; the sprite grid (`cols`, `rows`, `cell_size`, `sprite_url`) and the exposed `codes`.
- **`rooms`** — `ROM` key; the template props and at most one active `game`.
- **`games`** — `GAM` key; the `type` + the four policies (feedback / scoring / settlement / economy), the `secret` (server-side, selected by no player-facing query), the timer, the prize pool, and the fee props; indexed by `room`. For the blind (golden) mode it also carries the commit-reveal columns (`commitment` / `nonce` / `revealed_ms` / `top_k`). <!-- [RECONCILE] the games `type`/policy/blind columns + the widened status set are the relational redesign, authoritative in `codemojex.game-model.design.md` (VenusPG) + `specs/cm.1.md`; this mirror names them, the model doc shapes them. -->
- **`guesses`** — `GES` key; the player's `emojis` and `points`; indexed by `(game, player)`. (D-5 removes the `percentage` and `tier` columns — linear `points` is the sole score; `percentage` is computed-not-stored.)

Alongside it, the Valkey keyspace for one game is a small family under `cm:<game>:` — `board` (the sorted set, ranked by the best linear total), `base` (the per-player best-total hash that feeds the ZSET), `players` (a set), `attempts` (a counter), and `closed` (the one-shot close lock) — plus the global `cm:total_won`. (D-5 removes the first-mover `ptier` / `bonus` / `tierfirst` hashes.)

## Core flows

**A guess, end to end.** The surface accepts `POST /games/:id/guess` and returns immediately. `Codemojex.Guesses` reads the game from the system of record, checks it is open and unexpired, validates the six emojis against the keyboard, overlays the player's locks, and charges the fee through the wallet; only a charged guess is enqueued on the player's lane. The `ScoreWorker` claims it, reads the secret through the cache, scores it, writes a `GES` guess, increments the game's attempt counter, records the result on the board, and — in a **live** game — announces it on the bus and over PubSub; a **blind** game stores the guess and announces nothing (V-6 Arm B). A perfect 600 closes a live game there and then.

**Settlement, exactly once.** A game closes from one of two triggers — a perfect crack, or an expired timer — and the two can race. The close path takes a one-shot lock with an atomic `SET … NX` on `cm:<game>:closed`; only the closer that wins it pays, and the loser is a no-op. For a **live** game the winner reads the top of the board, computes the winner-take-all split over the diamond pool (shared evenly on a tie), deposits each prize through the wallet, bumps the global total-won counter, marks the game closed, and returns the room to waiting. A **blind** game closes on the timer, reveals the secret and nonce, runs one sealed settlement pass over every guess, and pays the top K from the pool. <!-- [RECONCILE] the blind close/reveal/sealed-settlement flow is the V-6 Arm B redesign — authoritative in `codemojex.game-model.design.md` + `specs/cm.1.md`; this mirror names it, the model doc shapes it. -->
...
**The leaderboard.** A scored guess updates the player's best linear total in the `cm:<game>:base` hash and writes that total to the sorted set; the board is ranked by the raw best total (D-5 removes the first-mover tier-bonus economy — there is no `HSETNX` tier race, no bonus, no effective-vs-base distinction). The score a player sees is the score they earned.
...
The live half is Phoenix Channels. A client joins `game:<id>`, which subscribes the channel to the matching PubSub topic. For a **live** game, when the scoring worker finishes an attempt the channel pushes the `scored` event to the client, and the leaderboard updates without any per-room process. For a **blind** game the channel carries state and the timer only — no per-guess results — until the sealed reveal at close pushes the revealed secret, the final board, and the payouts in one event. Joins return the game view, never the secret (and, for a blind game, the public commitment but never its preimage), and a refresh re-reads the view and the board on demand.
