...
Codemojex is a real-time, multiplayer code-breaking competition that runs as a Telegram Mini App — Mastermind played with an emoji keyboard, for money. This document is the binding design of the `codemojex` application as it stands in the Echo umbrella: its engine, its systems, its messaging, its storage tiers, its data model, the way it is built and deployed for production, and the questions still open before launch. It is written against the source, not ahead of it — every module, namespace, table, and Valkey key named here exists in `echo/apps/codemojex`. The **Golden Room tournament** (the approved launch model, calibration ledger `D-18`), once reconciled here ahead of its build, shipped via cm.5 — the document now reads as-built throughout, and the only remaining forward-tense surface is the `cm.8+` ladder (cash-out, the rake, membership), marked where named.
...
| `golden` | **`none`** — no per-guess signal until reveal | **`sealed`** — one batch at close, pay the top K | linear distance | per-guess fee (all-pay), top-K split over the pool |
...
**The Golden Room — a tournament on the classic base.** Distinct from the `golden` *type* above, a **Golden Room** is a `classic`-typed game carrying the `golden` **marker** — a live tournament that gathers a field of paid members, funds its prize from their **buy-ins**, and at close splits the pool among the top finishers. It selects a third settlement policy, `settlement: "live_split"` / `economy: "proportional"`, which reuses the same linear scoring and the same top-K split math as the blind mode but pays **live** (no reveal phase) over a **buy-in-funded** pool, and grants consolation clips to every other member. The marker is neither the blind type nor a "boost class"; the removed `gold_multiplier` was backend and canon drift absent from the product (`D-16`). The Golden Room's economy is the decision surface `specs/economy/economy.md` §8; its `:gathering` state, the buy-in, and `close_split` landed via cm.5 (`codemojex.roadmap.md`).
...
Fifteen namespaces carry the game today — the founding nine:
...
…and the six their shipped systems added: `SES` session (cm.4, Valkey via `EchoStore.Table`) · `RVL` revenue ledger (cm.6, Postgres `revenue_ledger`) · `PKG`/`ORD`/`OTX`/`WHK` commerce (cm.7, Postgres `packages`/`orders`/`order_transactions`/`webhooks`). The forward namespaces the roadmap schedules for the deferred systems (`RMP` membership · `BNK` bank · `RSC` resource · `SHR`/`AEV` for growth and analytics) land with their systems.
...
**Reads — `Codemojex.View`.** The player-facing reads carry the privacy invariant in their shape: no view returns the secret, and no view returns another player's guesses. For a golden-type (blind) game the gate widens — no score crosses the wire until the game's `revealed_ms` is set (a Golden Room, being `classic`-typed, has no such gate).
...
- **`games`** — `GAM` key; the per-play entity. It carries:
  ...
  - the timer, the diamond `prize_pool`, the fee props, and the `golden` tournament marker (the `gold_multiplier` boost is removed — `D-16` — backend and canon drift absent from the product; the build rung reconciles out the column);
  - the `status`, a CHECK-bounded text word over the canon states `scheduled · gathering · open · active · revealing · settling · settled · voided` (`gathering` landed with cm.5). Classic traverses `open → settled`; the blind `golden`-type game traverses `open → revealing → settling → settled`; a Golden Room (the tournament) traverses `gathering → open → settled`; `voided` is the abort path (a never-filled gather voids and refunds its buy-ins); indexed by `room`.
...
A room is a template and a container; a game is one playthrough inside it, with a small state machine. When a player joins a waiting room, a game is formed: a fresh six-emoji secret is minted from the game's snapshotted keyboard, the room's properties and policies are snapshotted onto it, an end instant `ends_ms` is set, and the game enters `open`. (A Golden Room is the exception: it forms in `gathering` with `ends_ms` unset, accepting buy-ins and guesses while no timer runs, and enters `open` — setting `ends_ms` — only when its `start_threshold`-th paid member gathers, ten by golden default.)
...
A **Golden Room** prepends a gather phase to this machine (cm.5): the game forms in `gathering` and admits buy-ins and guesses with no timer; when the tenth paid member buys in, the `gathering → open` transition — guarded by its own one-shot `SET cm:<game>:started NX` so two concurrent tenth buy-ins cannot both start the timer — sets `ends_ms` and the live round begins; from `open` it closes and settles like any classic game, paying the top finishers a proportional split and the rest consolation clips. A gather that never completes voids on a deadline and refunds the buy-ins exactly once. The timer-close and the never-fills auto-close are driven by the periodic **sweep** cm.5 wired — `Codemojex.Sweep` calls `close_if_expired/1` on a cadence, so an expired or never-filled game closes without any player action.
...
The surface is a thin Phoenix application, deliberately small. A JSON API under `/api` covers the lifecycle: a health check, the auth handshake (`POST /api/auth/:platform`), the room lobby, joining a room, a game view, submitting a guess, a player's own history, the leaderboard, and the two key operations (buy and convert). Every action calls the `Codemojex` facade and the privacy-safe views; a guess returns an accepted-and-on-its-way response while the bus carries the scoring, and there is no process per room, so a large field of idle rooms costs nothing.
...
Player identity is verified, not trusted (cm.4): the surface accepts Telegram `initData`, whose signature the pure `Codemojex.InitData` verifier checks with an HMAC-SHA-256 over the bot token keyed by the constant `WebAppData`; the handshake `POST /api/auth/:platform` resolves-or-creates the player by `tg_user_id` and mints the one `SES` session, held in Valkey through a mutable `EchoStore.Table` with `:tracking` coherence so revocation is immediate. The `:auth` plug and the socket read `conn.assigns.player`, and the old unauthenticated player mint (`POST /api/players`) is retired.
...
Fairness has two meanings here, and the design serves both. Procedural fairness is the per-player lane on the guess queue: service rotates across players, so paying for speed buys a turn, not the field. Economic fairness is the candid economy — every guess pays a fee, the prize pool is stated up front (platform-seeded for an ordinary room; funded by the field's own buy-ins for a Golden Room tournament, where the per-guess fee is platform revenue), and the conversion rate is fixed and public, rather than hidden house entries playing against the field. The blind mode adds an anti-abuse property: with no per-guess signal a feedback-driven clicker bot cannot hill-climb toward the secret, and the all-pay structure gives blind farming a negative expected value. The exact fee and pool terms are the kind of decision that belongs with the chief architect and legal review before launch (see Open questions).
