...
> **Status: ✅ BUILT — BUILD-GRADE (Director-verified 2026-06-30); A2 RULED + shipped arm (a) — composes
> Mercury's owned date foundation; sub-batch 1 of the mx.7.3 split (the FIRST to ship).** Inherits the
...
**Verdict: BUILD-GRADE** (Director independent verify, 2026-06-30; built via a REAL aaw Trio in two
connection-death-resilient mars waves — wave 1 `@mercury/core`, wave 2 `@mercury/ui` — per x.md §5 LAW-1b, the
write-ready dispatch this rung's deaths surfaced). All K-1..K-9 / INV-1..INV-12 / S-1..S-6 **MATCH**. A2 →
**arm (a)** (compose the owned date foundation).

**Shipped — Half 1 (`@mercury/core`, wave 1).** A curated, reusable `useDateField` composable over the existing
`internal/date-time` machinery — a *compose*, not a re-derivation:
- `packages/mercury-core/src/internal/date-time/field/use-date-field.ts` (NEW). Signature: `useDateField(options?:
  { value?: DateValue; defaultValue?: DateValue; onChange?: (v: DateValue|undefined) => void; locale?: string;
  granularity?: "day" }) => { segments: { part; value }[]; fieldProps; segmentProps: (part) => SegmentProps;
  value: DateValue|undefined }`.
- **Composed** (no native date math): `helpers.ts` `initializeSegmentValues`/`createContent`/`getValueFromSegments`/
  `areAllSegmentsFilled`/`isDateSegmentPart`; `segments.ts` `getSegments`/`getNextSegment`/`getPrevSegment`;
  `utils.ts` `getDaysInMonth`; `formatter.ts` `createFormatter`; `@internationalized/date` `CalendarDate` +
  `type DateValue`. The **only new logic** is the per-keystroke reducer (digit width/overflow/zero-pad + caret
  hand-off · Arrow ± with min/max wrap · ArrowLeft/Right nav · Backspace) + the spinbutton a11y props.
- Curated barrel additions (`D-5`, additive — the 3 prior exports byte-untouched): `useDateField` +
  `type { UseDateFieldOptions, UseDateFieldReturn }` + the value kit `type { DateValue }` +
  `{ CalendarDate, parseDate }` (re-exported from `@internationalized/date`).

**Shipped — Half 2 (`@mercury/ui`, wave 2).** The `inputs/DateField/` 4-file home consuming the composable:
`DateField.tsx` (a function component — not `forwardRef`; renders `useDateField`'s `segments`, spreads
`fieldProps`/`segmentProps`; `.mx-datefield` + tokens; `readOnly` segment inputs the hook drives) · `index.ts` ·
`DateField.prompt.md` (D-7; cross-links `useDateField` + `Calendar` [mx.7.3.2 gap] + `Input`) ·
`DateField.stories.tsx` (CSF3; Playground/Uncontrolled/Controlled). Public surface: `DateField` +
`DateFieldProps`; the value type is `DateValue` (through `@mercury/core`). `additions.css` +1 `.mx-datefield`
block (tokens only, `--radius-8`, `:focus-within`; no `tokens.css` change — INV-10 clean).

**Gate (Director independent re-run, EXIT 0):** `pnpm --filter "./packages/*" typecheck` ✓ · `build` ✓ ·
`pnpm sb:typecheck` ✓ · `pnpm sb:build` ✓ (DateField story registered) · `pnpm --filter "./apps/*" --filter
"!@mercury/storybook" build` ✓ (echomq + mobile). **Master invariant:** barrel **50 → 51** folder-exports
(+`DateField` +`DateFieldProps`), **0 removed/renamed** (git-diff = one added line; dist `index.d.ts` confirms).
**INV-6 holds:** `grep "@internationalized/date" packages/mercury-ui/src` = empty (the 2 hits are prose);
`mercury-ui/package.json` deps unchanged (only `@mercury/core`); no `@/` in the core hook. **Hygiene empty:**
inline-color, raw-hex, `mercAccent`, bundle-framing, `/design-sync`. **Mutation spot-check:** a `__bogus__`
field destructured from `useDateField`'s return → `tsc` TS2339 (the cross-package contract is type-checked),
restored net-zero.

**Deviations (realization-over-literal — none changed the public surface):** (1) the hook seeds uncontrolled
state from `defaultValue`; (2) a null-field-node guard routes to `onChange(undefined)` (else
`areAllSegmentsFilled(_, null)` returns true → a bogus neutral-date pre-mount/SSR); (3) `preventDefault` on
digits + Left/Right (the hook owns the displayed value); (4) literals render as plain separators without
`data-segment` (correct — `getSegments` excludes literals, so only editable segments need it); (5)
`minValue`/`maxValue` deferred (additive later — no gold-plating). **Known limitation (contract Notes):** `value`
is optional-not-nullable, so `value={undefined}` reverts to uncontrolled (React convention, matches the bundle API).

**Process (the rung's own lesson, folded forward).** The build surfaced the **connection-death pattern** — three
Mars spawns died to `ECONNRESET` mid-*read* (the understanding phase), writing nothing. The fix, now **x.md §5
LAW-1b** + the venus/mars/apollo charters + the mercury-ship skill: the **write-ready dispatch** (the Director
pre-grounds signatures/paths/usage into the spawn so the peer writes first), **short waves**, and
**recover-from-tree**. Both surviving builds were write-ready, wave-split spawns. (Memory:
`spawn-resilience-effective-messaging`.)

**Carry forward to mx.7.3.2 (`Calendar`).** Reuse the SAME foundation via a sibling `useCalendar` over
`internal/date-time/utils.ts` (`getDaysInMonth`/`getLastFirstDayOfWeek`/the month grid) + the SAME value kit
(`DateValue`/`CalendarDate`/`parseDate`, already on the `@mercury/core` barrel) — **no second machinery copy, no
god-hook**. `Calendar` cross-links `DateField` (both compose the one owned date foundation).
