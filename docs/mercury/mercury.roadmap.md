...
| **mx.9** | III | **One comprehensive showcase application** — `apps/showcase/` (vite/React, source-resolved) serving the library · documentation · API · do/don't · recipes, replacing the retired apps — **SUB-EPIC, Operator-split 2026-07-02** into mx.9.1–mx.9.5 (the layered-engine axis; forks A–E RULED; the Squad-tier verifier mandate lands at the mx.9.5 closer). Sub-epic frame: [`specs/mx.9/mx.9.md`](./specs/mx.9/mx.9.md) | ✅ **COMPLETE** (2026-07-02 — mx.9.1–9.5 all BUILT; the visual parity close + regression harness at **mx.9.6**; Movement III closes) |
| ↳ **mx.9.1** | III | the **spine** — the `apps/showcase` scaffold (the source alias byte-mirroring `apps/echomq` + the `storybook/test` no-op shim), a sanity page (barrel exports + token swatches), the fresh root `dev:showcase` (:5176), the apps gate 2→3 · epic INV-1/2/3/9 · NORMAL/Trio | ✅ **BUILT** (gate-green 2026-07-02; `/mercury-ship mx.9.1`; Duo+ pass-1 clean; 3-app gate; barrel byte-identical; LAW-1a alias mutation; lockfile importer deferred to the sibling commit) ([`specs/mx.9.1/mx.9.1.md`](./specs/mx.9.1/mx.9.1.md)) |
| ↳ **mx.9.2** | III | the **derived registry + shell** — two lazy `import.meta.glob`s over the real component tree (INV-6), the grouped sidebar + topbar, the persisted route, Stories/Docs tab stubs, the theme-toggle mechanism · epic INV-4/6 · NORMAL/Trio | ✅ **BUILT** (gate-green 2026-07-02; `/mercury-ship mx.9.2`; Duo+ pass-1 clean; 65-parity derived; INV-5 no-loader held; the S-1 probe run twice incl. an adversarial unknown-group pass; packages untouched, barrel byte-identical; as-built record §7) ([`specs/mx.9.2/mx.9.2.md`](./specs/mx.9.2/mx.9.2.md)) |
| ↳ **mx.9.3** | III | the **live-stories surface** — the CSF interpreter (the bundle `StoryBlock` pattern, typed; `play` never runs) + **the shim liveness gate** (all 65 story modules load; the 11 `storybook/test` value-importers are the proof surface) · epic S-5/INV-5 · ELEVATED/Trio + deepened verify | ✅ **BUILT** (gate-green 2026-07-02; `/mercury-ship mx.9.3`; Trio — re-sharpen census corrected the seed's algorithm (args merge · meta render · no `parameters`), FORK-1 decorators RULED Arm A; **the shim liveness gate PROVEN twice** — 65/65 modules + the 11 importers named, both probes run twice; behavioral mutation caught by the sweep; pass-1 clean; as-built record §7) ([`specs/mx.9.3/mx.9.3.md`](./specs/mx.9.3/mx.9.3.md)) |
| ↳ **mx.9.4** | III | the **contract surface** — the ported zero-dep compact markdown renderer; Docs = the full `.prompt.md`, API = `## Props`, do/don't = `## The enum language` + `## Notes` (the 56/65 census — a missing section renders EMPTY), recipes = `## Examples` · epic S-6/S-7/INV-5 · NORMAL/Trio | ✅ **BUILT** (gate-green 2026-07-02; `/mercury-ship mx.9.4`; Trio pass-1 clean — the ship-time construct inventory KILLED five seed assumptions (continuation-joining · the xref rule · italics · the sentinel mask-first pass · escaped pipes); FORK-1 grain RULED Arm A (nested sub-tabs, route untouched); the Director 8-witness corpus sweep over all 65 contracts + a cutter mutation net-zero; barrel byte-identical; as-built record §7) ([`specs/mx.9.4/mx.9.4.md`](./specs/mx.9.4/mx.9.4.md)) |
| ↳ **mx.9.5** | III | the **closer** — the seed-skinned chrome (Fork C Arm C, token-expressed), the dual-theme acceptance (epic S-8 full; the `--indigo-3` soft-bg limitation EXPECTED, logged upstream), the whole-epic INV-1..9 + S-9/S-10 re-run — **Movement III closes** · ELEVATED closer/**Squad + Apollo** | ✅ **BUILT** (gate-green 2026-07-02; `/mercury-ship mx.9.5`; Squad + Apollo BUILD-GRADE; Arm A clean skin — F1–F4 declined, zero new token; 308→388-line sheet; chrome fully dark-correct; whole-epic INV-1..9 + S-9/S-10 re-run green; as-built §8) ([`specs/mx.9.5/mx.9.5.md`](./specs/mx.9.5/mx.9.5.md)) |
| ↳ **mx.9.6** | III | **visual parity + the visual-regression harness** — completes mx.9.5's deferred `:5176` pixel acceptance (the mx.9.5 skin was fitted to the wrong donor `packages/mercury-ds/…/app.css`; the build gate is visually blind) + adds a Playwright harness (`apps/showcase/visual/`) so the pixel pass is **mechanical**, not a manual residual. Two Mars waves to stylistic parity with `static/showcase.html` in both themes + a live nav probe · Director + Mars | ✅ **BUILT** (2026-07-02; 6 `apps/showcase/src` files + the `visual/` harness + `package.json`; **A** mono-heading + **D** crosshatch fixed app-side; sidebar brand/dots · inset solid topbar · Home overview (65/9/3 derived) · eyebrow/title/lede header; barrel byte-identical, 0 hex, gate green; probe Actions→Button) ([`specs/mx.9.6/mx.9.6.md`](./specs/mx.9.6/mx.9.6.md)) |
| **mx.10** | — (toolchain) | **Workspace dependency reconciliation + the vite 7 lift** — a pnpm `catalog:` single-sources the shared toolchain versions (`typescript ~5.9.3` · `vite ^7` · `vitest ^4` · `react ^19` · `jsdom ^26` · `@vitejs/plugin-react`) across `packages/*` + `apps/*` + the workspace root (**NOT** `codemojex/**` — a sibling `/cm-ship` rung migrates it); lifts vite 6→7, converges the vitest dual-major, tilde-pins TS. **Orthogonal to Movement III** (a toolchain rung, not a feature rung); the barrel stays byte-identical. [`specs/mx.10/mx.10.md`](./specs/mx.10/mx.10.md) | ✅ **BUILT** (gate-green 2026-07-02; `/mercury-ship mx.10`; catalog + vite **7.3.6** + vitest **4.1.9** across 13 manifests incl. `apps/showcase`; jest-dom retired; +`@types/node` on `@mercury/core`; Duo; barrel byte-identical; INV-1..8 green) |
...
> `/cm-ship` migration); the master invariant (the `@mercury/ui` barrel) holds byte-identical. Under the
> 2026-07-02 mx.9 split, `mx.9.1` pins the CURRENT toolchain (vite `^6` via the `apps/echomq` mirror) and
> the later sub-rungs inherit the `mx.10` `catalog:` automatically if it ships mid-epic.
