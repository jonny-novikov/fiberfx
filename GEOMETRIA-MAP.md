...
>
> **Статус — карточки «скоро» (выполнено):** 9 заглушек «следующий модуль»
> (`href="#"` + бейдж `готовится` + `<span class="go">скоро</span>`) на главах M1–M8
> переведены на существующие страницы модулей M2–M9; бейдж убран, подпись → «читать →».
> Coming-soon «скоро» на страницах `/school/geometria` не осталось (слово «скорость» в
> текстах — это контент, не трогали). На *не*-geometria страницах школы такие заглушки
> ещё есть (вне объёма).
>
