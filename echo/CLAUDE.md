...
| F6.1 | **Phoenix** replaces the Bandit/Plug web layer in a NEW umbrella app `apps/portal_web` (OTP `:portal_web`, namespace `PortalWeb`, depends `{:portal, in_umbrella: true}`); Phoenix lives ONLY in `apps/portal_web/mix.exs`, so the master invariant is now app-level compiler-enforced. The supervision tree splits into two app supervisors: `Portal.Application` keeps `[Portal.Store, Portal.EventStore.adapter(), {Portal.Engine, []}]` (Bandit child dropped, four→three) and `PortalWeb.Application` owns `[PortalWeb.Telemetry, PortalWeb.Endpoint]` (`max_restarts: 300` — a brutal endpoint kill storms ~200 restarts through the LiveView socket pool). Adds `PortalWeb.Endpoint`, a facade-backed `/courses/:user_id` route (the `%Enrollment{}` read model `courses_of/1` reads), and `/health`. F6.1-D2 deliberately **retains** `Portal.Store` as the dual-write read model — the `f6.1.md`-tree concern is settled, not open. | **shipped 2026-06-04 (`470cd90`; self-heal `0f54026`)** |
| F6.2 | **Routing & the access surface** over the facade: the `:api` + `:require_auth` pipelines on top of F6.1's `:browser`; a public scope (landing `/`, the reused `/courses/:user_id` `:index`, `resources` `/lessons`, `post` `/enroll`, `live` `/enroll/:id`) plus a JSON `:api` route; a protected `/learn` scope `pipe_through [:browser, :require_auth]`; the `PortalWeb.RequireUser` module plug + a function plug; thin `PageController` / `LessonController` (an `:error -> 404` carve-out) / `EnrollmentController` over the facade; the `:live_view` macro + a real thin `EnrollmentLive`. INV1 holds — no Engine/Repo/`GenServer.call` in web code. | **shipped 2026-06-04 (`d0055d9`)** |
| F6.3–F6.9 | The remaining Phoenix arc above the preserved boundary (F6.3 = Ecto persistence, next), then deploy. The domain, ids, store, and engine boundary are **preserved** — nothing below the boundary changes. | planned |
