...
`echo_mq` · `echo_cache`, the Valkey-native data plane built on ETS and Lua. It records the layout, the
...
**In scope:** the four BCS data-layer apps and their build/test mechanics — **Valkey · EchoCache · ETS ·
...
| `echo_cache` | **The cache** — L1 **ETS** over L2 Valkey (cache-aside), a per-group SQLite journal + Litestream shadow. Keyspace `ecc:{table}:{id}`; coherence is *a message about a name* (the BCS law, literally). | `echo_data` + `echo_mq` + `echo_wire` + `exqlite` |
...
graphs, no shared mutable state, no embedded id lists. `echo_mq` and `echo_cache` are *systems* built to this
...
- `echo_cache`'s **L1 is ETS**, with L2 Valkey behind it (cache-aside) and the store pushing invalidation
...
```bash
cd /Users/jonny/dev/jonnify/echo/apps/<app>     # echo_wire | echo_data | echo_mq | echo_cache
