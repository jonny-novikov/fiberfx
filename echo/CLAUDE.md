...
| `Portal.Engine` | `dispatch/1`, `query/2` | The boundary GenServer (the master-invariant seam). `dispatch/1` takes a **map** command (`%{type: :enroll, user_id: …, course_id: …}`) → `{:ok, term} \| {:error, atom}`. `query/2` is `(name_atom, arg)` → `{:ok, term} \| :error \| [term]` (`:lesson`, `:courses_of`). **Not yet event-sourced** — `init/1` returns a stub `%{}`; the held fold is F5.6. |
...
| F5.6 | The engine's **runtime home**: a supervised `Portal.Engine` GenServer that holds the folded state, recovers by **replay at `init`**, routes `dispatch`/`query` through the fold, and maps `authorize`'s atom → `%Portal.Error{}`. Reconciles the two contract paths and decides `:enrollments` vs the live `:courses_of`. | next |
| F5.7 | Pragmatic testing. | planned |
