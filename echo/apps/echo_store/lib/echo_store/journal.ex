defmodule EchoStore.Journal do
  @moduledoc """
  ...
  admission dedup absorbs the ones it still holds. Streaming durable state off
  the box is a separate concern by design — now native, via Graft Volumes
  streamed to Tigris S3 (`EchoStore.Graft.Streamer`), not the retired
  litestream sidecar.
  """

  use GenServer

  alias EchoStore.Coherence
  ...
  def init(opts) do
    ...
    # WAL checkpoints can briefly take a write lock; wait rather than error
  ...
  defp apply_and_record(s, table, name_id, version) do
    {:ok, verdict} = EchoStore.Table.apply_coherence(table, name_id, version)
