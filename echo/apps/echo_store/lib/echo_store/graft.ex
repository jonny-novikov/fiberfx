defmodule EchoStore.Graft do
  ...
  wired onto the Echo umbrella with no foreign engine.
    ...
    * **Engine** — `EchoStore.Graft.VolumeServer`, one single-writer process per
      Volume; its mailbox is Graft's global write lock.
    * **Store** — `EchoStore.Graft.Store` on CubDB's append-only immutable B-tree
      (zero-cost MVCC snapshots = Graft snapshots; pure Elixir, no C).
    * **L1** — `EchoStore.Table`, the existing `read_concurrency` ETS cache, used
      as the write-through head-page cache.
    * **Remote** — `EchoStore.Graft.Remote.Tigris`: segments and conditional
      commit objects on Tigris S3, shipped in real time by
      `EchoStore.Graft.Streamer` (the native replacement for Litestream).
    * **Bus** — `EchoMQ.Connector` carries low-latency commit *notices* via
      `EchoStore.Graft.Sync`; the bytes travel via Tigris.
    * **IDs** — BCS branded GIDs (`VOL` / `SEG` / `CMT`) from `EchoData.Graft.Id`.

  A typical write/read:

      cfg = EchoStore.Tigris.config(bucket: "echo-graft")
      {:ok, _pid} = EchoStore.Graft.open_volume(vol, data_dir: "priv/graft/\#{vol}", conn: conn, remote_cfg: cfg)
      {:ok, base} = EchoStore.Graft.begin(vol)
      {:ok, lsn}  = EchoStore.Graft.commit(vol, base, %{0 => page0, 1 => page1})
      {:ok, bin}  = EchoStore.Graft.read(vol, 0)
  """
  alias EchoStore.Graft.{VolumeServer, Reader}
  ...
  def open_volume(volume_id, opts) do
    ...
    DynamicSupervisor.start_child(EchoStore.Graft.VolumeSup, spec)
  end
