defmodule EchoStore do
  ...
  @directory :echo_store_directory
  ...
  defmodule Directory do
    ...
    @impl true
    def init(:ok) do
      :ets.new(EchoStore.directory_table(), [
        :set,
        :public,
        :named_table,
    ...
    def handle_call({:register, name, spec, owner}, _from, state) do
      ...
      :ets.insert(EchoStore.directory_table(), {name, spec})
      {:reply, :ok, %{state | monitors: Map.put(state.monitors, ref, name)}}
    end

    def handle_call({:unregister, name}, _from, state) do
      :ets.delete(EchoStore.directory_table(), name)
      {:reply, :ok, state}
    end
    ...
    def handle_info({:DOWN, ref, :process, _pid, _reason}, state) do
      ...
      if name, do: :ets.delete(EchoStore.directory_table(), name)
