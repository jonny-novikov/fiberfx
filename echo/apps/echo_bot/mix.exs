defmodule EchoBot.MixProject do
  ...
  # The engine boots its own supervision tree via EchoBot.Application (F10.1-INV2).
  # `:inets`/`:ssl` are OTP's built-in HTTP client stack the vendored
  ...
  # app's `lib/` tree (F10.1-INV4). The wrap — not the copy — is
  # the long-lived boundary: only EchoBot.Platform.Telegram names a vendored module.
  defp elixirc_paths(_), do: ["lib", "vendor/ex_gram"]

  # echo_bot keeps a minimal, self-contained dependency set (F10.1-INV1): `jason`
  # (already in the umbrella lock) decodes the Telegram JSON; `yaml_elixir` parses
  # the v1.0 bot YAML.
  defp deps do
    [
      {:jason, "~> 1.4"},
