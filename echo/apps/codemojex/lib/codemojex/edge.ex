defmodule Codemojex.Edge do
  ...
  The board bundle lives at static.codemoji.games (a Tigris public bucket) under a
  content-hashed name; a small `manifest.json` pointer at the bucket root names the
  current hash. `board_url/0` reads that pointer at runtime — cached for a few
  seconds — and returns the URL `GameLive` renders into the board mount point. The
