defmodule Codemojex.Guesses do
  ...
  The host never scores; the consumer does. Rounds are opened by `Codemojex.Rooms`.
  """
  alias EchoMQ.Lanes
  ...
  def submit(_round, _player, _emojis), do: {:error, :bad_guess}
    ...
    case Cache.fetch_round(round) do
      %{secret: secret} ->
        ...
        Phoenix.PubSub.broadcast(
          Codemojex.PubSub,
          "round:" <> round,
          {:scored, %{round: round, player: name, pct: s.percentage, tier: s.tier, eff: eff, first: claimed}}
        )
...
defmodule Codemojex.Settle do
  ...
  (`Codemojex.Rooms.close_round/1`), depositing the diamond pool to the max-score
...
defmodule Codemojex do
  ...
  their rounds, and whose diamond prize pools settle winner-take-all through a
  second queue. Startup is `Codemojex.Application`'s job — the Repo, PubSub, the
  EchoMQ bus and consumers, and the Phoenix endpoint come up there.
  """
  alias Codemojex.{Rooms, Guesses, Settle, View, Board, Wallet}
