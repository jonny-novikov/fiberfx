# A live round, end to end, on the current API.
#
# Requires Postgres (mix ecto.create && mix ecto.migrate) and Valkey on $VK_PORT.
# Run with:  mix run priv/round.exs
...
IO.puts("== Codemojex -- a live round on the bus, persisted in Postgres ==")
IO.puts("room #{room}")
...
# The first join opens the round: snapshots the set, mints the secret (server-side,
# in `rounds.secret`), and starts the timer. The second join lands in the same round.
...
{:ok, _} = Codemojex.submit(round, alice, guess)
{:ok, _} = Codemojex.submit(round, bob, ["0000", "0101", "0202", "0303", "0404", "0501"])
Process.sleep(400)

IO.puts("leaderboard (max score per player, no guesses leaked):")

Codemojex.leaderboard(round, 10)
|> Enum.with_index(1)
|> Enum.each(fn {{p, s}, i} -> IO.puts("  #{i}. #{p}  #{s}") end)

bal = Codemojex.balance(alice)
IO.puts("\nalice balance: keys=#{bal.keys} diamonds=#{bal.diamonds}")
IO.puts("alice history (own attempts only): #{inspect(Codemojex.my_history(round, alice, 3))}")

IO.puts("\nround view served to clients (note: no :secret key):")
IO.inspect(Codemojex.round_view(round), pretty: true)
