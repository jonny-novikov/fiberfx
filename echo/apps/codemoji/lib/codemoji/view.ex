defmodule Codemoji.View do
  @moduledoc """
  The player-facing reads, with the privacy invariant built in: nothing here
  returns the secret, and nothing returns another player's guesses. A round view
  carries the keyboard, the timer, the pool, and the room totals; a player sees
  only their own attempt history; the leaderboard is max scores, never guesses.
  """
  alias EchoMQ.Connector
  alias Codemoji.{Bus, Store, Cache, EmojiSet, Economy, Board}
  ...
  @doc "The player's own attempts (their guesses + scores), newest first."
  def my_history(round, player, n \\ 50) do
    case Connector.command(Bus.conn(), ["LRANGE", hist_key(round, player), "0", to_string(n - 1)]) do
      {:ok, ids} when is_list(ids) ->
        ids
        |> Enum.map(&Store.guess/1)
  ...
  defp hist_key(round, player), do: "cm:" <> round <> ":hist:" <> player
end
