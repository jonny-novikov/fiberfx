...
Deduplication in EchoMQ prevents duplicate jobs from being added to the queue. When a job with a deduplication ID is added, any subsequent attempts to add a job with the same ID are ignored until the deduplication condition is cleared.

## Modes

EchoMQ supports three deduplication modes:
...
EchoMQ.Queue.add("tasks", "process-file", %{file: "report.csv"},
  connection: :redis,
...
EchoMQ.Queue.add("notifications", "send-email", %{to: "user@example.com"},
  connection: :redis,
...
EchoMQ.Queue.add("search", "update-index", %{query: "first"},
  connection: :redis,
  delay: 5_000,
  deduplication: %{
    id: "search-index",
...
EchoMQ.Queue.add("search", "update-index", %{query: "updated"},
  connection: :redis,
  delay: 5_000,
  deduplication: %{
    id: "search-index",
...
```elixir
{:ok, job_id} = EchoMQ.Queue.get_deduplication_job_id("my-queue", "dedup-id",
  connection: :redis
)
...
{:ok, 1} = EchoMQ.Queue.remove_deduplication_key("my-queue", "dedup-id",
  connection: :redis
)

# Now a new job with the same ID can be added
EchoMQ.Queue.add("my-queue", "job", %{},
  connection: :redis,
  ...
  def start_link(opts) do
    processor = fn job ->
      ...
      if job.opts[:deduplication] do
        ...
        EchoMQ.Queue.remove_deduplication_key("my-queue", dedup_id,
    ...
    processor = fn job ->
      case job.name do
        "scheduler-trigger" ->
          ...
          EchoMQ.Queue.add("tasks", "actual-task", %{},
            connection: Keyword.fetch!(opts, :connection),
...
EchoMQ.Queue.upsert_job_scheduler("tasks", "every-minute",
  connection: :redis,
  pattern: "* * * * *",
