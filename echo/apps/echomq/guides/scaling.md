# Scaling EchoMQ Elixir

This guide explains how to scale EchoMQ workers in Elixir, comparing it with Node.js BullMQ patterns and showing best practices for production deployments.
...
Each machine runs independently. EchoMQ's Lua scripts ensure atomic job distribution - no coordination between machines needed.
...
EchoMQ Elixir uses OTP supervision for fault tolerance:
...
# Good - supervised
children = [
  {EchoMQ.Worker, queue: "jobs", ...}
]
Supervisor.start_link(children, strategy: :one_for_one)

# Avoid - unsupervised
{:ok, worker} = EchoMQ.Worker.start_link(queue: "jobs", ...)
```
...
defmodule MyApp.WorkerManager do
  ...
  def handle_call({:scale_down, count}, _from, state) do
    ...
    Enum.each(to_stop, fn pid ->
      ...
      DynamicSupervisor.terminate_child(EchoMQ.WorkerSupervisor, pid)
    end)
