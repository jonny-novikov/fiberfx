...
EchoMQ Elixir provides cooperative job cancellation, allowing you to gracefully stop running jobs. This is useful for implementing timeouts, user-initiated cancellations, or graceful shutdown.

## Overview

Job cancellation in EchoMQ Elixir is:

- **Cooperative**: Processors must check for cancellation; EchoMQ cannot forcibly stop a running function
...
When `Worker.cancel_job/3` is called, EchoMQ sends a `{:cancel, token, reason}` message to the task running your processor. Your processor can check for this message using various patterns.
...
```elixir
alias EchoMQ.CancellationToken
...
EchoMQ automatically cancels jobs when their lock renewal fails. This can happen due to:
