...
EchoMQ provides built-in rate limiting to control job processing rates.
...
```elixir
# Node A
{:ok, worker_a} = EchoMQ.Worker.start_link(
  queue: "api-calls",
  connection: :my_redis,
  processor: &process/1,
...
{:ok, worker_b} = EchoMQ.Worker.start_link(
  queue: "api-calls",
  connection: :my_redis,
  processor: &process/1,
  limiter: %{max: 100, duration: 60_000}
...
EchoMQ uses a fixed window rate limiter:
...
{:ok, api_worker} = EchoMQ.Worker.start_link(
  queue: "api-calls",
  connection: :my_redis,
...
{:ok, email_worker} = EchoMQ.Worker.start_link(
  queue: "emails",
  connection: :my_redis,
...
{:ok, internal_worker} = EchoMQ.Worker.start_link(
  queue: "internal",
  connection: :my_redis,
...
EchoMQ.Queue.add(queue, "api-call", %{},
  attempts: 5,
