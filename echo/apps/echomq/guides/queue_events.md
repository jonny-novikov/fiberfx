...
EchoMQ provides real-time event subscriptions through Redis Streams, allowing you to monitor job lifecycle events across your queue.

## Overview

There are two ways to receive job events in EchoMQ for Elixir:
...
For monitoring all queue events (including jobs processed by other workers or Node.js workers), use `EchoMQ.QueueEvents`:
...
EchoMQ.QueueEvents.subscribe(events)  # subscribes self()
EchoMQ.QueueEvents.subscribe(events, other_pid)

# Unsubscribe when done
EchoMQ.QueueEvents.unsubscribe(events)
EchoMQ.QueueEvents.unsubscribe(events, other_pid)
```
...
EchoMQ.QueueEvents.close(events)
```
