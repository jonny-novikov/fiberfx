...
The `EchoMQ.JobScheduler` module provides functions for creating, listing, and managing scheduled jobs. Unlike one-time jobs, schedulers automatically create new jobs at specified intervals.
...
```elixir
# Run every minute
{:ok, job} = EchoMQ.JobScheduler.upsert(:my_redis, "my_queue", "heartbeat",
...
# Run every 5 seconds
{:ok, job} = EchoMQ.JobScheduler.upsert(:my_redis, "my_queue", "fast-check",
  %{every: 5_000},
  "check",
  %{},
  prefix: "bull"
...
{:ok, job} = EchoMQ.JobScheduler.upsert(:my_redis, "reports", "hourly-report",
  %{pattern: "0 * * * *"},
  "generate-report",
  %{type: "hourly"},
  prefix: "bull"
...
{:ok, job} = EchoMQ.JobScheduler.upsert(:my_redis, "emails", "daily-digest",
  %{pattern: "0 9 * * *"},
  "send-digest",
  %{},
  prefix: "bull"
...
{:ok, job} = EchoMQ.JobScheduler.upsert(:my_redis, "notifications", "workday-reminder",
  %{pattern: "0 18 * * 1-5"},
  "send-reminder",
  %{},
  prefix: "bull"
...
```elixir
{:ok, schedulers} = EchoMQ.JobScheduler.list(:my_redis, "my_queue", prefix: "bull")
...
```elixir
{:ok, scheduler} = EchoMQ.JobScheduler.get(:my_redis, "my_queue", "hourly-report",
  prefix: "bull")
...
```elixir
{:ok, count} = EchoMQ.JobScheduler.count(:my_redis, "my_queue", prefix: "bull")
...
```elixir
{:ok, removed} = EchoMQ.JobScheduler.remove(:my_redis, "my_queue", "hourly-report",
  prefix: "bull")
...
{:ok, _} = EchoMQ.JobScheduler.upsert(:my_redis, "my_queue", "my-scheduler",
  %{every: 60_000},
  "job-name",
  %{version: 1},
  prefix: "bull"
...
```elixir
# Run only 5 times
{:ok, job} = EchoMQ.JobScheduler.upsert(:my_redis, "my_queue", "limited",
...
{:ok, job} = EchoMQ.JobScheduler.upsert(:my_redis, "my_queue", "delayed-start",
...
{:ok, job} = EchoMQ.JobScheduler.upsert(:my_redis, "my_queue", "time-limited",
...
```elixir
{:ok, job} = EchoMQ.JobScheduler.upsert(:my_redis, "my_queue", "immediate",
...
{:ok, job} = EchoMQ.JobScheduler.upsert(:my_redis, "my_queue", "tz-aware",
...
```elixir
{:ok, job} = EchoMQ.JobScheduler.upsert(:my_redis, "my_queue", "with-options",
  %{every: 60_000},
  "job-name",
  %{data: "value"},
  prefix: "bull",
...
```elixir
# Create scheduler
{:ok, _} = EchoMQ.JobScheduler.upsert(:my_redis, "maintenance", "cleanup",
  %{pattern: "0 * * * *"},
  "run-cleanup",
  %{type: "hourly"},
  prefix: "bull"
...
```elixir
EchoMQ.JobScheduler.upsert(:my_redis, "health", "api-health",
...
```elixir
EchoMQ.JobScheduler.upsert(:my_redis, "reports", "daily-summary",
...
```elixir
EchoMQ.JobScheduler.upsert(:my_redis, "cache", "refresh-cache",
