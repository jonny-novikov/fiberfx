...
When a parent job is processed, it can access the results of its children using methods on `EchoMQ.Job`:

```elixir
# Child processor
def process(%EchoMQ.Job{name: "fetch-data"} = job) do
  data = fetch_from_source(job.data["source"])
  {:ok, %{data: data, count: length(data)}}
end

# Parent processor
def process(%EchoMQ.Job{name: "aggregate"} = job) do
...
- **`EchoMQ.Job.get_children_values/1`** - Get return values from completed children
- **`EchoMQ.Job.get_ignored_children_failures/1`** - Get failures from children that were ignored
- **`EchoMQ.Job.get_dependencies/1`** - Get list of pending child job keys
- **`EchoMQ.Job.get_dependencies_count/1`** - Get count of pending dependencies
...
{:ok, results} = EchoMQ.FlowProducer.add_bulk(flows, connection: :my_redis)
```
...
3. Parent can access failed children via `EchoMQ.Job.get_ignored_children_failures/1`
