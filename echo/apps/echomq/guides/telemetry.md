...
EchoMQ provides two complementary observability systems:
...
EchoMQ's trace context is compatible with Node.js BullMQ (using \`bullmq-otel\`). A trace can flow seamlessly:
...
EchoMQ.Queue.add("queue", "job", %{}, omit_context: true)
```
...
EchoMQ also integrates with Elixir's \`:telemetry\` library for local metrics and monitoring. This is separate from OpenTelemetry and is used for metrics aggregation (counters, histograms, gauges).
...
```elixir
defmodule MyApp.EchoMQTelemetry do
require Logger
