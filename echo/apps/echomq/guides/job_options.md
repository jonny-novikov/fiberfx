...
EchoMQ provides extensive options for customizing job behavior.

## Adding Jobs

Jobs are added using the `EchoMQ.Queue.add/4` function:

```elixir
{:ok, job} = EchoMQ.Queue.add(queue_name, job_name, data, opts)
```
...
EchoMQ.Queue.add("tasks", "urgent-task", %{},
  connection: :my_redis,
  priority: 1
)

# Normal priority (default)
EchoMQ.Queue.add("tasks", "normal-task", %{},
  connection: :my_redis
)

# Low priority job (processed last)
EchoMQ.Queue.add("tasks", "batch-task", %{},
  connection: :my_redis,
  priority: 100
)
```
...
```elixir
# Run in 5 minutes
EchoMQ.Queue.add("reminders", "send-reminder", %{message: "Don't forget!"},
  connection: :my_redis,
...
EchoMQ.Queue.add("reports", "scheduled-report", %{},
  connection: :my_redis,
  delay: delay
)
```
...
EchoMQ.Queue.add("api-calls", "call-api", %{url: "..."},
  connection: :my_redis,
  attempts: 3,
...
```elixir
# Using custom job ID
EchoMQ.Queue.add("users", "process-user", %{user_id: 123},
  connection: :my_redis,
...
EchoMQ.Queue.add("tasks", "process", %{},
  connection: :my_redis,
...
EchoMQ.Queue.add("tasks", "process", %{data: "latest"},
  connection: :my_redis,
  delay: 5_000,
...
EchoMQ.Queue.add("urgent", "urgent-task", %{},
  connection: :my_redis,
  lifo: true
)
```
...
{:ok, added_jobs} = EchoMQ.Queue.add_bulk("emails", jobs, connection: :my_redis)
```
