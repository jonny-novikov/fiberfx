...
EchoMQ is a robust, feature-rich message queue and job scheduling library for Elixir, built on top of Redis. It's a library build on top of [BullMQ](https://bullmq.io) protocol from the Node.js ecosystem, providing full compatibility with existing EchoMQ queues.
...
Add EchoMQ to your dependencies:
...
{:ok, job} = EchoMQ.Queue.add("emails", "send-welcome", %{
  to: "user@example.com",
  template: "welcome"
}, connection: :my_redis)
```
...
defmodule MyApp.EmailWorker do
  def process(%EchoMQ.Job{name: "send-welcome", data: data}) do
...
EchoMQ uses Redis data structures to implement a reliable, distributed job queue:
...
EchoMQ for Elixir provides both stateless and stateful APIs:
...
```elixir
# Add a job
{:ok, job} = EchoMQ.Queue.add("my_queue", "job_name", %{data: "value"},
  connection: :my_redis)
