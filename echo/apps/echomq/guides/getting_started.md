...
This guide walks you through setting up EchoMQ in your Elixir application.
...
EchoMQ uses [Redix](https://hexdocs.pm/redix) for Redis connections. Add a Redix connection to your supervision tree:
...
```elixir
# Add a simple job
{:ok, job} = EchoMQ.Queue.add("notifications", "push-notification", %{
  user_id: 123,
...
```elixir
# Check job state
{:ok, state} = EchoMQ.Queue.get_job_state("notifications", job.id,
  connection: :my_redis)
...
```elixir
# Delayed job
EchoMQ.Queue.add("emails", "send-reminder", %{user_id: 123},
  connection: :my_redis,
  delay: 60_000  # 1 minute delay
)

# Priority job
EchoMQ.Queue.add("emails", "urgent-alert", %{},
  connection: :my_redis,
  priority: 1  # Lower = higher priority
)

# Job with retries
EchoMQ.Queue.add("api-sync", "sync-data", %{},
  connection: :my_redis,
  attempts: 5,
...
By default, EchoMQ uses `"bull"` as a prefix for all Redis keys. You can customize this:

```elixir
# When adding jobs
EchoMQ.Queue.add("emails", "send", %{},
  connection: :my_redis,
  prefix: "myapp"
)
