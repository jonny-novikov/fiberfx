...
defmodule MyApp.EmailWorker do
  def process(%EchoMQ.Job{data: data}) do
...
{:ok, worker} = EchoMQ.Worker.start_link(
  ...
  telemetry: EchoMQ.Telemetry.OpenTelemetry, # OpenTelemetry integration (optional)
...
test "processes job successfully" do
  ...
  {:ok, job} = EchoMQ.Queue.add("test-queue", "test", %{value: 21},
    connection: :my_redis)
...
test "reports progress correctly" do
  ...
  {:ok, worker} = EchoMQ.Worker.start_link(
    ...
    connection: :my_redis,
    processor: fn job ->
      EchoMQ.Worker.update_progress(job, 50)
  ...
  {:ok, job} = EchoMQ.Queue.add("test-queue", "test", %{},
    connection: :my_redis)
...
def process(%{name: "process-batch"} = job) do
  ...
  Enum.each(job.data["items"], fn item ->
    EchoMQ.FlowProducer.add_child(job, "process-item", item)
  end)
...
- Parent can access child results via `EchoMQ.Job.get_children_values/1`
...
If your processor raises an exception, exits, or throws a value, EchoMQ catches it
...
{:ok, job} = EchoMQ.Queue.add("emails", "send", %{to: "user@example.com"},
  connection: :redis,
  attempts: 3,
...
```elixir
{:ok, job} = EchoMQ.Queue.get_job("my-queue", job_id, connection: :redis)
...
1. **Let it crash for unexpected errors** - EchoMQ handles retries automatically
...
EchoMQ automatically renews locks at half the lock duration interval.
...
EchoMQ automatically detects and recovers stalled jobs.
...
def process(%EchoMQ.Job{} = job) do
  ...
  {:ok, 1} = EchoMQ.Job.log(job, "Starting processing")

  result = do_work(job.data)

  {:ok, 2} = EchoMQ.Job.log(job, "Completed with result: #{inspect(result)}")
  {:ok, result}
end
```
...
def process(%EchoMQ.Job{} = job) do
  ...
  :ok = EchoMQ.Worker.log(job, "Starting processing")

  result = do_work(job.data)

  :ok = EchoMQ.Worker.log(job, "Completed with result: #{inspect(result)}")
  {:ok, result}
end
```
...
EchoMQ.Job.log(job, "Processing step 1", keep_logs: 10)
EchoMQ.Job.log(job, "Processing step 2", keep_logs: 10)
```

You can also set this globally when adding a job:

```elixir
{:ok, job} = EchoMQ.Queue.add("my-queue", "job-name", %{data: "value"},
  keep_logs: 100)
```
...
:ok = EchoMQ.Worker.close(worker)

# Force close without waiting for jobs
:ok = EchoMQ.Worker.close(worker, force: true)
```
...
:ok = EchoMQ.Worker.pause(worker)

# Resume processing
:ok = EchoMQ.Worker.resume(worker)

# Check if paused
EchoMQ.Worker.paused?(worker)
# => true
```
