...
- **RENAMED**: Package renamed from BullMQex to EchoMQ
  - 53 Elixir source files updated (`BullMQex.*` modules → `EchoMQ.*`)
  - Package directory moved from `phoenix/apps/bullmqex/` → `phoenix/apps/echomq/`
  - hex.pm package name: `echomq` (previously `bullmq`)
  - Redis key prefix `"bull"` preserved for wire compatibility with Node.js BullMQ workers
  - All existing `BullMQ.*` module APIs unchanged (Queue, Worker, Job, FlowProducer, etc.)
