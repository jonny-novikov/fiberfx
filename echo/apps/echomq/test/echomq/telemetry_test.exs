defmodule EchoMQ.TelemetryTest do
  use ExUnit.Case, async: true

  alias EchoMQ.Telemetry
  ...
  describe "emit/3" do
    test "emits telemetry event with atom name" do
      ...
      assert_receive {:telemetry_event, [:echomq, :test_event], %{value: 42}, %{queue: "test"}}
    ...
    test "emits telemetry event with list name" do
      ...
      assert_receive {:telemetry_event, [:echomq, :job, :complete], %{duration: 100},
                      %{job_id: "123"}}
  ...
  describe "attach/4" do
    test "attaches handler to echomq-prefixed event" do
      ref = make_ref()
      test_pid = self()
      ...
      :telemetry.execute([:echomq, :custom, :event], %{data: 1}, %{})

      assert_receive {:handler_called, [:echomq, :custom, :event], %{data: 1}, %{}}
  ...
  describe "attach_many/4" do
    test "attaches handler to multiple events" do
      ...
      :telemetry.execute([:echomq, :event, :one], %{}, %{})
      :telemetry.execute([:echomq, :event, :two], %{}, %{})

      assert_receive {:event_received, [:echomq, :event, :one]}
      assert_receive {:event_received, [:echomq, :event, :two]}
  ...
  describe "span/3" do
    test "emits start and stop events" do
      ...
      :telemetry.attach_many(
        ...
        [
          [:echomq, :operation, :start],
          [:echomq, :operation, :stop]
        ],
        fn event, measurements, metadata, _config ->
      ...
      assert_receive {:span_event, [:echomq, :operation, :start], %{system_time: _}, %{id: "op-1"}}

      assert_receive {:span_event, [:echomq, :operation, :stop], %{duration: duration},
                      %{id: "op-1"}}
    ...
    test "emits exception event on error" do
      ...
      assert_receive {:span_event, [:echomq, :operation, :start], _, _}
      assert_receive {:span_event, [:echomq, :operation, :exception], %{duration: _}, metadata}
  ...
  describe "convenience functions" do
    test "job_added/4 emits correct event" do
      ...
      assert_receive {:event, [:echomq, :job, :add], %{queue_time: 50}, metadata}
    ...
    test "job_completed/5 emits correct event" do
      ...
      assert_receive {:event, [:echomq, :job, :complete], %{duration: 1000}, metadata}
    ...
    test "job_failed/6 emits correct event" do
      ...
      assert_receive {:event, [:echomq, :job, :fail], %{duration: 500}, metadata}
      assert metadata.queue == "emails"
    ...
    test "rate_limit_hit/2 emits correct event" do
      ...
      assert_receive {:event, [:echomq, :rate_limit, :hit], %{delay: 5000}, %{queue: "api-calls"}}
