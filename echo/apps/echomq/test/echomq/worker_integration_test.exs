defmodule EchoMQ.WorkerIntegrationTest do
  @moduledoc """
  Integration tests for Worker functionality with a real Redis instance.

  These tests use the real EchoMQ.Worker, EchoMQ.Queue, and EchoMQ.QueueEvents
  ...
  alias EchoMQ.{Job, Queue, Worker, Keys, Scripts}

  @redis_url EchoMQ.TestHelper.redis_url()
  @test_prefix EchoMQ.TestHelper.test_prefix() <> "_worker"
  ...
  describe "Job failure and retry" do
    ...
    test "failed job stores stacktrace in Redis", %{conn: conn, queue_name: queue_name} do
      ...
      {:ok, job_data} = EchoMQ.RedisConnection.command(conn, ["HGETALL", job_key])
  ...
  describe "lock lost cancellation" do
    ...
    test "processor receives cancellation when lock is lost", %{conn: conn, queue_name: queue_name} do
      ...
      {:ok, _} = EchoMQ.RedisConnection.command(conn, ["DEL", lock_key])
