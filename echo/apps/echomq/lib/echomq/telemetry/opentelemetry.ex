defmodule EchoMQ.Telemetry.OpenTelemetry do
  @moduledoc """
  OpenTelemetry adapter for EchoMQ distributed tracing.

  This module implements the `EchoMQ.Telemetry.Behaviour` using OpenTelemetry APIs.
  ...
  ## Usage
      ...
      Tracer.with_span "my_operation" do
        ...
        {:ok, job} = EchoMQ.Queue.add(queue, "email", %{to: "user@example.com"})
      end
  ...
  EchoMQ creates spans with the following names:

  - `echomq.queue.add` - When adding a single job
  - `echomq.queue.add_bulk` - When adding multiple jobs
  - `echomq.worker.process` - When processing a job

  ## Span Attributes

  Spans include the following semantic attributes:

  - `messaging.system` - Always "echomq"
  ...
  - `echomq.job.name` - The job name/type
  - `echomq.job.priority` - The job priority
  - `echomq.job.delay` - The job delay in ms
  - `echomq.job.attempts` - Number of attempts made
  """
  ...
  defp get_tracer do
    if available?() do
      try do
        :opentelemetry.get_tracer(:echomq)
      rescue
        _ -> nil
      catch
        _, _ -> nil
  ...
  ## Examples
      ...
      # Simple trace
      EchoMQ.Telemetry.OpenTelemetry.trace("my.operation", [], fn _span ->
        do_work()
      end)

      # Trace with context propagation (for queue.add)
      EchoMQ.Telemetry.OpenTelemetry.trace("queue.add", [propagate: true], fn _span, metadata ->
