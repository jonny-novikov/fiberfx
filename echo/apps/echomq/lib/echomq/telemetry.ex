defmodule EchoMQ.Telemetry do
  @moduledoc """
  Telemetry integration for EchoMQ.

  EchoMQ emits various telemetry events that you can attach handlers to
  for metrics, logging, and monitoring.

  ## Event Names

  All events are prefixed with `[:echomq, ...]`.

  ### Job Events

    * `[:echomq, :job, :add]` - A job was added to the queue
      * Measurements: `%{queue_time: native_time}` (time to add to Redis)
      * Metadata: `%{queue: name, job_id: id, job_name: name}`

    * `[:echomq, :job, :start]` - A job started processing
      * Measurements: `%{system_time: native_time}`
      * Metadata: `%{queue: name, job_id: id, job_name: name, worker: pid}`

    * `[:echomq, :job, :complete]` - A job completed successfully
      * Measurements: `%{duration: native_time}`
      * Metadata: `%{queue: name, job_id: id, job_name: name, worker: pid}`

    * `[:echomq, :job, :fail]` - A job failed
      * Measurements: `%{duration: native_time}`
      * Metadata: `%{queue: name, job_id: id, job_name: name, worker: pid, error: term}`

    * `[:echomq, :job, :retry]` - A job is being retried
      * Measurements: `%{attempt: integer, delay: ms}`
      * Metadata: `%{queue: name, job_id: id, job_name: name}`

    * `[:echomq, :job, :progress]` - Job progress updated
      * Measurements: `%{progress: 0..100}`
      * Metadata: `%{queue: name, job_id: id}`

  ### Worker Events

    * `[:echomq, :worker, :start]` - Worker started
      * Measurements: `%{concurrency: integer}`
      * Metadata: `%{queue: name, worker: pid}`

    * `[:echomq, :worker, :stop]` - Worker stopped
      * Measurements: `%{uptime: native_time}`
      * Metadata: `%{queue: name, worker: pid}`

    * `[:echomq, :worker, :stalled_check]` - Stalled job check executed
      * Measurements: `%{recovered: integer, failed: integer}`
      * Metadata: `%{queue: name}`

  ### Queue Events

    * `[:echomq, :queue, :pause]` - Queue was paused
    * `[:echomq, :queue, :resume]` - Queue was resumed
    * `[:echomq, :queue, :drain]` - Queue was drained

  ### Rate Limiting Events

    * `[:echomq, :rate_limit, :hit]` - Rate limit was hit
  ...
  @doc """
  Attaches a handler function to EchoMQ events.

  This is a convenience wrapper around `:telemetry.attach/4`.

  ## Example

      EchoMQ.Telemetry.attach(
        "my-handler",
        [:job, :complete],
  ...
  def attach(handler_id, event_suffix, handler_fn, config \\ nil) do
    event = [:echomq | event_suffix]
    :telemetry.attach(handler_id, event, handler_fn, config)
  end

  @doc """
  Attaches a handler to multiple EchoMQ events.

  ## Example

      EchoMQ.Telemetry.attach_many("my-handler", [
        [:job, :complete],
        [:job, :fail],
        [:worker, :start]
      ], &handle_event/4)
  ...
  def attach_many(handler_id, event_suffixes, handler_fn, config \\ nil) do
    events = Enum.map(event_suffixes, fn suffix -> [:echomq | suffix] end)
  ...
  This is used internally by EchoMQ. You typically don't need to call this directly.
  """
  ...
  ## Example
      ...
      EchoMQ.Telemetry.span([:job, :process], %{job_id: "123"}, fn ->
        do_work()
      end)
  """
  ...
  def span(event_suffix, metadata, fun) do
    ...
    event_prefix = [:echomq | event_suffix]
