defmodule EchoMQ.Job do
  @moduledoc """
  Represents a job in a EchoMQ queue.
  ...
  ## Examples
      ...
      defmodule MyWorker do
        def process(job) do
          EchoMQ.Job.update_progress(job, 50)
          result = do_work(job.data)
          EchoMQ.Job.update_progress(job, 100)
          result
        end
      end
  ...
  alias EchoMQ.{Keys, RedisConnection, Scripts, Types}
  ...
  ## Parameters
    ...
    * `opts` - Job options (see `EchoMQ.Types.job_opts()`)

  ## Examples

      iex> job = EchoMQ.Job.new("my_queue", "email", %{to: "test@example.com"})
      iex> job.name
      "email"
  ...
  Note: For accurate state, use `EchoMQ.Queue.get_job_state/2` which
  ...
  ## Examples
      ...
      {:ok, ignored} = EchoMQ.Job.get_ignored_children_failures(job)
