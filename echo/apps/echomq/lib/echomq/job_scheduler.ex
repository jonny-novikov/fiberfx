defmodule EchoMQ.JobScheduler do
  @moduledoc """
  ...
  ## Usage
      ...
      {:ok, job} = EchoMQ.JobScheduler.upsert(conn, queue_name, "daily_report",
      ...
      {:ok, job} = EchoMQ.JobScheduler.upsert(conn, queue_name, "heartbeat",
  ...
  ## Examples
      ...
      # Every day at 9 AM
      {:ok, job} = EchoMQ.JobScheduler.upsert(conn, "emails", "daily_digest",
        %{pattern: "0 9 * * *"},
        "send_digest",
        %{recipient: "all"},
        priority: 10
      ...
      {:ok, job} = EchoMQ.JobScheduler.upsert(conn, "health", "heartbeat",
      ...
      {:ok, job} = EchoMQ.JobScheduler.upsert(conn, "sync", "data_sync",
  ...
  def upsert(conn, queue_name, scheduler_id, repeat_opts, job_name, job_data \\ %{}, opts \\ []) do
    ...
    case validate_scheduler_id(scheduler_id) do
      ...
      {:error, {:invalid_scheduler_id, message}} = error ->
        Logger.error("[EchoMQ.JobScheduler] #{message}")
        error

      {:error, reason} = error ->
        Logger.error("[EchoMQ.JobScheduler] Invalid scheduler_id: #{inspect(reason)}")
        error
    end
    |> case do
      :ok ->
        case validate_repeat_opts(repeat_opts) do
          ...
          {:error, reason} = error ->
            Logger.error("[EchoMQ.JobScheduler] Invalid repeat options: #{inspect(reason)}")
            error
        end
  ...
  ## Examples
      ...
      {:ok, scheduler} = EchoMQ.JobScheduler.get(conn, "my_queue", "daily_report")
      # => %{
      #   key: "daily_report",
      #   name: "daily_report",
      #   pattern: "0 9 * * *",
  ...
  ## Examples
      ...
      {:ok, schedulers} = EchoMQ.JobScheduler.list(conn, "my_queue")
      {:ok, first_10} = EchoMQ.JobScheduler.list(conn, "my_queue", start: 0, end: 9)
  """
  ...
  ## Examples
      ...
      {:ok, count} = EchoMQ.JobScheduler.count(conn, "my_queue")
      # => {:ok, 5}
  """
  ...
  ## Examples
      ...
      {:ok, true} = EchoMQ.JobScheduler.remove(conn, "my_queue", "daily_report")
  """
  ...
  ## Examples
      ...
      # Next minute
      next = EchoMQ.JobScheduler.calculate_next_millis(%{every: 60_000}, now)

      # Next cron execution
      next = EchoMQ.JobScheduler.calculate_next_millis(%{pattern: "0 * * * *"}, now)
  """
