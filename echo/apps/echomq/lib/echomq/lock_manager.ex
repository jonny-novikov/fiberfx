defmodule EchoMQ.LockManager do
  @moduledoc """
  Manages lock renewal for EchoMQ workers.
  ...
  defp extend_locks(jobs_to_extend, state) do
    ...
    case Scripts.extend_locks(state.connection, state.keys, job_ids, tokens, state.lock_duration) do
        ...
        updated_state =
          if length(failed_ids) > 0 do
            ...
            Enum.each(failed_ids, fn job_id ->
              Logger.warning("[EchoMQ.LockManager] Lost lock for job #{job_id}")
            end)
      ...
      {:error, reason} ->
        Logger.error("[EchoMQ.LockManager] Error extending locks: #{inspect(reason)}")
        state
    end
  end
  ...
  defp emit_callback(callback, args) when is_function(callback) do
    try do
      ...
      e ->
        Logger.warning("[EchoMQ.LockManager] Callback failed: #{Exception.message(e)}")
    end
