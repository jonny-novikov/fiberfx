defmodule EchoMQ.CancellationToken do
  @moduledoc """
  Provides cooperative job cancellation for EchoMQ workers.
  ...
  From outside the processor, cancel running jobs via the Worker:
      ...
      EchoMQ.Worker.cancel_job(worker, job_id, "User requested cancellation")

      # Cancel all running jobs
      EchoMQ.Worker.cancel_all_jobs(worker, "Worker shutting down")
  ...
  Useful for simple checkpoint-style cancellation:
      ...
      def process_items(items, token) do
        Enum.map(items, fn item ->
          EchoMQ.CancellationToken.check!(token)
          process_item(item)
        end)
      end
  """
