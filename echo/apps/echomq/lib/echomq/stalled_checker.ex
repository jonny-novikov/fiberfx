defmodule EchoMQ.StalledChecker do
  ...
  EchoMQ uses a two-phase stalled job detection:
  ...
  You can also run the stalled check manually:
      ...
      EchoMQ.StalledChecker.check(:redis, "emails")
  """

  use GenServer
  require Logger

  alias EchoMQ.{Keys, RedisConnection, Scripts, Telemetry}
    ...
    case do_check(state.connection, ctx, state.max_stalled_count) do
      {:ok, result} ->
        if result.recovered > 0 or result.failed > 0 do
          Logger.info(
            "[EchoMQ] Stalled check for #{state.queue}: " <>
              "recovered=#{result.recovered}, failed=#{result.failed}"
          )
        end

      {:error, reason} ->
        Logger.warning("[EchoMQ] Stalled check failed for #{state.queue}: #{inspect(reason)}")
    end
