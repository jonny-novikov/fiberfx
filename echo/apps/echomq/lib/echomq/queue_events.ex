defmodule EchoMQ.QueueEvents do
  @moduledoc """
  ...
  ## Usage
      ...
      {:ok, pid} = EchoMQ.QueueEvents.start_link(
        queue: "my_queue",
        connection: :redis
      )

      # Subscribe to events
      EchoMQ.QueueEvents.subscribe(pid, self())

      # Receive events
      receive do
        {:echomq_event, :completed, %{job_id: id, returnvalue: value}} ->
          IO.puts("Job \#{id} completed with \#{value}")

        {:echomq_event, :failed, %{job_id: id, failed_reason: reason}} ->
  ...
  alias EchoMQ.{Keys, RedisConnection, Types}
  ...
  @opts_schema NimbleOptions.new!(
                 ...
                 handler: [
                   type: {:or, [:atom, nil]},
                   doc:
                     "Optional handler module implementing `EchoMQ.QueueEvents.Handler` behaviour."
                 ],
                 handler_state: [
                   type: :any,
  ...
  Events are sent as `{:echomq_event, event_type, event_data}` messages.
  """
  ...
  def handle_info(:start, state) do
    case RedisConnection.blocking_connection(state.connection) do
      ...
      {:error, reason} ->
        Logger.error(
          "[EchoMQ.QueueEvents] Failed to create blocking connection: #{inspect(reason)}"
        )
  ...
  def handle_info({ref, result}, %{consumer_task: ref} = state) when is_reference(ref) do
    ...
    case result do
      ...
      {:error, reason} ->
        Logger.error("[EchoMQ.QueueEvents] Error reading events: #{inspect(reason)}")
  ...
  def handle_info({:DOWN, ref, :process, _pid, reason}, %{consumer_task: ref} = state) do
    ...
    Logger.error("[EchoMQ.QueueEvents] Consumer task crashed: #{inspect(reason)}")
    new_state = %{state | consumer_task: nil}
  ...
  defp process_events(events, state) do
    Enum.reduce(events, state, fn [event_id, fields], acc ->
      ...
      Enum.each(acc.subscribers, fn pid ->
        send(pid, {:echomq_event, event_type, event_data})
      end)
...
defmodule EchoMQ.QueueEvents.Handler do
  @moduledoc """
  ...
  defmacro __using__(_opts) do
    quote do
      @behaviour EchoMQ.QueueEvents.Handler
