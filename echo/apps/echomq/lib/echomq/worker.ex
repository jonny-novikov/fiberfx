defmodule EchoMQ.Worker do
  @moduledoc """
  Worker process for processing EchoMQ jobs.
  ...
  The processor function receives a `EchoMQ.Job` struct and should return:
  ...
  ## Examples
      ...
      defmodule MyApp.Jobs do
        def process(%EchoMQ.Job{name: "email", data: data}) do
        ...
        def process(%EchoMQ.Job{name: "heavy_task", data: data} = job) do
          # Update progress
          EchoMQ.Worker.update_progress(job, 0)

          result = do_work(data)

          EchoMQ.Worker.update_progress(job, 100)
          {:ok, result}
        end
      end
  ...
  alias EchoMQ.{CancellationToken, Job, Keys, LockManager, RedisConnection, Scripts, Types}
  ...
  @type processor ::
          ...
          | (Job.t(), String.t(), EchoMQ.CancellationToken.t() ->
               {:ok, term()} | :ok | {:error, term()} | {:delay, non_neg_integer()})
  ...
  - Checking `EchoMQ.CancellationToken.cancelled?(cancel_token)`
  - Subscribing with `EchoMQ.CancellationToken.subscribe(cancel_token)`
  ...
  ## Example
      ...
      EchoMQ.Worker.cancel_job(worker, "job-123", "User requested cancellation")
  """
  @spec cancel_job(GenServer.server(), String.t(), EchoMQ.CancellationToken.reason()) :: boolean()
  ...
  ## Example
      ...
      EchoMQ.Worker.cancel_all_jobs(worker, "System shutdown")
  """
  @spec cancel_all_jobs(GenServer.server(), EchoMQ.CancellationToken.reason()) :: :ok
  ...
  by Node.js EchoMQ. The function will:
  ...
  def handle_info(:start, state) do
    ...
    case RedisConnection.blocking_connection(state.connection) do
      ...
      {:error, reason} ->
        Logger.error("[EchoMQ.Worker] Failed to create blocking connection: #{inspect(reason)}")
        # Retry after a delay
  ...
  def handle_info({:EXIT, pid, reason}, state) when pid == state.lock_manager do
    Logger.error("[EchoMQ.Worker] LockManager crashed: #{inspect(reason)}, restarting worker")
  ...
  def handle_info({:cancel_jobs_lock_lost, job_ids}, state) do
    ...
    Enum.each(job_ids, fn job_id ->
      case Map.get(state.cancellation_tokens, job_id) do
        nil ->
          Logger.warning(
            "[EchoMQ.Worker] Lock lost for job #{job_id} but no cancellation token found"
          )

        {token, processor_pid} ->
          Logger.warning("[EchoMQ.Worker] Lock lost for job #{job_id}, cancelling processor")
          ...
          if state.on_lock_renewal_failed do
            try do
              ...
              e ->
                Logger.warning(
                  "[EchoMQ.Worker] on_lock_renewal_failed callback failed: #{Exception.message(e)}"
                )
            end
          end
      end
  ...
  defp attach_worker_client_name_handler(blocking_conn, state) do
    client_name = worker_client_name(state)
    handler_id = {:echomq_worker_client_name, self(), make_ref()}
  ...
  defp fetch_and_process_jobs(state, count) do
    case fetch_next_job(state) do
      ...
      {:error, reason} ->
        Logger.error("[EchoMQ.Worker] Error fetching job: #{inspect(reason)}")
  ...
  defp fetch_next_job(state) do
    ...
    case Scripts.move_to_active(state.connection, state.keys, state.token, opts) do
      ...
      when is_list(job_data) and job_data != [] ->
        # Convert flat list to map (EchoMQ returns [key1, val1, key2, val2, ...])
  ...
  defp wait_for_job(state, timeout_seconds) do
    ...
    if blocking_conn && Process.alive?(blocking_conn) do
      ...
      case RedisConnection.blocking_connection(state.connection) do
        ...
        {:error, reason} ->
          Logger.warning("[EchoMQ.Worker] Failed to create blocking connection: #{inspect(reason)}")
          {:error, reason}
      end
    end
  end
  ...
  defp run_processor(processor_fn, job, worker_pid, telemetry_mod, metadata) do
    ...
    span_opts = [
      ...
      attributes: %{
        ...
        "echomq.job.attempts" => job.attempts_made
      }
    ]

    span = telemetry_mod.start_span("echomq.worker.process", span_opts)
  ...
  defp schedule_next_repeatable_job(%Job{repeat_job_key: scheduler_id} = job, state) do
    ...
    if length(key_parts) < 5 do
      ...
      Logger.warning(
        "[EchoMQ.Worker] Job #{job.id} has repeat_job_key with #{length(key_parts)} colon-separated parts. " <>
      ...
      try do
        ...
        case Scripts.update_job_scheduler(
          ...
          {:ok, nil} ->
            ...
            Logger.warning(
              "[EchoMQ.Worker] Failed to schedule next iteration for scheduler '#{scheduler_id}': " <>
          ...
          {:error, reason} ->
            Logger.error(
              "[EchoMQ.Worker] Error scheduling next iteration for scheduler '#{scheduler_id}': #{inspect(reason)}"
            )
        end
      rescue
        e ->
          Logger.error(
            "[EchoMQ.Worker] Exception scheduling next iteration: #{Exception.message(e)}"
          )
      end
    end)
