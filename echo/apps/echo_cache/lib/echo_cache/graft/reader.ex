defmodule EchoCache.Graft.Reader do
  @moduledoc """
  ...
  Three paths, cheapest first:
    ...
    1. **Head read** — a straight `EchoCache.Table.fetch/3`, i.e. an `:ets.lookup`
    ...
    2. **Snapshot read** — `EchoCache.Graft.Store.page_at/3` against CubDB's
  ...
  # read is a direct lock-free `:ets.lookup` — not `EchoCache.Table.fetch/3`,
  ...
  defp lazy_fetch(%{conn: nil}, _snap, _page_idx), do: :absent

  defp lazy_fetch(%{conn: conn, volume_id: volume_id} = ctx, %Snapshot{} = snap, page_idx) do
    with {:ok, segment_id} <- Snapshot.segment_for(snap, page_idx),
         {:ok, %Segment{} = seg} <- Sync.fetch_segment(conn, volume_id, segment_id, snap.lsn),
  ...
  defp cache(%{table: table}, page_idx, bin) do
    ...
    _ = EchoCache.Table.put(table, {:page, page_idx}, bin)
    :ok
  end
end
