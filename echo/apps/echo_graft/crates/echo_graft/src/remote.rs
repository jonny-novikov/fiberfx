...
impl Remote {
    pub fn with_config(config: RemoteConfig) -> Result<Self> {
        let store = match config {
            ...
            RemoteConfig::S3Compatible { bucket, prefix } => {
                ...
                // echo_graft fork: `tcp_user_timeout` maps to the Linux
                // TCP_USER_TIMEOUT socket option and is only compiled on
                // Linux/Android, so cfg-gate it to keep the fork building on
                // macOS. Upstream Linux behavior is preserved verbatim (FORK.md).
    ...
    /// TESTUTIL: list every object path under `prefix` (recursive).
    ///
    /// Exposed under the `testutil` feature so the eg.2 sync/fence suite (in the
    /// `echo_graft_test` crate) can assert remote object counts — e.g. that a
    /// rolled-up push writes exactly one segment, or that a raced commit leaves
    /// exactly one object under `logs/{logid}/commits/`.
    #[cfg(feature = "testutil")]
    pub async fn testutil_list(&self, prefix: &str) -> Result<Vec<String>> {
        let entries = self.store.list_with(prefix).recursive(true).await?;
        Ok(entries.iter().map(|e| e.path().to_string()).collect())
    }
