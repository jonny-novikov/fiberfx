defmodule EchoMQ.ConformanceRunTest do
  ...
  the seventy-nine-scenario harness drives the public surface against Valkey
  on 6390 and every scenario passes — `run/2 → {:ok, 79}` (the eighteen
  ...
  overwrites, clear_archived DELetes and the seam is :empty again), and --
  time-travel (emq3.6) -- the mint-time window read's one (stream_time_travel:
  ...
  EQUALS the id-filtered full read and ACTUALLY excludes the below/above records,
  the bounds exact at the millisecond)). Scenarios run on per-scenario
  ...
  test "the seventy-nine-scenario harness passes whole against the truth row" do
    ...
    assert Conformance.run(conn, q) == {:ok, 79}
  end
end
