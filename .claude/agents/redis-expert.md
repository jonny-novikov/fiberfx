...
description: >-
  ...
  EchoMQ backed by Valkey, EchoStore in front, codemojex the worked consumer — no
  ...
  quoted verbatim from the as-built echo data layer (echo/apps/echo_mq, echo/apps/echo_store,
  echo/apps/echo_wire) + the codemojex consumer (echo/apps/codemojex) + the committed BCS figures
  (never an invented Redis/Valkey command, Lua script, EchoMQ module, or echo-data-layer/codemojex
...
the **BCS architecture**: EchoMQ backed by Valkey, EchoStore in front, with the **codemojex**
(`echo/apps/codemojex`) the worked consumer. You produce the page(s) you are briefed to
...
5. **The grounding rule — cite ONE real artifact, quoted VERBATIM from the as-built code (never a spec, NEVER a
   ...
   **grounding map** and the reframe contract's **figure inventory** name — the as-built code above plus the
   ...
   `complete`, `retry`, `promote`, `reap`), the `attempts` fencing token (`HINCRBY`), an EchoStore figure
   ...
   or an echo-data-layer/codemojex surface. **Ground every pattern in the real as-built echo data layer —
   `echo/apps/echo_mq` (EchoMQ), `echo/apps/echo_store` (EchoStore), `echo/apps/echo_wire` (the EchoWire
   connector) — and codemojex consumer (`echo/apps/codemojex`); cite only surfaces verified on disk;
...
7. **Interactives — the hover-select pattern.** A dive carries TWO (one inside the hero figure, one in the main
   ...
   always names the EchoMQ / EchoStore application) and a `.take`.
...
Redis/Valkey/EchoMQ/echo-data-layer/codemojex surface** — `grep -rnoE '(Codemojex|EchoMQ|EchoStore|EchoWire)\.[A-Za-z.]+'`
...
`echo/apps/echo_store` · `echo/apps/echo_wire` · `echo/apps/codemojex`, the committed `docs/echo/bcs/content/`, or
...
key/command/Lua/Go fn or echo-data-layer/codemojex surface); `interactives` `[{control_ids, pure_function_signatures, sample_readout}]`;
