...
|---|---|---|
| Starts with `TSK` | **Resume** | Treat as `task_id`. |
...
**Cardinal correctness rule.** Every `mcp__aaw__tool_x_*` artifact tool requires `task_id` + `slug`. If neither is supplied and the user can't provide one, operate slug-only against local notes and tell the user that no audit artifacts will land in the scope's `<scope>.progress.md` ledger.

**Preflight — deferred tools.** `mcp__aaw__*` tool schemas start **deferred** in this Claude Code build (token-saving). Before the first call to `tool_x_*`, `aaw_*`, `agent_*`, `channel_*`, or `pantry_*`, load schemas via `ToolSearch(query: "select:<name>,<name>...")` — calling deferred tools directly returns `InputValidationError`. Verify with `ToolSearch(query: "+tool_x", max_results: 15)` if uncertain which exist.
...
```
1. mcp__aaw__aaw_init(scope: "<team>", operator: "<L0 name>")
2. mcp__aaw__aaw_spawn(role: "director", archetype: "director")
   → returns the director's CCL-id
3. mcp__aaw__agent_register(name: "<director>", role: "director")
...
5. For each peer in [Venus, Mars, Apollo, Pluto]:
   ...
   Each spawned subagent, from its OWN context, calls:
     - mcp__aaw__aaw_spawn(role: "<role>", archetype: "<archetype>", parent: "<director-CCL-id>")
     - mcp__aaw__agent_register(name: "<peer>", role: "<role>")
6. Coordinate via SendMessage(to: "<peer>", ...) OR
   mcp__aaw__agent_send / mcp__aaw__channel_publish.
```
...
| V-SOLO-4 | Orchestrator/Director authors a SYSTEM spec (architecture/ADR) directly, skipping the §12 Design Phase | system-spec files changed with no registered Venus spawn (fired 2026-06-10: the `emq.*` orchestrator-solo rewrite) |
...
`[epic-code].progress.md` for example `emq.progress.md` tracking ANSII real time `emq.roadmap.md` progression (EchoMQ v2 backed by Valkey Roadmap).

Append only entries are written by the following rules:

| Tool                    | Prefix | When to write                                                |
|-------------------------|--------|--------------------------------------------------------------|
| `tool_x_trace`          | T-n    | Non-trivial inference (per LAW-3.4 audit-trail discipline)   |
| `tool_x_analyze`        | A-n    | NxM-ready alternatives (use `draft:true` for sampled draft)  |
| `tool_x_alternative`    | V-n    | Inverted-flow self-prompt outputs                            |
| `tool_x_decision`       | D-n    | Every locked-in contract                                     |
| `tool_x_learning`       | L-n    | Surprises, counterintuitive findings                         |
| `tool_x_nxm_synthesize` | S-n    | Convergence across N analysts × M lenses                     |
| `tool_x_consensus`      | C-n    | Agreement matrix + score threshold                           |
| `tool_x_resonance`      | R-n    | Inter-agent resonance score                                  |
| `tool_x_escalation`     | E-n    | Blockers routed up/out                                       |
| `tool_x_progress`       | P-n    | Token budget, phase, blockers, next action                   |
| `tool_x_complete`       | Z-n    | Phase-completion exit criteria                               |
| `tool_x_report`         | Y-n    | Final deliverable + evidence + self-assessment               |
...
Flat-L2 aaw is the default per `docs/aaw/aaw.framework.md`. Solo-Director `/x` is the exception — MUST name the category (Trivial / User override / Bootstrap) before proceeding.

### LAW-3.4 — Audit-trail discipline

Every non-trivial inference, decision, alternative analysis, learning, and phase-completion is captured via the `tool_x_*` writers in §6.
...
- [ ] All cited tools verified to exist in `mcp__aaw__*` (use `ToolSearch` if uncertain)
...
## 12. Design Phase — system specs are team deliverables (never orchestrator-solo)

**Trigger.** The deliverable founds or re-founds a **SYSTEM spec** — a new protocol, library, or architecture (e.g. the `emq.*` ladder). Course-content fan-out is NOT this; an existing-triad build rung is NOT this (that is §5's pipeline, Venus Stage 1). The tell: there is no settled design to brief from — the design itself is the work.

**Formation** (the missing stage the 2026-06-10 `emq.*` incident exposed — the orchestrator rewrote the system spec solo, with no architectural design and no ADR; V-SOLO-4):

```
Venus-1 ∥ Venus-2  →  cross-review  →  Apollo evaluation  →  Director ratify  →  OPERATOR APPROVAL
(independent designs)  (each reviews     (convergence/divergence  (synthesis)      (feedback gate —
 + ADR set each         the sibling's)    table + §11.2 charter,                    nothing is canon,
                                          design-shaped)                            NO code, before it)
```

- **Venus-1 and Venus-2 spawn in one message** with IDENTICAL locked-constraints briefs but **distinct leading lenses** (e.g. wire/protocol-first vs conformance/operations-first) — perspective diversity by construction, the Echo-Chamber-Break applied up front. Neither reads the sibling's draft until both designs are on disk (the Director holds that gate).
- **Mandatory deliverable per Venus:** an architectural design document + **one ADR per architecture decision** (context → ≥2 steelmanned alternatives incl. a baseline → decision → consequences). The spec triad(s) DERIVE from the approved design afterward — never the reverse.
- **Locked constraints are verbatim and non-negotiable** — the Operator's settled decisions ride in the brief unchanged; a design that re-litigates one is BLOCKED.
- **Apollo evaluates designs, not builds** (the apollo.md design-phase charter): convergence/divergence table, ADR completeness, constraint fidelity, the §11.2 charter design-shaped, the echo-chamber probe, a synthesis recommendation.
- **The Operator approval is a real gate** — the Director ratifies the synthesis and STOPS; the Operator's explicit feedback admits the design. Only then is the design applied to the spec files, and only after THAT may any build rung start. **No production code exists before an approved design.**
- **Git:** design-phase output is spec/doc work — peers and Director run no git; the Operator commits out-of-band at approval (the fan-out discipline), unless the runbook explicitly assigns a LAW-4 pathspec commit.