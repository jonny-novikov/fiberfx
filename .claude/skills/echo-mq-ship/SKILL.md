...
description: >-
  ...
  echo-mq-implementor, Apollo loads echo-mq-evaluator). The INPUT is the rung's
  ...
  that already has a .prompt.md + spec triad. Do NOT use for the
  Exchange/trd platform (the generic x-mode), the static-HTML courses (the *-course-writer skills), the
  elixir rungs (the generic mars charter), or generic documents.
---
...
1. **`.claude/commands/x.md`** + the **`/x-mode`** skill — the LAWS (CLAUDE_LAWS 1/1a/2/3/4), the pipeline
   (Venus → Mars-1 → Director solo review → Mars-2 → Director ship; Apollo only on a high-risk rung), the §5
...
- **The build team is echo_mq-SPECIALIZED via SKILLS, not a dedicated agent.** Spawn each peer
  ...
  - **Apollo** (evaluator, high-risk only) → `.claude/agents/apollo.md` + **load `echo-mq-evaluator`** (the
    post-build reconcile, the adversarial echo_mq probes, the §11.2 charter).
  Each self-registers via `mcp__aaw__agent_register` from its own context (LAW-1; no narrated spawns).
...
- **The boundary** is `echo/apps/echo_mq` (+ the ONE named `echo/apps/echo_wire` connector seam a rung touches
  — the emq.1 resubscribe seam is the precedent). `apps/echomq` (the frozen v1 line) is **UNTOUCHED** — a
  feature reference, never an edit target. No third app. `echo/mix.lock` ships ONLY if a real dep moved (expect
  it EXCLUDED). A change that reaches a third app is a diff no one can review.
- **The gate ladder is the echo_mq one** (`.claude/skills/echo-mq-program.md` §The gate ladder), NOT a generic
  ...
  - **The ≥100 determinism loop** ONLY for an id-minting / process / engine suite (the same-ms branded-id mint
    ...
    not introduce.
- **The risk tier decides Apollo** (the `.prompt.md`'s declared tier): a rung with a **destructive at-rest
  operation** (a migration DELETE), a **new process or lease surface** (emq.2.3's worker-side lock plane), a
  **frozen-line touch**, or an **auth/deploy** dimension is **high-risk → Apollo MANDATORY** (the §11.2 charter
  + `AskUserQuestion` to keep it shippable). A **pure-read / additive-verb** rung (emq.2.1) is normal → the
  Director's solo review (Stage 2) + Venus's independent reconcile (Stage 4) are the verification floor, **no
  Apollo**.
...
**Venus** (authors the triad, or reconciles it lag-1 against the as-built `echo_mq`/`echo_wire` tree; surfaces
— never decides — the seam forks) → **Mars-1** (design-make + build to the brief inside the boundary, cite the
spec line for every public call, the inline `Script.new/2` law (never `priv/`), declared keys, branded `JOB`,
the `EMQ*` typed refusals; register the conformance scenario + probe in the same change; compile
`--warnings-as-errors`) → **Director solo review** (a REAL pass: fresh-gate reconcile + an independent gate
re-run on Valkey 6390 + ≥1 adversarial probe + a mutation spot-check, **Edit-in → test-catches → revert →
verify `git diff --stat` clean** net-zero, LAW-1a; the REMEDIATE list) → **Mars-2** (resume the Stage-1 Mars —
one identity, two passes — remediate + harden + the full gate ladder; REMEDIATE loop MAX 3) → **Director ship**
(the solo ship-gate + one LAW-4 commit + the Stage-6 fold). **Apollo runs between Stage 4 and Stage 5 ONLY on a
high-risk rung** (the §11.2 charter — prompted-checks + an un-prompted finding + an attack-that-held + a
mutation kill-rate + `AskUserQuestion`); its echo_mq probes are the order theorem (byte = mint), declared-keys
(grep every NEW Lua script — the F-1 class), no-invented-`EchoMQ.*`-surface, and the
destructive/at-most-once/non-atomic-read probes.
...
- **the watch plane (emq.2.3)** — `EchoMQ.Events` (the existing connector pub/sub seam — **no `SSUBSCRIBE`**) ·
  ...
  + lease rung → high-risk → Apollo + the ≥100 determinism loop.**
...
Every capability is grounded in a real `echo_mq`/`echo_wire` module (`echo-mq-surface.md`) or the v1 feature
reference (`apps/echomq`, ported under the v2 laws — never lifted) or a design §; NO-INVENT, forward-tense
("emq.N builds …") for an unshipped surface.

## 4. LAW-4 — the single ratifying commit (Director-only, per x.md §10)

At `tool_x_complete` (Z-n), exactly once: Apollo BUILD-GRADE (high-risk only) or the Director's solo review
clean + the echo_mq gate green; **≥1 `tool_x_decision` (D-n)** + the **Z-n** written this turn; `git status
...
- [ ] The Director's Stage-3 review was a real pass (echo_mq-gate re-run on Valkey 6390 + an adversarial probe
      incl. **declared-keys** + a mutation spot-check), not a glance; a high-risk rung got Apollo + the §11.2
      charter + every `AskUserQuestion` resolved.
- [ ] The echo_mq gate ladder is green: per-app compile `--warnings-as-errors` + per-app suites (NEVER
      ...
      (`apps/echomq` + the untouched apps + `mix.lock`).
