...
# vite.config.ts writes the hashed bundle to codemojex's committed priv/static/game via
# an ABSOLUTE echo path (the source reorg moved this app under mercury/, so a relative
# ../priv no longer resolves). OUT is resolved the same way — cd+pwd, so a wrong depth
# fails fast here instead of silently uploading an empty dir.
GAME_DIR="$(cd "$(dirname "$0")/.." && pwd)"
OUT="$(cd "$GAME_DIR/../../../../echo/apps/codemojex/priv/static" && pwd)/game"
cd "$GAME_DIR"
echo "--- build (vite.config.ts -> $OUT) ---"
